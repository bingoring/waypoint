...
**단계**
- [x] L4.1 `cmd/gencontent`에서 catalog 생성 제거, 관련 파일 삭제.
- [x] L4.2 v2 패키지 삭제. 컴파일러가 잔존 참조를 잡음 → 전부 포트로 이관 확인.
- [x] L4.3 구 가드 테스트 삭제, 엔진 불변식 테스트 추가(고아 0·주제≥임계·스텝 해석·부서 코어 리드).
- [x] L4.4 `/me/curriculum` 어댑터 삭제(L3 이행 완료 시) 또는 deprecation 주석.
- [x] L4.5 커밋: `refactor(curriculum): v2 하드코딩 카탈로그 은퇴 — 엔진 단일화`.

**green 게이트**: 전체 테스트, `grep -rn 'internal/curriculum"'` = 0, gencontent가 catalog_gen 미생성.
**이 지점부터 어떤 부서 seed 전환도 카탈로그 수동 재지정 불필요(R-L15).**

> ✅ **L4 완료(2026-09-20). L3보다 먼저 했다** — L3는 "P3-B 여정 지도 UI 스펙과 합류"인데 그 스펙이 아직
> 없고, 모바일에는 tracks **타입과 호출 함수만 있고 그것을 쓰는 화면이 없다**. L4.4가 "L3 이행 완료 시
> 어댑터 삭제, 아니면 deprecation 주석"으로 갈라 두어 L4는 L3 없이 실행 가능했다.
> 게이트 전부 통과 — v2 import 0 · v2 파일 0 · `catalog_gen` 미생성 · `go test -count=1 ./...` 그린 ·
> 계약 무드리프트 · 모바일 tsc 0 · jest 907/907.
>
> **시드 가드가 절반으로 줄었다(설계 변화).** 예전에는 하드코딩 카탈로그가 번들과 **독립적으로** id를
> 지명해서, 번들에서 시나리오가 빠지면 스텝이 허공을 가리켰다. 이제 경로가 **그 번들의 태그에서
> 파생**되므로 번들을 떠난 시나리오는 경로도 함께 떠난다 — 카탈로그 쪽 dangling이 구조적으로 불가능하다.
> 남은 절반(시도·복습 카드·대화 세션이 가리키는 id)만 지키면 된다.
>
> **불변식 5종을 새로 넣었다**(`themed/invariants_test.go`, 실제 콘텐츠 번들 위에서): 고아 0 · 모든
> 스텝이 실재 시나리오와 제목으로 해석됨 · 모든 주제가 임계 이상 · 부서 코어가 트랙을 이끎 · 신규
> 학습자에게 시작점이 있음. 구 가드(`TestEveryStepNamesItsContent`)는 **아무도 저지르지 않은 이유로
> 깨질 수 있었다** — 부서를 재저작해 제목이 바뀌면, 다른 곳에 쓰인 카탈로그가 옛 제목을 주장했다.
> 이제 두 번째 목록이 없어 그 실패 자체가 성립하지 않는다.
>
> ⚠️ **불변식이 실제 콘텐츠 소견을 하나 잡았다: ER의 세 주제가 20건이다**(`core-safety-er` ·
> `core-family-er` · `er-deescalation`). 저작 목표는 21인데 `cmd/audit`의 게이트는 20이라 여태
> "THIN 0"으로 보고돼 왔다. 불변식도 `cmd/audit`과 같은 20으로 맞춰 두 검사가 어긋나지 않게 했고,
> **3건의 공백은 콘텐츠 작업으로 남는다.**
