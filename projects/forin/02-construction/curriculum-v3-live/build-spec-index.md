...
| L-U4 | **콘텐츠 먼저 완주(29부서 전부 themed) → 라이브 전환 1회.** 런타임을 themed로 컷오버하면 미완 부서가 회귀하고, v2 삭제 시 미완 부서 경로가 사라지므로, 전환과 콘텐츠 완주는 묶여 있다 | AskUserQuestion(2026-09-14) "A로 가자" |
| L-U5 | **부서를 seed로 낼 때, 그 부서의 v2 authored 스텝을 안정 손저작 scn(`SCN-<D>-000xx`)으로 repoint**해 v2에 남긴다(스텝 이름 = 그 scn 제목). v2 전체는 최종 전환(L4)에서 wholesale 삭제한다 | 초안은 "블록 삭제"였으나, 삭제는 v2 완결성 테스트(모든 층·부서·인사가 v2에 존재)를 부서마다 깨뜨려 더 큰 churn을 유발한다. repoint는 부서를 v2에 온전히 남겨 완결성·green을 유지하며, 기존 11부서(커밋 7b5c7ed)와 동일한 방식이다 |
...
- **콘텐츠 fan-out과의 관계 (L-U4·L-U5 반영, 2026-09-14 갱신):** 라이브 전환(L2~L4)은 **29부서 콘텐츠 완주
  뒤에 한 번에** 한다(런타임 컷오버는 모든 부서가 themed임을 전제 — 미완 부서 회귀 방지). 그 전까지 콘텐츠
  fan-out은 **"seed 시 v2 스텝을 안정 손저작 scn으로 repoint"**로 green을 유지한다(L-U5): 부서를 seed로 내면 그
  부서의 `authored_*.go` 스텝이 가리키는 gen id(`001xx`)를 손저작 `000xx`로 바꾸고 이름을 그 제목에 맞춘다 →
  `TestEveryStepNamesItsContent`와 v2 완결성 테스트(층·부서·인사)가 모두 통과. 부서는 v2에 온전히 남고, 모든
  부서가 themed되면 최종 전환(L2~L4)에서 v2 전체를 wholesale 삭제한다. **L1(포트·엔진, additive)은 이미 착지**했고
  전환 기반으로 대기한다.
