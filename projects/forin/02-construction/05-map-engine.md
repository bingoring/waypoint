...
- 맵 엔진: [`../inputs/design-handoff_v11/05_MAP_AND_INTERIORS.md`](../inputs/design-handoff_v11/05_MAP_AND_INTERIORS.md)
- 캐릭터: [`../inputs/design-handoff_v11/03_CHARACTERS.md`](../inputs/design-handoff_v11/03_CHARACTERS.md)
- **캐릭터 모션(신규 2026-06-12):** [`../inputs/design-handoff_v11/06_CHARACTER_MOTION.md`](../inputs/design-handoff_v11/06_CHARACTER_MOTION.md)
...
## 캠퍼스 v20 — 핸드오프 v19 방사형 5-파빌리온 캠퍼스 1:1 (2026-08)

최신 핸드오프(v19 `screens-explore-v2` = **RADIAL CENTRAL-HUB**)로 캠퍼스를 재구현.
중앙 **Healing Garden 광장**(시계탑+동상)을 **트레이드마크 파빌리온 5동**이 둘러싼 형태 —
본관(N)·암센터(NW)·행정(NE)·외래(SW)·여성소아(SE). 남측 정문 애비뉴가 광장으로 이어지고
북측에 백스테이지 서비스 도로. 엔진 이동/카메라/충돌/핫스팟/엘리베이터는 그대로 재사용하고
**비주얼 레이어만 교체**:

- **건물**: 확정된 `landmarks.tsx`(buildings-v2 포트: MedCenter/MedCenterC/MedCenterV/
  MedCenterH/MedCenterAdmin + ClockTower)를 `type:'landmark'`로 그대로 사용. (초기에 오래된
  `screens-explore-v2` 사본으로 GBA 타운을 만들었다가, v19가 5-파빌리온+광장임을 확인하고 정정.)
- **지면**: `Interior.groundMap?: string[]`(타일 레전드 g/G·p/P·z·r/l·c) 신설 + `CampusGround`
  렌더러(잔디 base + 비잔디 타일만 오버레이 → 26×60도 수백 View). `groundMap` 있으면 `TileFloor` 대체.
- **프롭**: `campusEquipment.tsx`(`CampusObjectView`) — 동상·헬기장·앰뷸런스·주차차량·가로등·
  쓰레기통·우편함·소화전·버스정류장·피크닉테이블·생울타리·자전거거치대·벤치·덤불·꽃(+미사용 cbuilding).
  전부 캠퍼스 전용 타입 → 인테리어 디스패치와 무간섭(무회귀). 나무는 엔진 `tree` 재사용.
- **충돌**: 파빌리온+시계탑은 `props.w/h`로 자동 블록, 장식은 w/h 생략 → 보행 가능. 광장/길/도로 보행 가능.
- **핫스팟**: 5개 파빌리온 정문 스포크를 각 건물 엘리베이터로 연결(tower/onco/admin/dx/women).
- 검증: tsc·jest(campus.test 4케이스: 지면 치수·파빌리온 무겹침·스폰 개방·모든 핫스팟 도달성) +
  시뮬(시계탑 광장·5 파빌리온·정원 프롭·NPC 마커 렌더 확인).
