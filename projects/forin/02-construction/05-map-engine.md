...
## 캠퍼스 v20 — 핸드오프 GBA 캠퍼스 1:1 (2026-08)

기존 v8 캠퍼스(landmark 스카이스크레이퍼 5동)가 핸드오프와 크게 달라 `screens-explore-v2`를
**탑다운 GBA 캠퍼스로 1:1 재구현**. 엔진 이동/카메라/충돌/핫스팟/엘리베이터는 그대로 재사용하고
**비주얼 레이어만 교체**:

- **지면**: `Interior.groundMap?: string[]`(타일 레전드 g/G·p/P·z·r/l·c·w) 신설 + `CampusGround`
  렌더러(잔디 base + 비잔디 타일만 오버레이 → 26×60도 수백 View). `groundMap` 있으면 `TileFloor` 대체.
- **오브젝트**: `campusEquipment.tsx`(`CampusObjectView`) — 탑다운 건물(지붕+적십자/엠블럼+간판+
  떠있는 라벨+전면벽 창/문/차양) + 분수·헬기장·앰뷸런스·주차차량·가로등·쓰레기통·우편함·소화전·
  버스정류장·자판기·피크닉테이블·생울타리·자전거거치대·수련잎·농구장·동상·벤치·덤불·꽃. 전부
  `c`-네임스페이스/캠퍼스 전용 타입 → 인테리어 디스패치와 무간섭(무회귀).
- **충돌**: 건물은 `props.w/h`로 자동 블록, 장식은 w/h 생략 → 보행 가능(핸드오프 자유이동감). 연못만 명시 블록.
- **핫스팟**: 16개 건물 중 임상 건물 정문을 elevator(본관/소아과/산부인과→women, 심장/외래→dx) 또는
  portal(ER·OR·약국·ICU·재활·영상 → 각 인테리어)로 연결. 나머지(연구/교육/카페/기숙사/채플)는 장식.
- 검증: tsc·jest(신규 campus.test 4케이스: 지면 치수·건물 무겹침·스폰 개방·모든 핫스팟 도달성) + 시뮬 확인.
