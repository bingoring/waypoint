...
> 이 폴더는 waypoint **구현 스펙(Build Spec)** 메커니즘(`FRAMEWORK.md` "구현 스펙" · `_templates/build-spec-template.md`)의
> 콘텐츠-시리즈 실사례다. 아래 "부서 문서 템플릿"은 Build Spec 정규 섹션을 맵 도메인으로 특화한 인스턴스.
...
> Build Spec 정규 섹션의 맵-도메인 특화: §개요→1 · §분해/도메인→2·5 · §동작규칙→3 · §명세표→4·5 ·
> §UI상호작용→5·6 · §엣지케이스+미해결질문→(핸드오프 대조 시) · §검증+편차→9.
```
