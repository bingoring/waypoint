...
- **Phase 2 · WOMEN 완결 — ✅ 완료**: **ld ✅(3F 통합 INT-LD)** → **nicu ✅(4F)** → **picu ✅(딥링크·sub선택 대기)**. (postpartum·nursery는 ld.jsx 통합에 subsumed; standalone은 FIXTURES 잔존.) **미해결: 층당 복수부서(4F NICU+PICU, DX 3F infusion+dialysis, ONCO 4F hospice+geri) sub-선택 UI** — 도입 시 두 번째 부서들 정식 배선.
