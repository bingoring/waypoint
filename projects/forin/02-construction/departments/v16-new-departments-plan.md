...
- **Phase 5 · ADMIN 완결 — ✅ 완료**: **sim ✅(3F)** → **lounge ✅(2F)** → **spd ✅(1F)** → **morgue ✅(B1)**. 행정·백스테이지 4개 층 전부 정식 배선.

> **🎉 v16 20개 신규 부서 전체 완결 (Phase 1–5).** WOMEN·DX·ONCO·ADMIN 4개 건물 모든 층 정식 배선 완료. 각 부서 핸드오프 1:1 구현 + 방·화면단위 시뮬레이터 검증 + Build Spec 5문서 + jest 통행 가드. 다음 단계: 시나리오(`scenarioId`) 연결·콘텐츠 채우기.
