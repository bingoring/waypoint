...
| SoT(핸드오프) | `interior-icu.jsx` (레이아웃) + `interior-objects-icu2.jsx`·`interior-shared.jsx` (오브젝트) |
| 그리드 | 34 cols × 44 rows · floorTheme `ICU` · scale **0.85** |
| playerStart | `{16,26}` (허브 open floor; 16,23은 데스크라 회피) |

> 구조·공통 규약은 [er.md](er.md)(기준선) + [README](README.md). 여기선 ICU 고유(**유리벽 1인실 ×4** + 중앙 텔레메트리 허브)만 상세.
> 아티팩트 매핑: **A** domain-entities · **B** business-rules · **C** business-logic-model · **D** frontend-components + 인덱스.

## 개요 & 범위 (Index) — 유리벽 1인실 + 중앙 허브
```
...
└───────┴────────┴───────────────────┘
```
- **규약**: 4개 1인실은 **투명 유리벽**(`glass`) — 허브에서 환자 전원 감시(ICU 원칙). 벽은 정적 collision이 아니라 **glass 오브젝트의 objectCollision으로 차단**. 각 방 y17에 **폭1 auto door**. 방 위 어두운 `tint`(#26354D .16)=ICU 저조도.
...
### A.2 오브젝트 배치 (SoT → `type` → 좌표; 마커=엔티티 속성)
...
- **collision**: 외벽 상단 `{0,0,34,1}` · 좌우 `{0,1,1,42}{33,1,1,42}` · 하단 `{0,43,6,1}{9,43,24,1}`(캠퍼스 문 x6-8 gap). divider y30(통로 x5-7/x13-15/x22-24) `{1,30,4,1}{8,30,5,1}{16,30,6,1}{25,30,8,1}`. 지원 세로 x13·x23(gap y35-37). **1인실 유리벽은 collision에 없음** — glass 오브젝트가 objectCollision으로 차단(세로 x8/x16/x24 + y17 경계).
...
- **footprint**(차단): crrt/ttmunit{2,2} · ivpumptower/evdstand/icpmonitor{1,1} · glass는 props.w/h로 차단 · bankofmonitors CEILING(배경).

---

## C. Business Logic Model (흐름)
- **진입**: 타워 4F `interior:'INT-ICU-00001'`, `entry:{x:7,y:42}`(캠퍼스 문 안쪽).
- **이동·카메라**: 공통([README](README.md) §렌더) — scale 0.85·컬링 off·RoomMask 옅게.
- **시나리오 배선** (마커=엔티티 속성, `hotspots: []`):
...
- **z-order**: bankofmonitors CEILING(배경) · 마커 소스별 dy.

---

## 검증 & 편차 (Index)
- **검증**: `tsc` 0 · `jest`(icu-fixture: 유리벽 차단 x8,8 · 방 auto door 통행 x4,17 · ibed footprint x2,4 차단 · playerStart hub open · 8 room 도달·threshold 통행) · `expo export` OK · 시뮬 확인.
