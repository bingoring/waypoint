...
## 진입 (Q2)
- **엘리베이터 타워 8F** — `ELEVATOR_BUILDINGS.tower`의 기존 8F("일반 내과 병동 · 외과 병동 · 정형외과 병동")에 `interior: 'INT-WARD-00001'`, `entry: {12,50}` 추가. (외과·정형은 후속 부서 — 8F는 당분간 내과 병동으로 진입.)
- 하단 캠퍼스 문(x12-14 y51)이 1인실\|격리 분리벽 x13을 걸쳐 → **entry는 1인실 쪽 `{12,50}`**(x13은 벽, x14는 격리실). 도착 후 위로 올라가며 병실→스테이션(핸드오프 세로 흐름).
