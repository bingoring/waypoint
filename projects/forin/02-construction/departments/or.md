...
| SoT(핸드오프) | `interior-or.jsx` (레이아웃) + `interior-objects-or2.jsx`·`interior-shared.jsx` (오브젝트) |
| 그리드 | 40 cols × 52 rows · floorTheme `sterile` · scale **0.85** |
| playerStart | `{7,40}` (OR1 open floor) |

> 구조·공통 규약은 [er.md](er.md)(기준선) + [README](README.md). 여기선 OR 고유(3단계 청정 존·양압·sterile threshold)만 상세.
> 아티팩트 매핑: **A** domain-entities · **B** business-rules · **C** business-logic-model · **D** frontend-components + 인덱스.

## 개요 & 범위 (Index) — 3단계 청정 존
```
┌──────── 비제한 (y0-14) ────────┐
...
└──────────┴───────┴──────────────┘
```
- **규약**: 청정도 상승(y14→y31)일수록 제한. 제한(OR) 진입 = **`tone:'sterile'`(파란) threshold**(가운/스크럽 시각 신호). OR1/OR2 바닥에 초록 `tint`(#CDE3D6 .28)=양압·수술 조도.
...
## B. Business Rules (통행/차단)
- **collision**: 외벽 상단 `{0,0,17,1}{21,0,19,1}`(캠퍼스 문 x17-20 gap) · 좌우 `{0,1,1,50}{39,1,1,50}` · 하단 `{0,51,40,1}`. divider y14/y31(통로 x5-7/x17-19/x29-31) 각 `{1,_,4,1}{8,_,9,1}{20,_,9,1}{32,_,7,1}`. 세로: preop|util x13(gap y18-20) · util|pacu x20(gap y19-21) · clean|dirty y22(gap x16-17) · family|locker x19(gap y6-8) · or1|scrub x15·scrub|or2 x23(gap y36-38).
...
- **footprint**(차단): anesthesia/roboticconsole/carm/sinkor{2,2} · bairhugger/bovie/laptower/co2insufflator/soiledcart{1,2} · kickbucket{1,1} · ibed 등. skip: door·threshold·tint·icurtain·nursestation.

---

## C. Business Logic Model (흐름)
- **진입**: 타워 3F `interior:'INT-OR-00001'`, `entry:{x:18,y:1}`(캠퍼스 문 안쪽).
- **이동·카메라**: 공통([README](README.md) §렌더) — scale 0.85·컬링 off·RoomMask 옅게.
- **시나리오 배선** — ⚠️ OR은 아직 **`hotspots[]` 배열**(엔티티 마커 미이관, ER/ICU와 다름):
...
- **공용**: ibed(or)/imonitor/iiv/icurtain/icabinet(sterile·linen·drug·equipment·supply)/sinkor/surgicallight/instrumenttray/bankofmonitors/nursedeski/crashcart + sofa/coffeetable/walltv/watercooler/iplant/ichair/sanitizer/compcart/suction.
- **z-order**: surgicallight OVERHEAD(8000) · orboommonitor CEILING(배경).

---

## 검증 & 편차 (Index)
- **검증**: `tsc` 0 · `jest`(or-fixture: room 도달·sterile threshold 통행·양압존 footprint) · `expo export` OK · 시뮬(3단계 존·sterile 파란 통로·수술등 overhead).
- **편차**: (a) scale 0.85 · (b) 상호작용이 아직 `hotspots[]`(14개) — ER/ICU 엔티티-마커 모델로 후속 이관 후보([README](README.md) §마커). 좌표·오브젝트·NPC는 `interior-or.jsx`와 1:1.
