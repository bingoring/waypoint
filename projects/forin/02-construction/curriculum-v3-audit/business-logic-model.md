...
정책(R-P2-8):
- **정리·보강은 append-우선.** 제거할 Topic은 목록에서 빼기보다 **비활성 표시**(생성 0)로 두어 뒤 id를
  보존하는 방식을 우선 검토한다. 신규 Topic은 부서 목록 **끝에 추가**한다.
- 다만 현재 사용자 규모(스토어 미출시·TestFlight 내부, `project_ios_testflight_pending`)를 감안하면
  **한 번의 정본 재배치(id 재발급)를 감수하는 선택**도 가능하다 — 이 경우 "커리큘럼 v3 정본 재생성"
  시점에 진도 리셋을 공지/수용한다. **둘 중 무엇을 택할지는 실행 착수 시 사용자 확인**(index §0.5).
- 어느 쪽이든 **재생성 후 id 매핑 diff를 audit로 출력**해 영향 범위를 눈으로 확인한다.
