...
## §0.7 콘텐츠 모델 재정의 (사용자 결정 2026-09-09) — P2의 무게중심 이동

초기 P2는 "기존 3,044개 태깅·정리"였다. 사용자 검수에서 두 가지가 드러나 **P2의 무게중심이 "대규모
구별-상황 생성"으로 이동**한다. 이 절이 이후 스펙·계획의 상위 전제다.

- **D-P2-A · 주제는 부서당 수십 개다.** "ER depth 2개"는 현 콘텐츠가 얕아서(부서당 구별 상황 ~10개 +
  손저작 ~14개, 나머지는 페르소나 변주) 나온 결과일 뿐이다. 실제 응급실은 ~35개 임상 주제를 갖는다
  (트리아지·흉통·뇌졸중·외상·소생·중독·감염·정신응급·소아·처치·… §ER 초안). 부서당 ~30 주제 × 30 부서
  ≈ **~900 주제**가 정상 규모다.
- **D-P2-B · 주제의 진도 경로 = 구별되는 상황들.** 페르소나×난이도로 같은 상황을 뻥튀기하지 않는다
  ("아까 봤던 트리아지를 왜 또?"). 주제당 20~30개는 **서로 다른 임상 표현형·합병증·소통 난점**을 가진
  구별 상황이어야 한다(예: 흉통 주제 = 전형 STEMI·비전형 당뇨 여성·불안성 감별·대동맥 박리·PE·… 각기 다름).
- **D-P2-C · 페르소나 변주 = 옵션 재도전.** 페르소나 다양성(같은 상황, 다른 환자에게 말하기)은 언어
  학습에 가치 있으므로 버리지 않되, **주 경로의 별도 진도 스텝이 아니라 한 상황의 선택적 "다시·다른
  환자로" 재도전**으로 둔다(진도 무관). P1 조립(스텝=시나리오)과 호환 — 스텝이 "구별 상황"이 될 뿐이다.

**규모·생성 함의:**
- 구별 상황 총량 ≈ 수십 주제 × 20~30 × 30 부서 ≈ **2만+ 구별 상황**(현재 구별 ~590). 페르소나 곱셈이
  경로에서 빠지므로 "많아 보이던 3,044"는 사실 얕았다.
- 이 규모는 수작업 불가 → **LLM-보조 상황 생성 파이프라인**이 P2의 핵심 산출물이 된다(Topic=구별 상황
  씨앗을 LLM으로 생성 → 임상 검수 → 템플릿 엔진). 기존 페르소나 변주분은 대표 1개만 경로에 남기고
  나머지는 재도전 변주로 강등(또는 정리). **정본 재발급(R-P2-8)**이 이 재구성을 가능케 한다.
- **진행 방식(사용자 결정): ER을 기함으로 완전히 깊게 먼저.** ER ~35주제를 각 20~30 구별 상황으로 채워
  파이프라인·품질을 증명한 뒤 29개 부서로 팬아웃.

> **이 절은 기존 §0.5·§logic §3(페르소나 자동 태깅으로 대량 해결)의 전제를 상위에서 수정한다.** 자동
> 태깅은 여전히 유효하지만(생성분에 theme emit), "대량은 페르소나 변주라 태깅만 하면 끝"이라는 그림은
> 폐기된다 — 대량 **생성**이 본체다. 계획(implementation-plan.md)은 이 재정의에 맞춰 개정이 필요하다.
