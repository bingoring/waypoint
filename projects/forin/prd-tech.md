...
- 역할: 소셜 원탭 인증, 사용자/진행도 영속화(XP·평판·자격·스티커·리뷰랩 저장),
  시나리오/콘텐츠·이벤트 전달(상황판 일일 갱신), **LLM 대화·교정 오케스트레이션**
  (키 보관·모델 티어링·스트리밍·비용/캐싱·가드레일).
...
## AI 레이어 (MVP 핵심) — 방향 확정 · 세부 게이트

> **AI 대화 + 문맥 교정**이 forin의 핵심(`prd.md` → AI/LLM 레이어). **MVP 포함**이며,
> **확장성·디자인 패턴에 리소스를 집중**한다 — 제공자/모델을 교체 가능하게 격리하고 비용·지연을 통제.

- **LLM 활용**: 대화 = 고급 모델(자연스러움·스트리밍), 교정 = 저가 모델(비용). **모델 티어링 라우팅**.
  제공자·모델·비용·지연·프롬프트/캐싱·가드레일은 게이트에서 확정. 키·호출은 **서버(Go) 오케스트레이션**(모바일에 키 노출 금지).
- **추상화(핵심 설계)**: LLM·STT·TTS를 **포트(인터페이스) + 어댑터**로 격리해 제공자 교체 가능
  (axios 래퍼 클라이언트와 동일 철학). 대화·교정·음성은 각각 독립 모듈로 경계.
- **대화 엔진**: 시나리오가 **목표·가드레일·핵심표현**을 제공하고, LLM은 그 제약 안에서 자유 대화.
  **교정 파이프라인**은 별도 경로로 사용자 발화를 교정 → 리뷰랩 카드 생성.
