...
- **LLM 활용**(서비스 전반): **자유 발화 대화 = 고급 모델**(자연스러움 우선),
  **답안 교정·채점 = 저가 모델**(비용 우선). 구체 모델·제공자·비용·지연·프롬프트/캐싱 전략은 게이트 확정.
- **TTS**(🔊 리뷰랩·대화): 최신·고사용성 우선. `expo-speech`로 품질·UX 충분하면 사용, 부족 시 최신 클라우드 TTS. 게이트 확정.
- **STT / 발음 비교**(🎤 따라 말하기 + free-speak): **MVP 포함**. `expo-av`(+발음 비교)로 충분하면 사용,
  부족 시 최신 STT. 게이트 확정.
