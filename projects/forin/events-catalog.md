...
- **목표:** MVP까지 **최소 300개** 이벤트 작성. 전수 작성은 **콘텐츠 워크스트림**으로 진행
  (임상 레퍼런스 조사 기반, 검수 포함). 현재 시드는 **파이프라인 검증용 소량**일 뿐 — 분량은 워크스트림이 채운다.

## 진료과(specialty) 범위 · 규모 분포

**진료과는 데이터다(코드 하드코딩 아님).** `department`/`ward`/`specialty`는 콘텐츠 행이라,
진료과 추가 = 행 추가(스키마 변경 없음). 내과·외과·정형외과·피부과 … + **세분화**(내과 → 심장/호흡기/소화기내과 …)는
department에 `specialty`/`parentSpecialty`(계층) 필드로 표현(확장 시 추가).

**⚠️ 비주얼 제약:** 핸드오프에는 **탐험 가능한 인테리어가 5개**(ER/OR/ICU/Peds/Pharmacy)뿐.
신규 진료과는 *콘텐츠*로는 무제한이나 *걸어다니는 인테리어*는 art가 필요. 처리 3안:
- **(A) MVP는 디자인된 5개 워드 내 300+ 집중** — 신규 진료과는 로드맵(+신규 인테리어 art). *(권고)*
- **(B) 신규 진료과는 상황판(이벤트 보드) 전용** — 전용 인테리어 없이 시나리오만 제공.
- **(C) 진료과별 신규 인테리어 art 제작** — 핸드오프 범위 초과, art 리소스 큼.

**MVP 300+ 잠정 분포(5개 워드 기준):** ER ~80 · ICU ~60 · Peds ~55 · OR ~50 · Pharmacy ~40 · 공통/시설 ~30.
카테고리(비상코드/임상/대인/시설/절차) × 티어(1–4)를 고르게 분포. (확정 분포는 워크스트림 착수 시.)
