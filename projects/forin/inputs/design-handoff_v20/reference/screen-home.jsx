  ...
  function PeerTicker() {
    ...
    return (
      <div style={{ margin: '13px 16px 0', background: t.cream, border: `3px solid ${t.ink}`, boxShadow: `3px 3px 0 0 ${t.ink}` }}>
        <div style={{ padding: '8px 12px 6px', fontFamily: '"DungGeunMo",monospace', fontSize: 10.5, color: t.ink,
          borderBottom: `2px dotted ${t.ink}33` }}>👥 같은 목표를 준비하는 사람들</div>
        {rows.map((r, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px',
            ...
            <span style={{ fontFamily: '"Galmuri11",monospace', fontSize: 10, color: t.textSoft, minWidth: 0, lineHeight: 1.25 }}>{r[2]}</span>
          </div>
        ))}
      </div>
    );
