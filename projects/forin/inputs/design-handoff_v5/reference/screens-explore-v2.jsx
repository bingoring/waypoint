...
(function () {
  ...
  function NPC({ x, y, shirt, hair, exclaim, quest, kind, dir, walking, seed, emote }) {
    const roleMap = {
    ...
    return (
      <div style={{ position: 'absolute', left: x * TILE - 4, top: y * TILE - 16, zIndex: 4, transition: 'left .55s linear, top .55s linear' }}>
        {Role ? <Role seed={idSeed} hair={hair} shirt={shirt} size={kind === 'child' ? 20 : 24} dir={dir} walking={walking}/> : null}
        {exclaim && (
  ...
  function PatrolNPCs() {
    ...
    return st.map((s, k) => {
      const a = agents[k];
      return <NPC key={'ag' + k} x={s.x} y={s.y} seed={a.seed} kind={a.kind}
