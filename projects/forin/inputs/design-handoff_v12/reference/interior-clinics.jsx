...
(function () {
  ...
  function UltrasoundCart({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 6, width: T * 1.4, height: T * 2.4, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.28))' }}>
        <svg viewBox="0 0 22 38" width={T * 1.4} height={T * 2.4} shapeRendering="geometricPrecision">
          {/* viewer-facing monitor with a top bezel cap */}
          <path d="M4 0.6 L18 0.6 L19 2 L3 2 Z" fill="#2C333B"/>
          <rect x="3" y="2" width="16" height="10.5" fill="#1F2937" stroke={C} strokeWidth=".5"/>
          <rect x="4" y="3" width="14" height="8.5" rx=".4" fill="#0F1A24"/>
          <path d="M11 4 L6 11 L16 11 Z" fill="#15314A"/>
          <path d="M11 4 L8 11 L14 11 Z" fill="#22506E"/>
          <rect x="10" y="6" width="2" height="2" fill="#A7F3D0"/>
          {/* full cart silhouette (top keyboard tray + front body) */}
          <path d="M2 14 Q1.4 14 1.4 14.6 L1.4 31 Q1.4 31.6 2 31.6 L20 31.6 Q20.6 31.6 20.6 31 L20.6 14.6 Q20.6 14 20 14 Z" fill="#AEB4BC"/>
          {/* TOP keyboard/console face */}
          <path d="M2 14 Q1.4 14 1.4 14.6 L1.4 21 L20.6 21 L20.6 14.6 Q20.6 14 20 14 Z" fill="#C7CDD4"/>
          <rect x="3" y="15" width="12" height="3.4" rx=".4" fill="#1F2937"/>
          {[0,1,2,3,4].map(i=><rect key={i} x={3.6+i*2.3} y="15.6" width="1.6" height="2.2" fill="#3A424B"/>)}
          <circle cx="17.6" cy="16.6" r="1.6" fill="#5B6672" stroke={C} strokeWidth=".3"/>{/* trackball */}
          {/* probe holder + probe on the top edge */}
          <rect x="0.2" y="15.5" width="1.6" height="2" fill="#6B7280"/>
          <path d="M1 17.5 Q-1 20 1 23" fill="none" stroke={C} strokeWidth=".5"/>
          {/* seam → front body */}
          <line x1="1.4" y1="21" x2="20.6" y2="21" stroke={C} strokeWidth=".55"/>
          <rect x="2.6" y="22" width="16.8" height="3.4" rx=".3" fill="#EDEFF2" stroke={C} strokeWidth=".35"/>
          <rect x="9" y="23.2" width="4" height="1" fill="#9CA3AF"/>
          <rect x="2.6" y="26" width="16.8" height="3.4" rx=".3" fill="#EDEFF2" stroke={C} strokeWidth=".35"/>
          <rect x="9" y="27.2" width="4" height="1" fill="#9CA3AF"/>
          {/* outer outline */}
          <path d="M2 14 Q1.4 14 1.4 14.6 L1.4 31 Q1.4 31.6 2 31.6 L20 31.6 Q20.6 31.6 20.6 31 L20.6 14.6 Q20.6 14 20 14 Z" fill="none" stroke={C} strokeWidth=".6"/>
          <ellipse cx="4" cy="33" rx="2" ry="1.4" fill="#2C3239"/>
          <ellipse cx="18" cy="33" rx="2" ry="1.4" fill="#2C3239"/>
        </svg>
      </div>
    );
  }
  ...
  function XrayViewbox({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T, width: T * 2, height: T * 1.4, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.22))' }}>
  ...
  function CastCart({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 4, width: T * 1.4, height: T * 1.8, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.24))' }}>
        <svg viewBox="0 0 22 30" width={T * 1.4} height={T * 1.8} shapeRendering="geometricPrecision">
          ...
          {/* TOP tray — plaster/fiberglass rolls seen from above (round ends) */}
          <path d="M2 2 Q1 2 1 3 L1 13 L21 13 L21 3 Q21 2 20 2 Z" fill="#E1E5EA"/>
          {[[5,'#fff'],[10,'#E8F0D8'],[15,'#CFE3EC']].map(([rx,col],i)=>(
            <g key={i}>
              <ellipse cx={rx} cy="5" rx="2.6" ry="1.5" fill={col} stroke={C} strokeWidth=".35"/>
              <path d={`M${rx-2.6} 5 L${rx-2.6} 9 Q${rx-2.6} 10.4 ${rx} 10.4 Q${rx+2.6} 10.4 ${rx+2.6} 9 L${rx+2.6} 5`} fill={col} stroke={C} strokeWidth=".35"/>
              <circle cx={rx} cy="5" r=".8" fill={C} opacity=".25"/>
            </g>
          ))}
          {/* seam → front drawer */}
  ...
  function Crutches({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 2, top: y * T - 6, width: T, height: T * 2, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.2))' }}>
  ...
  function DermLamp({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 8, width: T * 1.3, height: T * 2.2, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.24))' }}>
  ...
  function LaserUnit({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 4, width: T * 1.3, height: T * 2.2, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.26))' }}>
        <svg viewBox="0 0 20 36" width={T * 1.3} height={T * 2.2} shapeRendering="geometricPrecision">
          {/* full silhouette: TOP console face folds straight into the front body */}
          <path d="M2 3 Q1 3 1 4 L1 31 Q1 32 2 32 L18 32 Q19 32 19 31 L19 4 Q19 3 18 3 Z" fill="#CBD5E1"/>
          {/* TOP face — console lid with recessed control panel */}
          <path d="M2 3 Q1 3 1 4 L1 15 L19 15 L19 4 Q19 3 18 3 Z" fill="#E2E8F0"/>
          <rect x="3" y="5" width="10" height="6" rx=".5" fill="#0F1A24" stroke={C} strokeWidth=".3"/>
          <rect x="3.8" y="5.8" width="8" height="1.8" fill="#A78BFA"/>
          <rect x="3.8" y="8.2" width="5.5" height="1.3" fill="#22D3EE"/>
          <circle cx="16" cy="8" r="1.6" fill="#5B6672" stroke={C} strokeWidth=".3"/>{/* dial */}
          {/* seam top↔front */}
          <line x1="1" y1="15" x2="19" y2="15" stroke={C} strokeWidth=".55"/>
          {/* front body: handpiece in holster + drawers */}
          <rect x="13.5" y="16" width="4" height="2" fill="#1F2937"/>
          <rect x="14.6" y="18" width="1.8" height="7" fill="#374151" stroke={C} strokeWidth=".3"/>
          <rect x="2.5" y="17" width="9" height="3" rx=".3" fill="#EDEFF2" stroke={C} strokeWidth=".35"/>
          <rect x="2.5" y="21" width="9" height="3" rx=".3" fill="#EDEFF2" stroke={C} strokeWidth=".35"/>
          {/* outer outline + casters */}
          <path d="M2 3 Q1 3 1 4 L1 31 Q1 32 2 32 L18 32 Q19 32 19 31 L19 4 Q19 3 18 3 Z" fill="none" stroke={C} strokeWidth=".65"/>
          <ellipse cx="4" cy="33.5" rx="2" ry="1.4" fill="#2C3239"/>
          <ellipse cx="16" cy="33.5" rx="2" ry="1.4" fill="#2C3239"/>
        </svg>
      </div>
    );
  }
  ...
  function ExamStool({ x, y, color = '#4B5563' }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 2, top: y * T, width: T - 4, height: T * 1.3, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.22))' }}>
        <svg viewBox="0 0 12 16" width={T - 4} height={T * 1.3} shapeRendering="geometricPrecision">
          {/* five-star caster base seen from above (behind/under the seat) */}
          {[0,1,2,3,4].map(i=>{const a=i*1.2566-1.57;return <line key={i} x1="6" y1="11" x2={6+4.5*Math.cos(a)} y2={11+3*Math.sin(a)} stroke="#6B7280" strokeWidth="1.4" strokeLinecap="round"/>;})}
          {[0,1,2,3,4].map(i=>{const a=i*1.2566-1.57;return <circle key={i} cx={6+4.5*Math.cos(a)} cy={11+3*Math.sin(a)} r=".9" fill="#2C3239"/>;})}
          {/* gas cylinder */}
          <rect x="5.2" y="7" width="1.6" height="4" fill="#9CA3AF" stroke={C} strokeWidth=".3"/>
          {/* round seat cushion — dominant top face */}
          <ellipse cx="6" cy="5" rx="5" ry="3.6" fill={color} stroke={C} strokeWidth=".5"/>
          <ellipse cx="6" cy="4.3" rx="3.6" ry="2.4" fill="#5E6A78"/>
          <ellipse cx="5.4" cy="3.6" rx="1.6" ry=".9" fill="#6E7B8A" opacity=".7"/>{/* sheen */}
          {/* seat side thickness */}
          <path d="M1 5 Q1 7 6 7 Q11 7 11 5 L11 6.2 Q11 8.2 6 8.2 Q1 8.2 1 6.2 Z" fill="#3A424B"/>
        </svg>
      </div>
    );
  }
  ...
  function SkincareShelf({ x, y, w = 3 }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T, width: T * w, height: T - 4, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.2))' }}>
  ...
  function BoneModel({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 2, top: y * T - 8, width: T - 4, height: T * 2, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.2))' }}>
  ...
  function ClinicReception({ x, y, w = 6, tone = '#0E7490', label = '접수' }) {
    const px = w * T;
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 6, width: px, height: T * 2.4, filter: 'drop-shadow(3px 0px 0 rgba(0,0,0,.28))' }}>
