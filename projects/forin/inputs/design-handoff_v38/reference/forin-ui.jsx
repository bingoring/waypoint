...
function StatBar({ label, value, max = 100, color = ForinTokens.mint, w = 120,
                  labelW, showPct = false, fill }) {
  const pct = Math.min(100, (value / max) * 100);
  // Two readouts: ratio "value/max" (default) or "NN%" (showPct) — the latter
  // is the old RepRow style (fixed label column + right-aligned percent).
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: labelW ? 10 : 6, fontFamily: '"Galmuri11","DungGeunMo",monospace', fontSize: showPct ? 11 : 10, color: ForinTokens.ink }}>
      {label && <span style={{ minWidth: labelW || 28 }}>{label}</span>}
      <div style={{ width: w, flex: w ? undefined : 1, height: 10, background: ForinTokens.cream, border: `2px solid ${ForinTokens.ink}`, position: 'relative' }}>
...
// ─── StatTile — large metric block: big value + label + sub + corner accent.
//   (Used in Daily Growth Report.)  props: label, value, sub, color
function StatTile({ label, value, sub, color = ForinTokens.mint, style = {} }) {
  const T = ForinTokens;
  return (
    <div style={{ background: '#fff', border: `3px solid ${T.ink}`, padding: 12, boxShadow: `3px 3px 0 0 ${T.ink}66`, position: 'relative', ...style }}>
      ...
      {sub && <div style={{ fontFamily: '"Galmuri11",monospace', fontSize: 10, color: T.textSoft, marginTop: 2 }}>{sub}</div>}
      <div style={{ position: 'absolute', right: 8, top: 8, width: 12, height: 12, background: color, border: `2px solid ${T.ink}` }}/>
    </div>
  );
}

// ─── MiniStat — compact metric: big value + label + left accent bar.
//   (Used in Review Lab.)  props: label, value, color
function MiniStat({ label, value, color = ForinTokens.mint, style = {} }) {
  const T = ForinTokens;
  return (
    <div style={{ background: '#fff', border: `2.5px solid ${T.ink}`, boxShadow: `2px 2px 0 0 ${T.ink}66`, padding: '8px 6px', textAlign: 'center', position: 'relative', ...style }}>
      <div style={{ fontFamily: '"DungGeunMo","Galmuri11",monospace', fontSize: 22, color: T.ink, lineHeight: 1 }}>{value}</div>
      <div style={{ fontFamily: '"Galmuri11",monospace', fontSize: 9, color: T.textSoft, marginTop: 3 }}>{label}</div>
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 4, background: color }}/>
    </div>
  );
}

// ─── BadgeTile — collectible square (career badge / sticker slot).
//   props: emoji, label, got (earned?), special (highlight), tag (corner ribbon)
function BadgeTile({ emoji, label, got = true, special = false, tag, style = {} }) {
  const T = ForinTokens;
  return (
    <div style={{
      aspectRatio: '1', background: got ? (special ? T.yellow : '#fff') : T.cream,
      border: `3px solid ${T.ink}`,
      boxShadow: got ? `3px 3px 0 0 ${special ? T.yellowShadow : T.ink}` : 'none',
      opacity: got ? 1 : 0.5,
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      position: 'relative', ...style,
    }}>
      <div style={{ fontSize: 22, filter: got ? 'none' : 'grayscale(1)' }}>{emoji}</div>
      {label && <div style={{ fontFamily: '"DungGeunMo",monospace', fontSize: 8, color: T.ink, marginTop: 2, textAlign: 'center', padding: '0 2px' }}>{label}</div>}
      {tag && <div style={{ position: 'absolute', top: -6, right: -6, background: '#EF4444', color: '#fff', border: `2px solid ${T.ink}`, padding: '0 4px', fontFamily: '"DungGeunMo",monospace', fontSize: 8 }}>{tag}</div>}
    </div>
  );
}

// ─── Highlight — marker-pen text emphasis. props: color (yellow/mint/…), raised
function Highlight({ children, color = ForinTokens.yellow, raised = false }) {
  return (
    <span style={{
      background: color, padding: '0 4px',
      boxShadow: raised ? `2px 2px 0 0 ${ForinTokens.ink}` : 'none',
    }}>{children}</span>
  );
}

// ─── Badge — small square status pill. props: bg, color, border
function Badge({ children, bg = ForinTokens.yellow, color = ForinTokens.ink, border = ForinTokens.ink, style = {} }) {
  return (
    <span style={{
      background: bg, color, border: `1.5px solid ${border}`,
      padding: '0 4px', fontFamily: '"DungGeunMo",monospace', fontSize: 8,
      lineHeight: '13px', display: 'inline-block', whiteSpace: 'nowrap', ...style,
    }}>{children}</span>
  );
}

// ─── Pips — n-of-m filled squares (difficulty / mastery meter).
//   props: filled, total (default 3), color, label
function Pips({ filled = 0, total = 3, color = ForinTokens.mint, label, size = 9 }) {
  const T = ForinTokens;
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
      {label && <span style={{ fontFamily: '"DungGeunMo",monospace', fontSize: 9, color: T.textSoft, marginRight: 2 }}>{label}</span>}
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{ width: size, height: size, background: i < filled ? color : '#fff', border: `1.5px solid ${T.ink}` }}/>
      ))}
    </div>
  );
}

// ─── FilterTab — pill tab with optional count badge. props: label, count,
//   active, color, onClick
function FilterTab({ label, count, active = false, color = ForinTokens.mint, onClick, style = {} }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      boxShadow: active ? `2px 2px 0 0 ${C}` : 'none',
      padding: '5px 10px', display: 'inline-flex', alignItems: 'center', gap: 6,
      fontFamily: '"DungGeunMo","Galmuri11",monospace', fontSize: 11, color: C, ...style,
    }}>
      {label}
      {count != null && (
        <span style={{ background: active ? '#fff' : color, border: `1.5px solid ${C}`, padding: '0 4px', fontFamily: '"DungGeunMo",monospace', fontSize: 9 }}>{count}</span>
      )}
    </div>
  );
}

// ─── PathStepper — horizontal step tracker (career path, multi-stage flows).
//   props: steps = [{ label, done, here }]
function PathStepper({ steps = [], style = {} }) {
  const T = ForinTokens;
  return (
      ...
      {steps.map((r, i, a) => (
        <React.Fragment key={i}>
          <div style={{ flex: 1, textAlign: 'center' }}>
            ...
            <div style={{ marginTop: 4, fontFamily: '"DungGeunMo",monospace', fontSize: 9, color: r.here ? T.ink : T.textSoft }}>{r.label}</div>
            {r.here && <div style={{ marginTop: 2, fontFamily: '"DungGeunMo",monospace', fontSize: 8, color: T.yellowShadow }}>● HERE</div>}
          </div>
          {i < a.length - 1 && <div style={{ width: 16, height: 4, marginTop: 12, background: a[i + 1].done ? T.mint : T.ink + '22' }}/>}
        </React.Fragment>
      ))}
    </div>
  );
...
// ─── PixelDropdown — 명확한 드롭다운 (라벨 · 트리거 · 화살표 · 펼침 목록) ───
// state: closed(기본) | open. 트리거는 "선택된 값 + ▼ 뾰족한 화살표 버튼"으로
// 눌러서 목록이 펼쳐진다는 것을 확실히 드러낸다. open일 때는 트리거 아래로
// 픽셀 패널이 붙어 나오고 선택 항목에 ✓ 표시.
function PixelDropdown({
  label, value, items = [], open = false, width = 200, placeholder = '선택하세요',
}) {
  const t = ForinTokens;
  return (
    <div style={{ width, position: 'relative', display: 'inline-block', verticalAlign: 'top' }}>
      {label && <div style={{ fontFamily: '"DungGeunMo",monospace', fontSize: 10, color: t.textSoft, marginBottom: 4 }}>{label}</div>}
      {/* trigger */}
      <div style={{
        display: 'flex', alignItems: 'stretch', background: '#fff',
        border: `3px solid ${t.ink}`, boxShadow: open ? 'none' : `3px 3px 0 0 ${t.ink}`,
        transform: open ? 'translate(1px,1px)' : 'none',
      }}>
        <div style={{ flex: 1, minWidth: 0, padding: '9px 10px', fontFamily: '"Galmuri11",monospace', fontSize: 12,
          color: value ? t.ink : t.textFaint, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {value || placeholder}
        </div>
        {/* arrow button — 픽셀 삼각형으로 "펼쳐짐"을 명확히 */}
        <div style={{ width: 30, flexShrink: 0, borderLeft: `3px solid ${t.ink}`, background: open ? t.yellow : t.cream,
          display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="12" height="9" viewBox="0 0 12 9" shapeRendering="crispEdges"
            style={{ transform: open ? 'scaleY(-1)' : 'none' }}>
            <rect x="0" y="0" width="12" height="2" fill={t.ink}/>
            <rect x="2" y="2" width="8" height="2" fill={t.ink}/>
            <rect x="4" y="4" width="4" height="2" fill={t.ink}/>
            <rect x="5" y="6" width="2" height="2" fill={t.ink}/>
          </svg>
        </div>
      </div>
      {/* expanded panel */}
      {open && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: '100%', background: '#fff',
          border: `3px solid ${t.ink}`, borderTop: 'none', boxShadow: `3px 3px 0 0 ${t.ink}`, zIndex: 40 }}>
          {items.map((it, i) => {
            const sel = it === value;
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 10px',
                background: sel ? t.mint : '#fff',
                borderBottom: i < items.length - 1 ? `2px dotted ${t.ink}33` : 'none',
                fontFamily: '"Galmuri11",monospace', fontSize: 12, color: t.ink }}>
                <span style={{ width: 12, fontFamily: '"DungGeunMo",monospace', fontSize: 12, color: sel ? t.ink : 'transparent' }}>✓</span>
                <span style={{ minWidth: 0, flex: 1 }}>{it}</span>
              </div>
            );
          })}
        </div>
...
Object.assign(window, {
  ...
  StatTile, MiniStat, BadgeTile, Highlight, Badge, Pips, FilterTab, PathStepper, PixelDropdown,
