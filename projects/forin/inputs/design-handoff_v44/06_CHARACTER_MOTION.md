...
| `up` | **Back of head** — `backHead()`, **drawn per style** (short nape, bob/long sheets, pigtails puffs, bun knot, curly poof, mohawk strip, ponytail tail + red tie, balding side-patches + top strands); hats render their back; no face. Chest mark/cross is hidden. |
| `left` / `right` | **3/4 side view** — one visible eye + a small nose bump at the head's front edge; minimal mouth. **Hat** uses a side-profile variant (`hatSide()`): visor/brim points toward the front, and the nurse cross / police badge sits at the front of the cap (never center). **Hair** uses a side-profile variant (`hairSide()`) per style: the crown is **swept back so the forehead shows**, with a **visible ear** and a small forelock bang, draping diagonally behind the ear — pigtails show one tail at the back, long hair drapes down the back, bun sits at the back, ponytail kicks up at a high red-tied gather into an S-tail, balding is just a patch above the ear (designed right-facing, mirrored for left). **Body** is a narrower side torso with back-edge shading; the chest mark/cross is hidden, and **a single arm** is tucked along the torso (not two front-facing arms). **Two legs swing in opposite phase at body center, crossing front↔back** (far leg darker for depth) for a natural side stride. `left` is the `right` drawing mirrored (`scale(-1,1)`). |
...
- Side view draws **two legs at body center swinging in opposite phase** so
  they cross front↔back (natural profile stride; the far leg is drawn darker
  for depth). When standing they rest overlapped. A **single arm** is tucked
  along the narrow torso; front/back draw two arms. The walking swing pivots
  legs + arm around the body center for side view.
...
> Hairstyle override for ambient/showcase NPCs: role components accept a
> `hairStyle` prop that overrides the deterministic pick (ignored for
> hat-locked roles cap/peakedCap). The campus seeds one free-roaming NPC per
> hairstyle as a living showcase. The DS "Characters · Derp" catalog has a
> "4방향" section and a "머리스타일 × 측면 프로필" section showing each style
> **front + side + back** (all 10 styles, the back drawn per style).
