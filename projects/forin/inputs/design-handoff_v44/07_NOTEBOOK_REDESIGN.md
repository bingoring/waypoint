# 07 — forin Notebook 리디자인 (근무 수첩 그림체)

> 2026-09 확정: 사용자 만족도 비교용 **디자인 개편 라인**. 기존 픽셀 라인과 병행.
> 후보 탐색(카툰CN풍·클레이·에디토리얼·레트로OS) 끝에 **"간호사 근무 수첩(스크랩북)"** 방향 채택.
> 카툰 라인(forin Cartoon - Home.html / forin-cartoon.jsx)은 보조 후보로 보관.

## 그림체 규칙 (NB 토큰)
- 배경: 크림 종이 #F1EBDD + 줄노트(27px 반복 라인) / 카드 종이 #FFFdf4 + 1px #E0D6C0 테두리 + 부드러운 그림자
- 잉크 #3E362B · 보조 #9A8F7C · 빨강 #C75146 · 파랑 #4A6FA5 · 초록 #5F8D5A
- 폰트: 손글씨 Gaegu(제목·라벨) + Pretendard(본문) + IBM Plex Mono(코드·시각·IPA)
- 소품 문법: 마스킹테이프(하늘색 반투명) · 이중선 원형 도장(통과/근무중/연속출근) · 빨간펜 취소선 → 화살표 교정 · 형광펜 mark(투명 55%→#F9E37B) · 점선 메모 박스 · 카드 ±0.3~0.8° 기울임 · 비스듬한 입체 압정(라운지)
- 아이콘: **NbIcon** (forin-notebook.jsx, 24×24 펜 낙서 — 1.7px 스트로크 + 옅은 수채 필). UI 이모지 금지(사용자 콘텐츠 제외).
  보유: home hospital board lab me mic speaker siren scalpel baby monitor pill bandage bell star pencil magnify bulb trophy speech compass chartup shield handshake2 pushpin dice lock
- 폰 프레임: 402×874, boxSizing border-box, 하단 탭 5개(홈·일터·라운지·리뷰랩·나)

## 워딩 확정
- 캠퍼스/커리어 탭 → **일터** (직업군 확장 대응)
- 상황판 탭 → **라운지** (스태프 라운지 커뮤니티로 개편)

## 화면 인벤토리 (파일 → 아트보드)
| 페이지 | 컴포넌트 파일 | 아트보드 |
|---|---|---|
| forin Notebook - Home.html | forin-notebook.jsx + forin-notebook-career.jsx | A 홈(HomeScrapbook) · B 일터 목록(CareerScrapbook) · C 층 바텀시트(FloorSheetScrapbook) · D 상황 준비(BriefingScrapbook) |
| forin Notebook - Dialogue.html | forin-notebook-dialogue.jsx | A 직접 말하기(DialogueSpeak) · B 보기 중 선택(DialogueOptions) |
| forin Notebook - Review Lab.html | forin-notebook-lab.jsx | A 교정 노트(LabNotes) · B 말하기(LabSpeak) · C 모범답안(LabModel) |
| forin Notebook - Profile.html | forin-notebook-profile.jsx | A 프로필(ProfileScrapbook) · B 성장 리포트(GrowthScrapbook) · C 동료 추가(ColleagueAddScrapbook) |
| forin Notebook - Pronunciation.html | forin-notebook-pron.jsx | A 대기(PronReady) · B 녹음중(PronRecording) · C 채점(PronScore) |
| forin Notebook - Lounge.html | forin-notebook-lounge.jsx | A 피드(LoungeFeed) · B 대화 공유 작성(LoungeShare) · C 상대 프로필(LoungeProfile) |
| forin Notebook - Quiz Result.html | forin-notebook-session.jsx + forin-notebook-quiz2.jsx | A~F 기본 6유형(문장완성·잇기·객관식·받아쓰기·순서·안전체크) · G 바이탈 라벨링(QuizVitalsNb) · H ESI 판정 도장(QuizTriageNb) · I 용량 계산 연습장(QuizDosageNb) · J 신체부위 라벨링(QuizAnatomyNb) · K APGAR 채점표(QuizApgarNb) · L 라벨 오류찾기(QuizErrorNb) · M 결과 · N 복습 세션 |
| forin Notebook - Colleagues More.html | forin-notebook-social.jsx | A 동료 목록(ColleaguesNb) · B 동료 상세(ColleagueDetailNb) · C 길찾기(WayfindingNb) · D 스플래시(SplashNb) · E 로그인(LoginNb) |

## 화면별 핵심 결정
- **홈**: 오늘의 할 일(테이프 종이) · 호출 쪽지(!! 점선) · 과별 출근 카드 · 오늘의 문장(형광펜) · 연속출근 도장.
- **일터**: 실구현 구조 이식 — 검색 밑줄 → ★즐겨찾기 쪽지 → 건물 아코디언(층 행: 잉크 층스탬프 + 진행 네모칸 + 별) → 탐험 모드 점선 버튼. 층 탭 → **바텀시트**: 스탯 2칩 + 이 층의 커리큘럼(완료=취소선·통과 / 진행=노란 테두리 + 스텝: 대화speech·퀴즈pencil·시험trophy, ✓/다시/lock) + 커리큘럼 밖 상황(완료=복습 / 긴급=빨간 쪽지 시작!).
- **상황 준비**: 폴라로이드 초상 + 위치/레벨 태그 + 감정 메모(빨간 점선) + 미션 ☐ 4 + 보상/커리큘럼 좌표 + 잉크 CTA + 보기모드 링크.
- **대화**: 초상화 무대(테이프 폴라로이드 + 이름 스탬프[nowrap 필수] + 감정 태그) / 영역 조절 grabber 3곳(무대·메시지·옵션) / QUICK INFO 칩 / 직접 말하기(SPEAK FREELY 모노 라벨 + 마이크 종이함) vs 보기 선택(옵션 카드 + 노란 말하기 탭 + 접기 + 직접 입력 링크).
- **리뷰랩**: **견출지(인덱스 스티커) 탭** 3분할이 최상단 — 비활성 = 파스텔 견출지(핑크/블루/그린)가 살짝 기울어져 뒤에 꽂힘, 활성 = 종이색으로 크게 앞으로 나와 본문과 경계선 없이 이어짐 + 탭 위 테이프 조각. (NbUI.NbIndexTabs 동일 스타일) 교정 카드 = 맥락 ✎ → 빨간펜 취소선 → → 형광펜 교정 + 듣기 → 왜?(파란 점선) → 숙련 점 3 → SRS 4버튼(다시<1분/어려움10분/알맞음1일/쉬움4일).
- **프로필**: 사원증(이름 밑줄 필기란 + RANK 타자체 + 빗금 연필 레벨 게이지) · CAREER PATH(체크→HERE) · 칭호 4열(잠김 ??? 반투명, 장착=보라 테두리+태그) · 히든 미션 · 설정(잉크 토글). **성장 리포트**: 근무대 달력(데이/이브닝/나이트 색만, 셀 이모지 금지) + 선택일 상세 + 4스탯(시나리오·새표현·대화시간·레벨). 스티커보드·연속스트립 **삭제 확정**.
- **동료 추가**: 그린 코드 쪽지(대형 타자체 코드 + 복사/공유) + "또는" 점선 + 받은 코드 필기란(노란 포커스) + 비활성 요청 버튼 + 안내 메모.
- **발음**: 문장 카드(IPA 타자체 + 지난 약점 음소 형광 표시 + 원어민 칩 + 회차) → 주의 메모 → 원형 낙서 녹음 버튼("꾹!") → 회차 점수(빗금 게이지 + 상승 코멘트). 녹음중 = 다크 잉크 배경 + 밝은 문장 쪽지 + REC 파형 + 기울어진 형광 눈금. 채점 = 총점/3지표 → 음절 스탬프 칩(좋아요그린/애매옐로/다시핑크) → 속도 비교 다크 패널 → 교정 포인트(음소 스탬프 + 팁 + 듣기) → 다시 녹음/다음 문장 + 드릴 예고.
- **라운지(상황판 개편)**: 코르크보드 은유 — 모든 피드 카드에 **비스듬한 입체 압정**(색·위치 랜덤). 글 3종: AI 대화 공유(출처 헤더 + "연속 N턴" + 말풍선 스니펫), 현지 근무자 글(그린 뱃지), 질문 글. 대화 공유 작성: **연속 턴만 선택 가능**(건너뛰면 ✕ 잠김) + 최대 6턴 + 한마디/태그. 상대 프로필 시트: 현지 근무중 뱃지 + 스탯 + 최근 글 + 동료 요청(handshake2)/응원 — 멘토·멘티 확장 진입점.

## 알려진 규칙(재발 방지)
- 모든 pill/라벨/스탯 텍스트에 whiteSpace:'nowrap' (한글 임의 줄바꿈 방지).
- 폰 프레임 boxSizing:'border-box' 필수.
- NbIcon에 없는 이름은 별(star)로 폴백됨 — 새 아이콘 쓰기 전 반드시 세트에 추가.
- design-canvas.jsx: 가벼운 페이지는 DCArtboard eager={true}로 즉시 마운트(스로틀 환경 레이지 마운트 실패 대응).

## NbUI 공용 컴포넌트 킷 (forin-notebook-ui.jsx · window.NbUI)
중복되던 소품을 컴포넌트화. 모든 버튼·칩은 **누름 인터랙션 내장**(.nb-press: 눌리면 1.5~2px 가라앉고 그림자 소멸 / .nb-chip: scale .94).
- NbFrame(폰 프레임+하단 탭, dark 지원) · NbPaper(rot/tape/pinned) · NbTape · NbPin(입체 압정)
- NbButton(variant: ink/paper/yellow/dashed/danger · size sm/md/lg · icon) · NbChip(필터 토글) · NbTag(필)
- NbInkStamp(층표) · NbStamp(이중선 원형 도장) · NbMark(형광펜) · NbMemo(점선 메모)
- NbGauge(빗금 연필 게이지) · NbCheck(체크박스) · NbProgSquares(진행 네모칸) · NbSearchLine · NbIndexTabs(견출지 스타일) · NbGrabber
- 카탈로그: forin Notebook - Home.html → "Z · NbUI 컴포넌트 킷" 아트보드 (직접 눌러볼 수 있음)
- 마이그레이션 방침: 기존 화면은 렌더 결과 동일하므로 그대로 두고, **신규/수정 화면부터 NbUI 사용**.

## 온보딩 인터랙티브 플로우 스펙 (forin-notebook-onboarding-flow.jsx · OnbFlow)
여정: 표지(여권) → 직업 → 목적지 → 여권 닫힘 → 비행 → 입국심사 → 입국 승인 → 출근 스플래시 → (루프)
- **여권 표지**: 딥그린 #2E4636 + 금색 이중 테두리 · "PASSPORT / forin" · 금장 이중원 엠블럼 안 손글씨 **f** · 하단 MRZ 2줄. 표지 = 로그인 화면: 소셜 3버튼(공식 브랜드 리소스: Google 공식 G 로고 SVG + 흰 버튼, Apple 검정 버튼, 카카오 공식 로그인 이미지 uploads/kakao_login_large_wide.png)만 — id·pw 없음, 기존 유저는 OAuth로 자동 로그인, 초기화는 프로필에서.
- **페이지 넘김(CurlOut)**: 24관절 중첩 3D 컬 — 베이스 회전(-128°, 끝까지 넘어가며 말미 페이드)과 관절 컬(48% 시점 최대 곡면 → 38%로 풀림) 2단 합성. 슬라이스 균일 명도 오버레이(인접 차 ~1%)로 줄무늬 없음, 오른쪽 끝부터 들림(관절당 4.5ms 딜레이), 1.25s. 다음 장 위로 그림자 스윕(nb-sweep).
- **목적지**: 카드 선택은 하이라이트만(자유 변경). **출국하기를 누르면** 옵션-버튼 사이 공간에 ADMITTED {나라} 도장이 쾅(nb-stamp) → 1.15s 후 여권 뒷면(CurlIn: 역방향 컬, BON VOYAGE)이 닫힘 → 1.7s 후 비행.
- **비행**: 구름 루프 + 점선 항로 + 비행기 nb-fly(2.3s) · 문구/공항코드는 DESTS[dest] 파생(ICN → JFK/SYD/YVR/LHR) · 2.7s 후 입국심사.
- **입국심사**: 배경 선준비(위 하늘/아래 종이) → 심사관이 몸(-13px 오버슈트)·머리(-30px, 지연)·모자(-84px 치솟아 회전 낙하) 관성 분리로 벌떡 등장 → 1.3s 후 영어 대사 타자기(34ms/자, 커서) + 한국어(24ms/자) → 완료 전 옵션 비활성(흐림).
- **전환 규칙**: 심사대 통과 = 좌측 슬라이드(0.6s, 종이 넘김 아님) · 첫 출근하기 = 출근 스플래시(낙서 캐릭터 워크 사이클: 다리 교차 nb-leg + 봉봉 nb-bob + 병원 목적지) 3.1s 후 리셋.
- **뒤로가기**: 직업/목적지 우상단 ‹ = **goBack(CurlIn으로 이전 장이 되돌아옴**, 표지는 그린 배경) · 입국심사 ‹ = **귀국 비행**(step 8: 반대 방향 비행, "{APT} → ICN · 다시 집으로") 2.7s 후 목적지 선택으로 복귀.

### 2026-09 추가 화면 결정
- **퀴즈 6유형**: 공통 헤더 = ✕ 그만두기 쪽지 + 과 태그 + n/6 + 기울어진 잉크 진행 바. ① 문장완성 = 빈칸 밑줄 + 낱말 칩 종이. ② 용어 잇기 = 좌우 종이 카드 + 펜 선(확정 실선 그린/진행 점선 블루). ③ 객관식 = 원형 낙서 라디오(✓그린/✕레드) + 선택지별 손글씨 해설 + 왜? 메모. ④ 받아쓰기 = 스피커 쪽지(파형 낙서+배속) + 필기란 3줄(깜빡 커서). ⑤ 순서 배열 = 번호 원 + 확정/미정(점선) 카드 + ↕ 끌기. ⑥ 안전 체크 = 손그림 체크박스 복수 선택 + '위험' 태그.
- **결과**: PASSED 근무 완료 도장(그린) → 근무 요약 4스탯 종이 → 미션 체크리스트(미달성=태그 '다음에!') → 빨간펜 교정 프리뷰 + '복습 노트에 저장됨' → 다시 보기/다음 근무.
- **복습 세션**: 카드 스택(뒤 2장 겹침) + D+3 태그 + 앞면 한국어 상황 → 뒤집기 + '먼저 말해보고 확인' 마이크 → 하단 SRS 4버튼(종이 버튼).
- **동료**: 목록 = 폴라로이드 아바타 + 현지 근무중 그린 필 + 연속/출근 여부 + 응원 버튼, 상단 코드 맺기 쪽지(RN-JIMIN) + 이번 주 함께 목표(빗금 게이지). 상세 = 폴라로이드 헤더 + 3스탯 + 최근 라운지 글(압정) + 응원/멘토 요청(현지 근무자만).
- **길찾기**: 손그림 약도 종이(방 = 옅은 워시 사각 + Gaegu 라벨) + 빨간펜 점선 경로/화살표 + 현재 위치 블루 점 + 목적지 ★ 점선 원 + 단계 3스텝('지금 여기' 태그).
- **스플래시/로그인**: 스플래시 = 여권 그린 + 금장 f 엠블럼 + FORIN 자간 로고 + 점 로더. 로그인 = f 잉크 엠블럼 + '다시 출근할 시간이에요' + 소셜 로그인 3종이 카드만(Google 종이/Apple 잉크/카카오 옐로 — id·pw 입력 없음) + 약관 동의 문구.

### 병동 특화 퀴즈 6종 (forin-notebook-quiz2.jsx · 픽셀 라인 유형 이식)
- **바이탈 라벨링(ER)** = 종이 위 다크 모니터 낙서 + 수치별 라벨 쪽지 드래그(확정 = 흰 쪽지 ✓, 미정 = 점선 ?) + 쇼크 징후 주의 메모.
- **ESI 판정(ER)** = 환자 메모 종이 → 1~5 이중선 도장 중 하나를 찍기(선택 = 링 + 워시) + 기준 메모.
- **용량 계산(약국)** = 처방전 종이 + 연습장(파란 손글씨 계산 + 빨간 동그라미 답) + 답 필기란 + 숫자 쪽지 키패드.
- **신체부위 라벨링(병동)** = 낙서 인체 종이 + 색 화살표 ①②③ + 부위별 답 쪽지/필기란 + 어휘 쪽지.
- **APGAR 채점표(NICU)** = 5항목 × 0/1/2 원 선택(확정 = 이중선 그린) + 누적 점수 도장 + 기준 메모.
- **라벨 오류찾기(약국)** = 처방전 vs 조제 라벨 나란히 + 오류에 빨간 동그라미(10배 용량·복용 시점) + 찾은 오류 카운터.

### 병동 특화 퀴즈 II (forin-notebook-quiz3.jsx · 12종, O~Z)
- **ICU 벤트 알람**(MCQ) = 다크 알람 패널(HIGH PRESSURE) + DOPE 원칙 해설.
- **ICU 승압제 적정**(게이지) = 지시 종이(Target MAP) + 펌프 게이지 바(목표선 점선) + ±0.02 버튼.
- **OR 기구 매칭** = 기구 낙서 3종 카드 + 기구 노트(forceps/retractor/hemostat).
- **OR 타임아웃**(말하며 체크) = 체크리스트 + 항목별 영어 대사 이탤릭 + 마이크로 체크.
- **분만실 FHR 판독**(스트립 MCQ) = 손그림 CTG(FHR 실선 + UC 점선) + early/late/variable.
- **소아과 쉬운 말**(용어→보호자 말 잇기) = NPO/febrile/IV/antipyretic + teach-back 팁.
- **정신과 위험 신호**(복수 선별) = 환자 발화 이탤릭 쪽지 + '보고' 태그 + 경고신호 해설.
- **재활 이동 보조**(순서) = 게이트벨트→앉히기→건측 배치→구령 기립 + 건측 힌트.
- **투석 수분 계산**(연습장) = 체중 3칩 + 손글씨 계산 + UF Goal 필기란 + 영어 문장.
- **내시경 NPO**(문장 완성) = midnight 확정 + clear liquids 칩 + 흡인 위험 해설.
- **정형외과 CMS**(판정 MCQ) = C/M/S 3행 판정 태그(정상/주의/위험) + 구획증후군 보고 문장.
- **감염관리 PPE**(순서 그리드) = 착용 4단계 아이콘 카드(미정 점선) + donning/doffing 암기 메모.

퀴즈 유형 라이브러리 현황: 공통 6 + 특화 I 6 + 특화 II 12 = **24유형**. 병동별 10~20개 목표는 공통 유형(문장완성·잇기·객관식·받아쓰기·순서·체크)을 병동 콘텐츠로 돌려쓰기 + 특화 유형 조합으로 충족.

## NbAvatar 아바타 에셋 시스템 (forin-notebook-avatar.jsx · forin Notebook - Avatar.html)
프로필 초상화 그림체(2px 잉크 외곽선 + 플랫 파스텔 필 + 점눈/선입, 64×70 흉상) 기반의 레이어 조합 컴포넌트.
- **레이어 순서**: 배경 → 뒷머리 → 옷 → 얼굴(피부) → 입 → 눈 → 앞머리 → 모자 → 액세서리.
- **props**: skin·hair·hairColor·eyes·mouth·outfit·outfitColor·hat·hatColor·bg·acc·size (키 또는 hex 직접 지정 가능).
- **에셋 수**: 피부 8 · 머리 스타일 19 · 머리색 12 · 눈 15 · 입 14 · 옷 18 · 옷색 12 · 모자 12 · 배경 10 · 액세서리 16 — 조합 수십억.
- **NPC 통합 원칙**: NPC 전용 풀은 없다 — 환자복·구급대원·보안 제복·정장·가디건·방호복(후드는 얼굴 뒤 원으로 표현), 지친/걱정/화남 눈, 통증/이 악묾 입, 콧수염·턱수염·주름·산소 캐뉼라·페이스실드 액세서리, 보안 정모까지 전부 공용 풀에 있고 사용자도 착용 가능. 초상화 NPC는 이 풀의 조합 프리셋(쇼케이스 'NPC 프리셋' 12종: 환자·노인 환자·의사·수간호사·보안요원·구급대원·보호자·약사·방호복 간호사·원목·통증 환자·외과의)으로 만든다.
- **머리 스타일 19종**: short·part·midPart(중간 가르마)·buzz·curlyShort·bob·ponytail·bun·twintails·longStraight·curlyLong·wavyMid·wavyLong·fringe·spiky·afro·braid·baldFringe(탈모: 테두리 안쪽 얇은 원호 옆머리 + 정수리 가닥 2줄)·bald.
- **두상 곡률 규칙(중요)**: 모든 머리·모자 돔은 얼굴 원(중심 32,32 · r14)과 **동심 원호**로 그린다 — 앞머리 r13.7, buzz r13.7, 뒷머리 r15.2 (외곽 스트로크 포함 얼굴 원 안쪽 안착). 납작한 Q커브 돔 금지.
- **긴 머리 규칙**: longStraight·curlyLong·wavyMid/Long 뒷머리는 목 뒤 중앙까지 채운 통 커튼(중앙 공백 금지 — 갈라지면 양갈래로 오독). 양갈래(twintails)·포니테일 꼬리·braid는 얼굴 원 밖으로 3px 이상 떨어뜨린다.
- 사진→아바타 매핑을 위한 커버리지 설계: 피부 8단계 명도 스케일, 머리색에 염색 포인트(red/pink/mint) 포함, 마스크는 착용(maskOn)/턱걸침(maskChin) 2종.
- 키 목록은 window.NbAvatarAssets로 노출 — 랜덤/매핑 생성에 사용.

## 온보딩 플로우 2 — "여권 발급" (forin-notebook-onboarding-flow2.jsx · OnbFlow2)
신분(ID) 페이지의 빈칸을 순서대로 채워 여권을 발급하는 대안 플로우 (★★ 아트보드).
- **ID 페이지**: 여권 신분 페이지 레이아웃 — 사진 칸(96×118 점선) + NATIVE LANGUAGE·OCCUPATION·GIVEN NAME 필드 + 하단 MRZ 2줄(채운 값이 실시간 반영: 이름 대문자·직업/언어 코드).
- **채우기 순서 고정**: 사용 언어 → 직업 → 사진(아바타) → 이름. 다음 빈칸이 노란 점선+‘여기를 채워요 ✎’ 깜빡임으로 활성화되고, 그 칸 위치를 transform-origin으로 **선택지가 화면 중앙으로 확대**(nb2-zoom, 배경 딤) → 고르면 줄어들며 칸이 nb2-fill 팝으로 채워짐 → 0.6s 후 다음 픽커 자동 오픈.
- **픽커 4종**: 언어 6개 국기 그리드 / 직업 4행(간호사만 활성) / 아바타 NbAvatar 프리셋 6종 3열(“나중에 프로필에서 꾸미기” 힌트) / 이름 필기란 input(Enter 또는 ‘이렇게 적을게요 ✎’).
- **수정 가능**: 채워진 칸을 탭하면 해당 픽커 재오픈.
- **이후**: 4/4 완료 → ‘다음 장 넘기기’(CurlOut) → 목적지 페이지(이름 호명 “OO 님, 어디로 떠나나요?”) → 도장 쾅 → 출국하기 → CurlIn 뒷면 커버(“OO 님의 여권이 발급됐어요”) → 리셋 루프.
- **플로우 1에도 이름 추가**: 직업 페이지 하단 ‘여권에 쓸 이름’ 필기란 → 입국 승인 사원증에 RN · {이름} 반영.
- CurlOut/CurlIn/Gutter는 flow1이 window.NbOnbFX로 export — flow2가 재사용 (중복 정의 금지).

## 홈 개선안 — "출근 브리핑 데스크" (forin-notebook-home2.jsx · HomeV2)
홈 = 매일 아침 받는 근무 브리핑 쪽지. 모듈 순서(위→아래):
1. **라이브 병동**(LiveWardNb) — 기기 시간 연동 3무드(DAY 해/EVENING 노을/NIGHT 별+달+어둠 워시). 수첩 낙서 병동: 침대 3, 바이탈 모니터(파형 펄스), **NbCharacter** 간호사 순회(nbh-walk + 아장아장 waddle) — 하단 무드 바가 시간대별 시나리오 출현 확률 변화를 설명(회진/SBAR/야간 호출).
2. **오늘의 호출**(PagingNb) — 하루 1회 페이저 쪽지: 빨간 헤더 + 흔들림 + 빗금 제한시간 바(오늘 놓치면 소멸) + [지금 응답 → 3분 단기 시나리오 / 무시] + +40 XP. 응답 후 한 줄 '✓ 응답함'으로 접힘.
3. **오늘의 근무 브리핑**(TodayBrief) — 할 일 3칸 체크(복습 5장·커리큘럼 이어서·오늘의 문장), n/3 카운터, 완료 시 취소선, 3/3이면 근무 도장+연속일.
4. **이어서 하기**(ContinueCard) — 커리큘럼 좌표(ER 2-3) + 빗금 진행 바 + 계속.
5. **오늘의 문장**(PhraseCard) — 형광펜 문장 + 듣기/따라하기.
6. **동료 소식 한 줄**(PeerLine) — 라운지 연동 1건 + 응원 링크.
아트보드 A(DAY·호출 대기·1/3) / B(EVENING·응답 완료·2/3) / C(NIGHT·3/3 완료, 인사말 "오늘도 무사히, 밤 근무"). 기존 홈은 참고 섹션으로 보존.

### NbCharacter — 전신 2등신 캐릭터 (forin-notebook-avatar.jsx)
- NbAvatar 에셋 완전 호환: 머리는 NbAvatar의 BACK/FRONT 머리·눈·입 레이어를 0.8 스케일로 그대로 재사용 (skin·hair·hairColor·eyes·mouth·outfitColor props 공유).
- 몸: 2등신 — 옷색 몸통(V넥 선, y38 시작 — 머리와 살짝 겹치게 밀착) + 짧은 팔 2(끝에 살색 손) + 짧은 다리 2(발 꺾임). viewBox 64×96, waddle 기준점 32,83.
- walk=true: 아장아장 — 몸 전체 waddle(±3.5° 발끝 기준) + 팔·다리 교차 스윙(nbc-limb ±22°, .42s alternate). flip=true로 좌우 반전.
- 용도: 홈 라이브 병동 순회 캐릭터, 향후 탐험 모드·라운지 연출 공용.

## 재미 콘텐츠 (forin Notebook - Extras.html · forin-notebook-extras.jsx)
- **나이트 근무 라디오**(NightRadio) — 밤 10시~새벽 5시에만 열리는 다크 페이지(밤하늘 별 깜빡임). 낙서 라디오(주파수 눈금 "FM 3.5 · 3병동" + EQ 바) + Lo-fi 트랙 재생 + '오늘 밤의 이야기'(실화 기반 3분 미니 스토리 → 핵심 문장 형광펜 + 따라 말하기) + 밤 학습 XP 1.5배 메모.
- **환자 인수인계 노트**(HandoffNotes) — 만난 AI 환자들의 후일담 쪽지함: 퇴원 감사(답장 한마디), 후속 시나리오 예약(출산→수유 상담), 복습 연결(배액관 제거 표현). 미읽음 = 테이프 + 빨간 점. 대화 품질(정확도·공감)이 좋을수록 후일담 확률↑.
- **슬랭/약어 카드 도감**(SlangDeck) — 하루 1장 떨어지는(nbe-drop) 오늘의 카드(code brown: 모노 대형 워드 + 뜻 + 실전 예문) + 3열 수집 그리드(획득/내일 잠금 점선) + 10장마다 히든 카드, 30장 '은어 마스터' 칭호.
- **미니게임 허브**(GameHub) — 쉬는 시간은 간호와 무관한 캐주얼 6종: 종이비행기 날리기(타이밍 게이지)·펜 뚜껑 스피드(반응속도)·낙서 짝 맞추기(기억력)·완벽한 원 그리기(정확도 100점 채점)·복도 달리기(러너)·왼오른 농구(번갈아 슛). 하루 3판 + **이번 주 동료 랭킹**(일요일 리셋, 내 순위 형광펜) + 1위에게 도전장 보내기(같은 판으로 대결).
- **완벽한 원 그리기 인게임**(CircleGame) — 점선 목표 원 + 파란 손그림 궤적(구간 피드백 good!/삐끗) + 중앙 대형 점수(평균 오차 px 기반 100점 만점, 손 떼면 즉시 채점) + 동료 최고점 표시.
- **복도 달리기 인게임**(RunnerGame) — 크롬 공룡식 러너: NbCharacter가 병원 복도를 달리고 단일 점프 버튼(누른 시간에 비례해 점프 높이 — 홀드 게이지 표시), 낮은 장애물(청소 카트)=뛰어넘기·높은 장애물(걸이 표지판)=밑으로, 속도 ×1.6 가속 + 속도 비례 장애물 간격 보장(불가능 배치 금지) + 거리 점수·동료 1위 비교.
- **왼오른 농구 인게임**(BasketGame · 플레이 가능 프로토타입) — 탭=공 위로 튐(궤적 표시 없음). 화면엔 목표 골대 **하나만** 보이고 반대쪽 골대는 화면 밖 월드에 존재 — 득점 순간 카메라(translateX 0↔150, .55s)가 반대편으로 슬라이드하며 다음 골대가 들어온다. 골대(레퍼런스 추종) = 측면 백보드 패널(흰 상단+아이보리+회색 브래킷) + 뒤 노란 나무 기둥 — 기둥에서 화면 가장자리 밖으로 뻗는 어두운 나무 마운트 암 2개(위·아래)로 "화면 옆면에 매달린" 구조 표현. 림 = 굵은 파란 타원(살짝 아래 시점). 그물 = 잉크색, 림 앞테두리 가닥(진하게)+뒤테두리 가닥(opacity .35)의 2겹, 수직 하강 → V-메시 매듭이 아래로 갈수록 안쪽으로 수렴 → 좁은 입구(단조 오므라듦 — 중간 벌어짐 금지). 공 = 주황 라디얼 그라데이션 + 갈색 곡선 심. 득점 판정 = 하강 중 림 높이 통과 + 림 폭 이내. 바닥에 떨어지면 그 자리 리셋(연속 점수 유지), 연속 카운터 + 최고 기록.
- **종이비행기 인게임**(PlaneGame) — 하늘 낙서(구름·점선 궤적·"슝—") + 지면 거리 눈금 + 내 최고 기록 깃발 + 결과("37.2m! Jiyoon까지 11m") + 파워 게이지(초록 구간 탭 PERFECT ×1.5) + 시도 3회.

## 앱 아이콘 (확정: 여권 커버 · forin Notebook - App Icon.html)
- **원본 1024×1024**: 딥그린 #2E4636 바탕(미세 그레인 + 하단 비네트) + 금색 #D4B46A 이중 사각 테두리 + 금장 이중원(외 r200/19px, 내 r163/9px) 안 손글씨(Gaegu) **f** + 하단 FORIN 타자체(IBM Plex Mono 700, 넓은 자간).
- **축소 규칙**: 중형(~120px) = 테두리 1겹 + 이중원 + f + FORIN 유지 / 소형(~60px) = 사각 테두리·FORIN 제거, 금장 단일원 + f만 크게.
- **선정 근거**: 아이콘은 앱의 '표지' — 여권 커버를 여는 행위가 곧 앱 실행이라는 온보딩 은유와 일치. 크림 내지(B안)는 밝은 홈 화면에서 존재감이 약함.
- **이중 컨셉 원칙(여권↔수첩)**: 여권(딥그린+금장)은 1회성 의식(온보딩·국가 변경)에만, 근무 수첩(크림+잉크)은 일상 공간에 사용 — 병렬이 아니라 '출국 → 현지 근무'의 서사적 순서로 연결한다. 두 세계의 접착제는 공통 토큰: 잉크 #3E362B, 손글씨 f, Gaegu+IBM Plex Mono, 도장·스탬프 문법. 이 토큰을 벗어난 제3의 스타일 추가 금지.

## 메인 루프 인터랙티브 데모 (forin Notebook - Main Loop.html · MainLoop)
클릭으로 이어지는 핵심 루프 프로토타입: **홈 → 상황 준비 → 대화 → 결과 → 리뷰랩 적립** (하단 진행 칩 5개).
- **홈(축약)**: 오늘의 상황 카드(ER 트리아지, 약 3분) → [출근해서 맡기].
- **상황 준비**: 폴라로이드 NbAvatar 환자(통증 표정) + 미션 체크 2개(pain scale/reassurance) + 보상 메모 → [대화 시작].
- **대화(감정 전환)**: 2턴 대본, 선택지 2개(좋은/아쉬운 답 + 즉시 피드백 메모). 답에 따라 환자 NbAvatar 표정이 실시간 전환 — 통증(uu/pain) → 불안(worried/frown) → 안심(happy/smile), 전환 시 nbl-face 팝 애니메이션 + 감정 태그 색 변경.
- **결과**: CLEAR 이중선 도장 팝 + 모범 답변 n/2 + 답변 로그(✓/✕ + 교정 태그) → [리뷰랩에 적립 보기].
- **리뷰랩 적립**: 아쉬운 답이 있으면 빨간펜 취소선 → 형광펜 교정 카드로, 전부 정답이면 표현 카드로 '방금 적립' 뱃지와 함께 저장 + SRS 4버튼 + "내일 아침 오늘의 복습에 나타나요" 루프 클로저 → [처음부터 다시 ↺].
- 화면 전환 nbl-in 슬라이드. 데모는 선택식이지만 직접 말하기 진입점(마이크 안내) 표기.

## 여정 지도 — 커리큘럼 진행감 (forin Notebook - Journey.html)
일터 탭의 커리큘럼을 목록 대신 '여정'으로 보는 뷰 (목록으로 보기 토글 공존).
- **A 여정 지도(JourneyMap) — 목표 부서 트랙 구조(중요)**: 모든 부서 일렬 통과 금지 — "부서 하나만 파면 타 부서가 쓸모없어 보인다"는 우려의 해법은 프레이밍. 경로 = ①공통 필수(기본기·인계, 부서 무관 코어) → ②목표 부서(ER) 심화 정거장 → ③**협업 정거장**: 타 부서를 내 부서 시점의 상황(ER→ICU 인계, ER→약국 전화, ER→OR 이송)으로 경로에 삽입 — 파란 점선 링 + "협업 · ER→ICU" 파란 라벨로 구분. ④지도 아래 **자유 탐방** 가로 칩(잠금 없음, 부서별 도장 컬렉션 표시). 정거장 상태 표기는 동일(PASSED 이중선 도장 / HERE 깃발+노란 파선 링 / 열린 원 / 점선 원). 상단 'ER 트랙' 태그 + '트랙 변경' 링크(목표 부서 교체). 하단 고정 '현재 정거장' 바.
- **B 정거장 상세(StationSheet)**: 바텀시트 — 정거장 스텝 5(대화speech/퀴즈pencil/시험trophy), 완료 취소선+통과 ✓ / 현재 노란 테두리+시작 / 잠김 lock. 구간 시험 통과 = 지도에 도장.
- **C 면허 로드맵(LicenseTrack)**: 국가 트랙(미국 RN) 세로 타임라인 — 서류/NCLEX 모의/OET/본시험/비자, 상태색(그린·앰버·블루·소프트), 현재 단계에 진행 네모칸 + 학습 연동 문구("ICU 구간 통과 → 본시험 준비도 +8%"). 여정 지도의 마일스톤 깃발과 동일 소스.

## 레벨 진단 — 배치고사 (forin Notebook - Placement.html)
온보딩 입국 직후, 첫 출근 전 '수간호사 면담' 프레이밍의 5분 진단. 결과가 여정 시작점과 대화 난이도를 배치.
- **A 안내(PlaceIntro)**: DAY 0 이중선 도장 + "못해도 괜찮아요 — 배치일 뿐" 카피 + 구성 3종(듣고 답하기 3 / 따라 말하기 2 / 병원 어휘 3) + [건너뛰고 기초부터] 탈출구.
- **B 듣고 답하기(PlaceListen)**: 수간호사 NbAvatar(번 헤어·네이비 스크럽) 폴라로이드 + 발화 종이(스피커·다시 듣기 1회) + 3지선다. 진단 중 정오답 미공개 명시.
- **C 결과(PlaceResult)**: PLACEMENT B1+ 이중선 도장 + 한 줄 진단(형광펜 강조) + 영역별 빗금 게이지 3종 + 배치 3카드(여정 시작점 = 공통 필수 스킵 / 대화 모드 비율 / 집중 처방 발음 드릴) + "배치는 언제든 조정돼요" 적응형 메모 + [이 배치로 첫 출근] / [더 기초부터].
- 여정 지도(08 문서)와 연동: 시작 정거장·모드 비율이 여기서 결정됨.

## 리텐션 시스템 (forin Notebook - Retention.html)
- **A 잠금화면 푸시(PushLock)**: 다크 잠금화면 목업 + OS 알림 3종 — 스트릭 위기(보호권 언급), 복습 골든타임(잊힐 확률 %), 야간 호출(+40 XP·기한). 앱 아이콘 = 그린 f 뱃지. 설계 원칙 명문화: 하루 최대 1건(호출 예외) · 학습 습관 시각 ±30분 · 죄책감 금지, 구체적 이득(3분/+40XP/78%)으로 말하기.
- **B 스트릭 보호(StreakGuard)**: 주간 도장 스트립(완료 = 이중선 ✓ 도장, 동결 = 파란 물방울, 오늘 = 앰버 파선) + 보호권 지갑(동결권 자동 사용 / 응급 복구권 24h·7일 개근 지급) + 자정 위기 모먼트 카드(23:40 — 1분 미니 복습으로 출근 도장 인정). 철학: 스트릭은 채찍이 아닌 안전망.
- **C 알림 설정(NotifSettings)**: '나의 출근 시간' 슬롯 선택(±30분 약속) + 알림 5종 잉크 토글 + 약속 메모(마케팅 알림 0건 · 2주 무반응 시 자동 침묵).

## 여정 확장 — 우표 정거장 + 부서→주제→상황 3뎁스 (forin-notebook-journey2.jsx)
정보 구조: **부서(차트 바인더) → 주제(인덱스 간지, 부서당 5±) → 상황(우표, 주제당 30~40)**.
**부서는 비선형** — 사용자는 자기 근무 예정 부서부터 학습하므로 부서 간 잠금·순서 없음, 부서별 독립 진행. 공통 필수(손위생·환자확인·SBAR 기본)만 전 부서 공유(한 번 완료하면 모든 바인더에 완료 반영).
- **G 바인더 서가(BinderShelf, 진입 화면)**: 상단 '내 부서' 바인더가 펼쳐진 카드(앰버 링·주제 6칸 게이지·이어서) → 공통 필수 점선 카드 → **서가 2단(바인더 8권)**: 색 등(spine)·등 라벨(코드/이름)·아이콘·하단 잉크 진행바 + 읽던 곳 파란 테이프 표시. 아무 바인더나 꺼내 E로 진입. 추천 순서는 강제 아닌 메모.
- **H 불규칙 우표 산책길(StampTrail, F의 대안)**: 36장을 D 스타일 지그재그 산책길로 — 손으로 흩뿌린 듯한 x 지터(60~330)·y 82px 간격 지터, 우표별 손글씨 상황명 라벨(먼 잠김은 '· · ·'), 구간 경계 잔선+손글씨 라벨(기초/실전/심화), 낙서 장식, 빨간 실(완료→현재) + 연필 점선(미래), 진입 시 현재 우표 근처로 자동 스크롤. 한 화면에 ~8장, 세로 총 3,000px.
- **✅ 확정 플로우 (2026-09-23): G 바인더 서가 → E 차트 바인더 간지 → H 불규칙 우표 산책길.** H 최종 사양: D의 10점 배치 패턴(70/80 · 200/130 · 318/92 · 300/232 · 160/262 · 64/372 · 212/400 · 330/372 · 250/520 · 96/540)을 600px 주기로 반복(홀수 주기 x±8 지터), 우표 62px, 라벨 top 66, 낙서 세트(구름·별·십자·점) 주기마다 동일 배치, 구간 경계선 y−62.
- 보관(미채택, 비교용 유지): A 점선 원 지도 · D 단독 대안 · F 우표 앨범(조감형). 개발은 G→E→H만.
- **공용 부품**: `NbStampNode`(우표 노드 — 톱니 테두리 SVG, 상태 done/here/next/locked, 번호 스탬프, done = 이중선 ✓ 도장, here = 살랑이는 빨간 깃발) · `NbYarn`(빨간 실 — 점 배열을 흔들리는 Q커브로 잇고 각 점에 핀, 끝 핀 앰버, 드로잉 애니메이션).
- **D 우표 정거장 대안(JourneyStampsAlt)**: A의 점선 원 대신 우표 노드 + 완료 구간은 빨간 실, 미래 구간은 연필 점선 + 낙서 장식(구름·별·십자). 규칙적 배열을 깨는 지그재그 배치.
- **E 차트 바인더(TopicBinder)**: 부서 헤더(아이콘 카드 + 주제/상황/우표 합계) + 부서 게이지(주제 비율 구간 분할) + 바인더 링 + 주제 간지 스택: 오른쪽 색 인덱스 탭(01~05), 진행중 간지는 테이프+앰버 링, **상황 수만큼 미니 우표 격자(9px)** 로 30~40 규모를 한눈에 · 완료 이중선 도장 · 잠김은 해제 조건 문구. 추천 순서 메모(실무 순서).
- **F 우표 앨범(StampAlbum)**: 주제 헤더 + 구간 칩(기초 1–12 / 실전 13–24 / 심화 25–36) + 앨범 종이(격자 라인) 위 5열 우표 그리드(±3° 기울임) + 좌측 손글씨 구간 라벨 + **빨간 실이 완료 우표 12장을 순서대로 꿰어 13번(here)까지** + 하단 현재 우표 카드(제목·모드·시간·XP·시작). 앨범은 세로 스크롤, 실은 스크롤에 따라 함께 이동.
- UX 원칙: 30~40개를 목록으로 나열하지 않고 **한 페이지 우표 앨범**으로 압축 — 진도는 '채워진 면적'으로, 위치는 '실 끝'으로 직관 인지. 구간 칩으로 점프.

### G→E 전환 연출 (BinderFlyDemo) · E 게이지 재설계
- **바인더 날아오기**: 탭한 바인더 등(spine)의 중심 좌표를 기준으로 `--fx/--fy` 오프셋 계산 → 표지 레이어가 scale .22·rotate −10°에서 화면 중앙까지 날아와(0.62s, 오버슈트 1.03) → 표지가 왼쪽 축 rotateY −88°→0로 **펼쳐지며** E 내지 노출(0.55s). 비행 중 큰 드롭섀도.
- **뒤로(‹ 서가)**: 표지가 닫히고(0.45s) 바인더가 scale .22로 원래 서가 위치로 **날아 돌아감**(0.68s, ease-in). 서가는 그동안 페이드 유지.
- 표지 디자인: 빨간 등 46px + 흰 등 라벨(ER / 응급센터 차트) + 대형 부서 아이콘 + 하단 진행바.
- **E 부서 게이지**: 주제 35개를 칸으로 쪼개지 않음 → **3구간 한 줄 잉크 게이지**(완료 그린 / 진행중 앰버 빗금 / 남음 빈칸) + 5주제 단위 눈금 + 범례(완료 14 · 진행 3 · 남음 18) + 14/35 스탬프. 헤더 카피도 '주제 35 · 상황 1,180'.

## 상황 학습 4단계 (forin-notebook-lesson.jsx · Dialogue 페이지)
커리큘럼 상황 1건 = **단어 → 문장 → 보기 대화 → 자유 대화** 4 STEP. 사용자 피드백 반영: 텍스트 최소, 아이콘·큰 버튼·시각 요소 우선.
- **StepTrack(공통)** — 4개 원형 아이콘 스텝(연필·말풍선·클립보드·마이크, 색 앰버/블루/퍼플/레드) + 점선→실선 연결, 완료는 그린 ✓ 배지. STEP 화면 상단에 항상 노출.
- **A LessonHub(상황 허브)** — 구 '상황 준비(D)' 대체. 폴라로이드 NbAvatar(환자 표정 = 감정) + 태그 3(위치·레벨·시간) + 한 줄 상황 + 감정/보상/노트 아이콘 3칩 → **4단계 티켓**(STEP 카드: 아이콘 타일 + 메타 아이콘 + 상태 = 완료 도장 / 시작 › / 잠금) + 진행 게이지. CTA 문구는 done에 따라 변함.
- **B LessonWords(STEP 1)** — 8칸 아이콘 진행 칩 → 큰 플래시카드(단어 아이콘 원 + MONO 단어 + IPA + 형광펜 뜻 + 예문 한 줄 + 듣기/따라하기 원형 버튼) → 하단 2분할 '헷갈려요 / 알아요 ✓'. 헷갈린 단어는 교정노트로 자동 저장.
- **C LessonSentences(STEP 2)** — 환자 말풍선(아바타 + 감정 아이콘) → 청크 조립 슬롯(붙인 조각 블루, 빈 슬롯 점선) + 조각 풀 종이 → 한국어 힌트 메모 + 듣기 버튼 → '따라 말하기 / 확인'. C' 완료 = PASSED 도장 + 5문장 발음 점수 원형 배지 리스트.
- **D·E** = 기존 DialogueOptions(보기 대화) / DialogueSpeak(자유 대화) 재사용.
- NbIcon 추가: faceAngry. BriefingScrapbook은 Dialogue 페이지 하단 '참고' 섹션으로 이관(Home에서 제거).
- **레벨별 건너뛰기** — 온보딩 입국심사 답변(a 더듬더듬 / b 병원 영어 막힘 / c 실전 감각)에 따라 SKIP = a:[] · b:[단어] · c:[단어, 문장]. 건너뛴 STEP은 허브에서 빗금 점선 카드 + 취소선 + '그래도 할래요'(옵트인) 버튼, StepTrack에서는 빗금 원 + 사선. 게이지 분모·CTA 문구는 남은 단계 기준.
- **STEP 2 문장 유형 4종** (한 상황 안에서 섞어 출제): ① 듣고 고르기(큰 재생 버튼 + 파형 + 아이콘 뜻 카드 3) ② 청크 조립(조각 슬롯 + 풀) ③ 빈칸 채우기(핵심 청크 1 + 2×2 아이콘 선택 카드) ④ 순서 배열(공감→이유→확인→감사 대화 흐름 정렬). 모두 하단에 듣기 아이콘 버튼 + 따라 말하기 CTA.
- **NbIcon mic 재작성(2026-09)** — 좁고 긴 캡슐(4.8×10.5, rx2.4) + 캡슐 밑까지 감싸는 진짜 원호 U 요크(A5.7) + 짧은 스탠드. 이전의 납작한 Q커브 요크는 부자연스럽다는 피드백으로 폐기. 모든 Notebook 화면에 자동 반영.
- **LessonHub CTA 규칙** — 탭바가 있는 프레임(nav=true)에서 absolute CTA는 `bottom ≥ 84`(탭바 68 + 16) + zIndex 31, 스크롤 컨테이너는 `bottom:68 · paddingBottom:96`. STEP 화면은 nav=false라 bottom 30 유지.
- **STEP 3 가이드 응답(DialogueOptions 재정의)** — '보기 3개 중 선택' 폐기. 정답 문장 **1개만 한국어**로 형광펜 카드에 제시 → 사용자가 타겟 언어로 **말하기(큰 마이크 홀드)** 또는 **타이핑(밑줄 필기란 + 단어 칩 탭 삽입)**. 힌트 청크는 첫 1개만 노출, 나머지는 빗금 가림 + '힌트 더'. 하단: 보내기(타이핑 입력 시 활성) · 듣기(모범 발음) · 노트. 스텝 라벨 '보기 대화' → '가이드 대화'(아이콘 speech).
