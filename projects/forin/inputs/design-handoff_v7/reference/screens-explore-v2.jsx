...
(function () {
  ...
  function MedCenter({ x, y, w, h, label, sign, signColor, onSelect }) {
    ...
    return (
      <div onClick={onSelect} style={{
        position: 'absolute', left: px, top: py, width: pw, height: ph,
        cursor: 'pointer', filter: 'drop-shadow(4px 8px 0 rgba(0,0,0,.32))',
      }}>
        {/* LEFT — dark glass tower with silver pilasters */}
        <div style={{ position: 'absolute', left: 0, width: 38, bottom: 16, height: 146, background: 'linear-gradient(180deg,#3C4856,#28333D)', border: `2px solid ${ink}`, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', left: 9, width: 16, height: 9, top: -64, background: '#8E98A2', border: `1.5px solid ${ink}` }}/>

        {/* RIGHT — white stone tower with punched windows */}
        <div style={{ position: 'absolute', left: 60, width: 38, bottom: 16, height: 118, background: 'linear-gradient(180deg,#E4DECE,#CDC6B2)', border: `2px solid ${ink}`, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', left: 68, width: 14, height: 7, top: -36, background: '#B8B0A0', border: `1.5px solid ${ink}` }}/>

        {/* connecting glass bridge */}
        <div style={{ position: 'absolute', left: 56, width: 12, bottom: 72, height: 22, background: '#8FB8D2', border: `1.5px solid ${ink}` }}>
        ...
        <div style={{ position: 'absolute', left: 34, width: 30, bottom: 16, height: 160, background: 'linear-gradient(180deg,#F8D26A,#E89A2C)', border: `2px solid ${ink}`, borderTopLeftRadius: 9, borderTopRightRadius: 9, overflow: 'hidden', boxShadow: '0 0 12px rgba(244,196,86,.75)' }}>
          {/* bright glowing core */}
        ...
        <div style={{ position: 'absolute', left: 37, width: 24, height: 8, top: -88, background: '#D89A38', border: `1.5px solid ${ink}`, borderRadius: '7px 7px 0 0' }}/>
  ...
  function MedCenterH({ x, y, w, h, label, sign, signColor, onSelect }) {
    ...
    return (
      <div onClick={onSelect} style={{
        position: 'absolute', left: px, top: py, width: pw, height: ph,
        cursor: 'pointer', filter: 'drop-shadow(4px 7px 0 rgba(0,0,0,.3))',
      }}>
  ...
  function MedCenterV({ x, y, w, h, label, sign, signColor, onSelect }) {
    ...
    return (
      <div onClick={onSelect} style={{
        position: 'absolute', left: px, top: py, width: pw, height: ph,
        cursor: 'pointer', filter: 'drop-shadow(4px 6px 0 rgba(0,0,0,.3))',
      }}>
  ...
  function PatrolNPCs() {
    ...
    const agents = React.useRef([
      ...
      { seed: 41, kind: 'visitor', mode: 'patrol', path: [{x:16,y:30},{x:16,y:29},{x:16,y:28},{x:16,y:27}] },
      // wanderers (free roam within bounds — kept on open plaza/path areas)
      { seed: 63, kind: 'patient', mode: 'wander', bound: { x:14, y:44, w:8, h:6 } },
      { seed: 88, kind: 'parent',  mode: 'wander', bound: { x:3,  y:24, w:7, h:5 } },
      { seed: 95, kind: 'child',   mode: 'wander', bound: { x:4,  y:25, w:6, h:4 } },
      // hairstyle showcase — one free-roamer per hairstyle (patient role, so
      // the hair isn't hidden by a hat), spread across the central plaza.
      { seed: 201, kind: 'patient', hairStyle: 'short',    mode: 'wander', bound: { x:11, y:24, w:4, h:4 } },
      { seed: 202, kind: 'patient', hairStyle: 'bob',      mode: 'wander', bound: { x:15, y:24, w:4, h:4 } },
      { seed: 203, kind: 'patient', hairStyle: 'long',     mode: 'wander', bound: { x:19, y:24, w:4, h:4 } },
      { seed: 204, kind: 'patient', hairStyle: 'pigtails', mode: 'wander', bound: { x:11, y:29, w:4, h:4 } },
      { seed: 205, kind: 'patient', hairStyle: 'bun',      mode: 'wander', bound: { x:15, y:29, w:4, h:4 } },
      { seed: 206, kind: 'patient', hairStyle: 'curly',    mode: 'wander', bound: { x:19, y:29, w:4, h:4 } },
      { seed: 207, kind: 'patient', hairStyle: 'mohawk',   mode: 'wander', bound: { x:11, y:34, w:4, h:3 } },
      { seed: 208, kind: 'patient', hairStyle: 'bald',     mode: 'wander', bound: { x:15, y:34, w:4, h:3 } },
    ]).current;
  ...
  function ScreenExplore() {
    ...
    const buildings = [
      ...
      { x: 0,  y: 0,  w: 5, h: 4, roof: { mid: P.roofRed, dk: P.roofRedDk, lt: P.roofRedLt },     label: '연구동', sign: 'LAB', emblem: '🔬', arch: 'glass' },
      { x: 6,  y: 0,  w: 9, h: 5, roof: { mid: P.roofGreen, dk: P.roofGreenDk, lt: P.roofGreenLt }, label: '의과대', sign: '🎓 MED SCHOOL', signColor: '#7E3A2E', arch: 'landmark', landmark: 'victorian' },
      ...
      { x: 1,  y: 8,  w: 6, h: 6, roof: { mid: P.roofRed, dk: P.roofRedDk, lt: P.roofRedLt }, label: '본관 · MAIN', sign: 'HOSPITAL', signColor: P.red, redCross: true, mainEntrance: true, arch: 'landmark' },
      { x: 19, y: 8,  w: 6, h: 6, roof: { mid: P.roofWhite, dk: P.roofWhiteDk, lt: '#F2EDDE' }, label: '응급실 ER', sign: '🚑 ER', signColor: P.red, mainEntrance: true, arch: 'flat', helipad: true },

      // Middle row buildings  
      { x: 1,  y: 22, w: 5, h: 5, roof: { mid: P.roofBlue, dk: P.roofBlueDk, lt: P.roofBlueLt },  label: '소아과', sign: '👶 PEDS', emblem: '🧸' },
      { x: 10, y: 22, w: 7, h: 5, roof: { mid: P.roofMauve, dk: P.roofMauveDk, lt: '#B89BC0' },   label: '수술실 OR', sign: '🔪 OR', signColor: '#6E4F7C', redCross: true, mainEntrance: true, arch: 'flat' },
      { x: 20, y: 22, w: 4, h: 5, roof: { mid: P.roofGreen, dk: P.roofGreenDk, lt: P.roofGreenLt }, label: '약국', sign: '💊 PHARMA', emblem: '💊', arch: 'flat' },

      // Bottom row
      { x: 3,  y: 35, w: 4, h: 5, roof: { mid: P.roofBlue, dk: P.roofBlueDk, lt: P.roofBlueLt }, label: '직원기숙사', sign: '🏠 DORM', emblem: '🏠' },
      { x: 12, y: 35, w: 4, h: 5, roof: { mid: P.roofRed, dk: P.roofRedDk, lt: P.roofRedLt }, label: 'ICU', sign: 'ICU', signColor: P.red, redCross: true, arch: 'tower' },
      { x: 18, y: 35, w: 6, h: 5, roof: { mid: P.roofTeal, dk: P.roofTealDk, lt: '#85B5A8' }, label: '암병원', sign: '🎗 CANCER CTR', signColor: '#1E6FA8', arch: 'landmark', landmark: 'curved' },

      // South ward — added when campus expanded
      { x: 0,  y: 49, w: 5, h: 4, roof: { mid: P.roofRed, dk: P.roofRedDk, lt: P.roofRedLt }, label: '영상의학', sign: '🩻 X-RAY', signColor: P.red, emblem: '🩻', arch: 'flat' },
      { x: 6,  y: 48, w: 6, h: 5, roof: { mid: P.roofMauve, dk: P.roofMauveDk, lt: '#B89BC0' }, label: '산부인과', sign: '👶 MATERNITY', signColor: '#6E4F7C', emblem: '👶', mainEntrance: true, arch: 'tower' },
      { x: 13, y: 49, w: 4, h: 4, roof: { mid: P.roofRed, dk: P.roofRedDk, lt: P.roofRedLt }, label: '심장내과', sign: '♥ CARDIO', signColor: P.red, emblem: '♥', arch: 'glass' },
      { x: 18, y: 48, w: 6, h: 5, roof: { mid: P.roofGreen, dk: P.roofGreenDk, lt: P.roofGreenLt }, label: '외래 클리닉', sign: 'OPD', emblem: '🩺', mainEntrance: true, arch: 'landmark', landmark: 'horizontal', signColor: '#3E6E62' },
      // Clinic departments (south campus cluster)
      { x: 0,  y: 56, w: 4, h: 3, roof: { mid: P.roofTeal, dk: P.roofTealDk, lt: '#85B5A8' }, label: '내과', sign: '🫀 INTERNAL', signColor: '#0E7490', emblem: '🫀', arch: 'flat' },
      { x: 5,  y: 56, w: 4, h: 3, roof: { mid: P.roofBlue, dk: P.roofBlueDk, lt: P.roofBlueLt }, label: '외과', sign: '🔪 SURGERY', signColor: '#2563EB', redCross: true },
      { x: 18, y: 56, w: 4, h: 3, roof: { mid: P.roofWhite, dk: P.roofWhiteDk, lt: '#F2EDDE' }, label: '정형외과', sign: '🦴 ORTHO', signColor: '#B45309', emblem: '🦴', arch: 'flat' },
      { x: 22, y: 56, w: 4, h: 3, roof: { mid: P.roofMauve, dk: P.roofMauveDk, lt: '#B89BC0' }, label: '피부과', sign: '✨ DERM', signColor: '#DB2777', emblem: '✨' },
      ...
      <div data-screen-label="06 Explore" style={{ height: '100%', background: P.grassA, position: 'relative', overflow: 'hidden' }}>
        ...
        <div ref={scrollRef} style={{
          ...
          <div style={{ position: 'relative', width: mapW, height: mapH, imageRendering: 'pixelated' }}>
            ...
            {/* buildings */}
            {buildings.map((b, i) => b.arch === 'landmark'
              ? (b.landmark === 'horizontal' ? <MedCenterH key={i} {...b}/>
                : b.landmark === 'victorian' ? <MedCenterV key={i} {...b}/>
                : b.landmark === 'curved' ? <MedCenterC key={i} {...b}/>
                : <MedCenter key={i} {...b}/>)
              : <Building key={i} {...b}/>)}
