...
function ScreenSplash() {
  ...
  return (
    <div data-screen-label="01 Splash" style={{ height: '100%', position: 'relative', overflow: 'hidden',
      background: `linear-gradient(180deg, ${T.peach} 0%, ${T.mint} 100%)` }}>
      {/* clouds */}
      <Cloud style={{ top: 110, left: 24 }} />
      <Cloud style={{ top: 180, right: 30 }} size={1.3} />
      <Cloud style={{ top: 260, left: 80 }} size={0.8} />
      <Cloud style={{ top: 340, right: 60 }} />
      {/* sun */}
      <PixelSun style={{ position: 'absolute', top: 78, right: 30 }} />

      {/* airplane */}
      <PixelPlane style={{ position: 'absolute', top: 200, left: '50%', transform: 'translateX(-50%)' }} />

      {/* logo + tagline */}
      <div style={{ position: 'absolute', top: 420, left: 0, right: 0, textAlign: 'center', padding: '0 24px' }}>
      ...
      {/* CTA */}
      <div style={{ position: 'absolute', bottom: 70, left: 0, right: 0, padding: '0 32px' }}>
        <PixelButton full size="lg" bg={T.yellow} shadow={T.yellowShadow}>▶  처음 시작하기</PixelButton>
        <div style={{ textAlign: 'center', marginTop: 14, fontFamily: '"Galmuri11",monospace', fontSize: 11, color: T.textSoft }}>
          이미 계정이 있다면 · <span style={{ color: T.ink, textDecoration: 'underline' }}>로그인</span>
        </div>
      </div>
    </div>
  );
...
// ────────────────────────────────────────────────────────────────
// Login — Korean-optimized one-tap. A Google One-Tap dialog slides in from
// the top over a base screen whose primary CTA is a big Kakao button, with
// Naver / email as small secondary icons.
function ScreenLogin() {
  const T = window.ForinTokens;
  const Derp = window.DerpPlayer;
  return (
    <div data-screen-label="01b Login" style={{
      height: '100%', position: 'relative', overflow: 'hidden',
      background: `linear-gradient(180deg, ${T.mint} 0%, ${T.cream} 55%)`,
    }}>
      {/* soft pixel grid on lower half */}
      <div style={{ position: 'absolute', inset: 0, ...window.pixelGridBg('rgba(55,65,81,0.05)', 7) }}/>

      {/* clouds up top */}
      <Cloud style={{ top: 90, left: 26 }} size={1.1}/>
      <Cloud style={{ top: 140, right: 30 }} size={0.8}/>

      {/* hero: forin logo */}
      <div style={{ position: 'absolute', top: 150, left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 2 }}>
        <div style={{ fontFamily: '"DungGeunMo","Galmuri11",monospace', fontSize: 48, color: T.ink, letterSpacing: 3, textShadow: `4px 4px 0 ${T.yellow}` }}>forin</div>
        <div style={{ fontFamily: '"Galmuri11",monospace', fontSize: 12, color: T.textSoft, marginTop: 12, textAlign: 'center', lineHeight: 1.5 }}>
          한 번의 탭으로 시작하세요.
        </div>
      </div>
      ...
      <div style={{ position: 'absolute', bottom: 44, left: 0, right: 0, padding: '0 28px', zIndex: 3 }}>
        ...
        <div style={{ textAlign: 'center', marginTop: 18, fontFamily: '"Galmuri11",monospace', fontSize: 10, color: T.textSoft, lineHeight: 1.6 }}>
