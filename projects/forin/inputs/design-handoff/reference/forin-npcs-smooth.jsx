...
(function () {
  ...
  function SmoothSprite({
    ...
    return (
      <div style={{ position: 'relative', width, height, ...(bob ? { animation: 'forinSmoothBob 1.4s ease-in-out infinite' } : null), ...style }}>
        <style>{`@keyframes forinSmoothBob{0%,100%{transform:translateY(0)}50%{transform:translateY(-2px)}}`}</style>
  ...
  function makeRole(kind, mood) {
    return function ({ x = 0, y = 0, hair, shirt, size, expression }) {
      const h = hash(x, y, ROLE_SALT[kind]);
      const cfg = ROLES[kind](h, shirt);
      const sz = size || DEFAULT_SIZE[kind] || 40;
      const { mask, ...rest } = cfg;
      const expr = expression || (mood === 'derp' ? 'neutral' : 'happy');
      return <SmoothSprite width={sz} {...rest} hair={hair || cfg.hair} mask={!!mask} expression={expr}/>;
    };
  }
  ...
  function SmoothPlayer({ size = 40, tag = 'YOU', expression = 'happy' }) {
    return <SmoothSprite width={size} hair="#3C2A18" hairStyle="cap"
      hatTone="#FFFFFF" hatTrim="#EF4444" skin="#FCE5C8"
      shirt="#A7F3D0" shirtDk="#4FC79D" leg="#3F3D52" shoe="#1F2937" chestCross tag={tag} expression={expression}/>;
  }
  function DerpPlayer({ size = 40, tag = 'YOU', expression = 'neutral' }) {
    return <SmoothSprite width={size} hair="#3C2A18" hairStyle="cap"
      hatTone="#FFFFFF" hatTrim="#EF4444" skin="#FCE5C8"
      shirt="#A7F3D0" shirtDk="#4FC79D" leg="#3F3D52" shoe="#1F2937" chestCross tag={tag} expression={expression}/>;
  }
  ...
  function SmoothNpc({ x, y, kind = 'nurse', shirt, hair, size, mood = 'happy', expression }) {
    ...
    return (
      <div style={{ position: 'absolute', left: x * T - offX, top: y * T - offY, zIndex: 4 }}>
        <Role x={x} y={y} hair={hair} shirt={shirt} size={sz} expression={expression}/>
      </div>
    );
  }
