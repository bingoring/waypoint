...
function IGlass({ x, y, w = 1, h = 1 }) {
  return (
    <div style={{
      position: 'absolute', left: x * ITILE, top: y * ITILE,
      width: w * ITILE, height: h * ITILE,
      background: IP.glass + 'CC',
      border: `2px solid ${IP.glassFrame}`,
      boxShadow: `inset 0 0 0 1px rgba(255,255,255,0.4)`,
    }}>
      {/* horizontal frame divider */}
      <div style={{ position: 'absolute', left: 0, right: 0, top: '50%', height: 2, background: IP.glassFrame }}/>
    </div>
  );
}
...
function IReception({ x, y, w = 4, h = 2, label }) {
  return (
    <div style={{
      ...
      width: w * ITILE, height: h * ITILE + 8,
      filter: 'drop-shadow(3px 4px 0 rgba(0,0,0,.3))',
    }}>
      <svg viewBox={`0 0 ${w * 16} ${h * 16 + 8}`} width={w * ITILE} height={h * ITILE + 8} shapeRendering="crispEdges" preserveAspectRatio="none">
        {/* DESK TOP (foreshortened trapezoid) */}
        <path d={`M 3 1 L ${w*16-3} 1 L ${w*16-1} ${h*16-12} L 1 ${h*16-12} Z`}
              fill="#A88862" stroke={IP.ink} strokeWidth=".5"/>
        {/* top back-edge highlight */}
        <path d={`M 4 1.5 L ${w*16-4} 1.5 L ${w*16-5} 3 L 5 3 Z`} fill="#C49D6C"/>
        {/* wood grain */}
        <line x1="3" y1="8" x2={w*16-3} y2="8" stroke={IP.ink} strokeWidth=".25" opacity=".25"/>
        <line x1="2" y1={h*16-15} x2={w*16-2} y2={h*16-15} stroke={IP.ink} strokeWidth=".25" opacity=".25"/>

        {/* DESK FRONT APRON (thickness band under top) */}
        <rect x="1" y={h*16-12} width={w*16-2} height="4" fill="#7C5A38" stroke={IP.ink} strokeWidth=".4"/>
        <rect x="2" y={h*16-11.5} width={w*16-4} height=".7" fill="#956B40"/>

        {/* BACK-LEFT + BACK-RIGHT leg stubs (peek behind top) */}
        <rect x="3" y="0" width="2" height="2" fill="#5C3A1A" stroke={IP.ink} strokeWidth=".3"/>
        <rect x={w*16-5} y="0" width="2" height="2" fill="#5C3A1A" stroke={IP.ink} strokeWidth=".3"/>

        {/* FRONT-LEFT + FRONT-RIGHT legs (full height) */}
        <rect x="2" y={h*16-8} width="3" height="8" fill="#5C3A1A" stroke={IP.ink} strokeWidth=".4"/>
        <rect x="2.5" y={h*16-7.5} width="1" height="7" fill="#7C4F2C"/>
        <rect x={w*16-5} y={h*16-8} width="3" height="8" fill="#5C3A1A" stroke={IP.ink} strokeWidth=".4"/>
        <rect x={w*16-4.5} y={h*16-7.5} width="1" height="7" fill="#7C4F2C"/>

        {/* MONITOR on the desk top */}
        <rect x={w*8-6} y="3" width="12" height="8" fill="#1F2937" stroke={IP.ink} strokeWidth=".4"/>
        <rect x={w*8-5} y="4" width="10" height="6" fill="#0F1A24"/>
        <rect x={w*8-4} y="5" width="8" height=".7" fill="#10B981"/>
        <rect x={w*8-4} y="6.5" width="8" height=".7" fill="#22D3EE"/>
        <rect x={w*8-4} y="8" width="6" height=".7" fill="#FACC15"/>
        {/* monitor stand */}
        <rect x={w*8-1} y="11" width="2" height="1.5" fill="#4B5563"/>
        <ellipse cx={w*8} cy="13" rx="3" ry=".7" fill="#374151"/>

        {/* CLIPBOARD (left) */}
        <rect x="6" y="4" width="7" height="9" fill="#FEF3C7" stroke={IP.ink} strokeWidth=".3"/>
        <rect x="8.5" y="3" width="2" height="1.5" fill="#9CA3AF" stroke={IP.ink} strokeWidth=".25"/>
        <line x1="7" y1="6.5" x2="12" y2="6.5" stroke={IP.ink} strokeWidth=".2"/>
        <line x1="7" y1="8" x2="12" y2="8" stroke={IP.ink} strokeWidth=".2"/>
        <line x1="7" y1="9.5" x2="12" y2="9.5" stroke={IP.ink} strokeWidth=".2"/>

        {/* COFFEE MUG (right) */}
        {w >= 3 && (
          <g>
            <ellipse cx={w*16-9} cy="5" rx="2.5" ry="1" fill="#1F2937" stroke={IP.ink} strokeWidth=".3"/>
            <path d={`M ${w*16-11.5} 5 L ${w*16-11.5} 9 Q ${w*16-9} 10 ${w*16-6.5} 9 L ${w*16-6.5} 5`} fill="#FFFFFF" stroke={IP.ink} strokeWidth=".4"/>
            <ellipse cx={w*16-9} cy="5" rx="2" ry=".7" fill="#6B2C0E"/>
            <path d={`M ${w*16-6.5} 6 Q ${w*16-5} 7 ${w*16-6.5} 8.5`} fill="none" stroke={IP.ink} strokeWidth=".4"/>
          </g>
        )}
      </svg>
...
function IMonitor({ x, y, beep }) {
  return (
    <div style={{ position: 'absolute', left: x * ITILE, top: y * ITILE - 8, width: ITILE, height: ITILE * 2, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.24))' }}>
      {/* wheeled base */}
      ...
      {/* top face of monitor box */}
      <div style={{ position: 'absolute', left: 4, right: -1, top: -2, height: 3, background: IP.metal, border: `1.5px solid ${IP.ink}` }}/>
      {/* right side face */}
...
function IIV({ x, y }) {
  return (
    <div style={{ position: 'absolute', left: x * ITILE, top: y * ITILE - 22, width: ITILE, height: ITILE * 2.6, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.28))' }}>
      ...
      <svg viewBox="0 0 16 18" width={ITILE} height={ITILE * 1.1} shapeRendering="crispEdges" style={{ position: 'absolute', top: 0, left: 0 }}>
        {/* pole top stub */}
        <rect x="7" y="6" width="2" height="6" fill={IP.metal} stroke={IP.ink} strokeWidth=".5"/>
        {/* curl over top — left arm goes up, top connects, right side comes back down (the "hook" that opens upward) */}
        <rect x="5" y="4" width="2" height="3" fill={IP.metal} stroke={IP.ink} strokeWidth=".5"/>
        <rect x="3" y="2" width="2" height="3" fill={IP.metal} stroke={IP.ink} strokeWidth=".5"/>
        <rect x="3" y="0" width="6" height="2" fill={IP.metal} stroke={IP.ink} strokeWidth=".5"/>
        ...
        <rect x="4" y="0.5" width="3" height="0.5" fill="#E2E5EB"/>
      </svg>
...
function IChair({ x, y, color = '#FED7AA', facing = 'down' }) {
  ...
  return (
    <div style={{ position: 'absolute', left: x * ITILE, top: y * ITILE - 4, width: ITILE, height: ITILE + 6, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.26))' }}>
      <svg viewBox="0 0 16 22" width={ITILE} height={ITILE + 6} shapeRendering="crispEdges">
        {/* backrest TOP edge */}
        <path d="M 3 1 L 13 1 L 14 2 L 2 2 Z" fill={lite} stroke={IP.ink} strokeWidth=".4"/>
        {/* backrest FRONT */}
        <rect x="2" y="2" width="12" height="8" fill={color} stroke={IP.ink} strokeWidth=".4"/>
        <rect x="3" y="3" width="10" height="1.5" fill={dark} opacity=".55"/>
        <rect x="3" y="7" width="10" height="1.5" fill={dark} opacity=".55"/>
        {/* seat TOP face (perspective) */}
        <path d="M 1 10 L 15 10 L 14 13 L 2 13 Z" fill={lite} stroke={IP.ink} strokeWidth=".4"/>
        <path d="M 1.5 10.5 L 14.5 10.5 L 14 11.8 L 2 11.8 Z" fill={color}/>
        {/* seat FRONT (thickness) */}
        <rect x="1" y="13" width="14" height="2" fill={dark} stroke={IP.ink} strokeWidth=".4"/>
        {/* 4 legs */}
        <rect x="1.5" y="15" width="2" height="6" fill="#5C3A1A" stroke={IP.ink} strokeWidth=".3"/>
        <rect x="2" y="15.5" width=".7" height="5" fill="#A88862"/>
        <rect x="12.5" y="15" width="2" height="6" fill="#5C3A1A" stroke={IP.ink} strokeWidth=".3"/>
        <rect x="13" y="15.5" width=".7" height="5" fill="#A88862"/>
      </svg>
    </div>
  );
}
...
function IPlant({ x, y }) {
  return (
    <div style={{ position: 'absolute', left: x * ITILE, top: y * ITILE - 6, width: ITILE, height: ITILE * 1.3, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.22))' }}>
      <svg viewBox="0 0 16 22" width={ITILE} height={ITILE * 1.3} shapeRendering="crispEdges">
        ...
        {/* pot rim (top face) */}
        <rect x="3" y="12" width="10" height="2" fill="#A0531C" stroke={IP.ink} strokeWidth=".5"/>
        {/* pot body */}
        <path d="M 4 14 L 12 14 L 11 21 L 5 21 Z" fill="#7C3F1A" stroke={IP.ink} strokeWidth=".7"/>
        {/* pot front shading */}
        <path d="M 5 17 L 11 17 L 11 21 L 5 21 Z" fill="#5C2A0D" opacity="0.4"/>
        {/* pot rim highlight */}
...
function IPlayer({ x, y }) {
  return (
    <div style={{
      ...
      {window.DerpPlayer
        ? <window.DerpPlayer size={28} tag="YOU"/>
        : window.ChibiPlayer
...
function InteriorScreen({
  ...
  topRight, missionText, missionUrgent,
}) {
  ...
  return (
    <div data-screen-label={label} style={{ height: '100%', background: '#1F2937', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', bottom: 100, left: 12, right: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <window.PixelDPad size={72} onMove={(d) => {
...
Object.assign(window, {
  ...
  IBed, IReception, IMonitor, IIV, ICurtain, IChair, ICabinet, IPlant,
