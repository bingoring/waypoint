...
function ScreenDialogueResult() {
  ...
  // One-time initial burst — anchored to the sticker centre.
  React.useEffect(() => {
    const s = stickerRef.current;
    if (!s) return;
    const sr = s.getBoundingClientRect();
    const p = toLocal(sr.left + sr.width / 2, sr.top + sr.height / 2);
    spawnBurst(p.x, p.y);
...
function ConfettiBurst({ x, y }) {
  ...
  const pieces = React.useMemo(() => Array.from({ length: 48 }, () => {
