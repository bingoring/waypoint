...
All are SVG primitives drawn at a **high-angle top-down "2.5D"** view (RPG-Maker /
쯔꾸르 style): the camera looks down-and-forward (~70° from the floor) so every
free-standing object shows a **dominant flat TOP face + a shorter FRONT band**,
joined as **one continuous silhouette** (the front's top edge continues straight
down out of the top face — no rounded fold, no 90° notch). Screens, dials,
keypads, drawers and handles live on the **viewer-facing FRONT** (a nurse reads
them head-on), never lying flat on the top; only things physically resting on the
unit (lids, trays, bottles, cylinders, bellows) sit on the top face. Wall-mounted
items (view boxes, shelves, wall lamps, suction regulators), stand/pole units (IV
poles, monitors on stands, magnifier arms) and small props (crutches, clipboards)
keep their natural front-on read. **Shadows are right-only** —
`drop-shadow(3px 0px 0 rgba(0,0,0,…))` — so objects sit on the floor instead of
floating. Port to `react-native-svg` by copying the `<rect>/<path>/<ellipse>`
data. They take `{x, y}` tile coords (+ a few extras like `occupied`, `beep`,
`w`, `color`).
