...
| xp / gem | 파란 다각 보석 | 별 도형 폐기, 보석으로 통일 (⭐🌟★ 매핑) |
...
| tower | 유리 고층 타워 + 적십자 간판 | 본관 |
| women | 핑크 라운드 건물 + 하트 간판 | 여성소아 센터 (🤰 매핑) |
| ribbon | 핑크 어웨어니스 리본 | 암센터 (🎗 매핑) |
| clinic | 낮고 넓은 건물 + 파란 줄무늬 어닝 + 청록 십자 | 외래·진단 지원동 (🏬 매핑) |
| admin | 회색 창고 + 셔터 하역장 + 박스 | 행정·백스테이지 윙 |
