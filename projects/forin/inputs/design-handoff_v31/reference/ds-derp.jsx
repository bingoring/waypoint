...
function ScreenDSDerp() {
  ...
  const HAIR_STYLES = ['short','bob','long','pigtails','bun','curly','bald','mohawk','ponytail','balding'];
  ...
  return (
    ...
    >
      ...
      {/* ── Directions + side hairstyles ── */}
      <DSSection title="◆ 4방향 (front / back / side)" hint="SmoothSprite dir prop — down / up / left / right">
        <DSGrid minItem={110} gap={12}>
          {[['down','정면'],['up','뒷모습'],['left','왼쪽'],['right','오른쪽']].map(([d, ko]) => (
            <div key={d} style={{
              background: '#fff', border: `2px solid ${t.ink}`, boxShadow: `3px 3px 0 0 ${t.ink}`,
              padding: '12px 4px 4px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
            }}>
              <F.DerpNurse seed={7} size={56} dir={d}/>
              <div style={{ fontFamily: '"DungGeunMo",monospace', fontSize: 9, color: t.ink }}>{d}</div>
              <div style={{ fontFamily: '"Galmuri11",monospace', fontSize: 8, color: t.ink, opacity: 0.6 }}>{ko}</div>
            </div>
          ))}
        </DSGrid>
      </DSSection>

      <DSSection title="◆ 머리스타일 × 측면 프로필" hint="머리스타일별 정면 · 측면 · 후면 실루엣 (hairFront/hairSide/backHead)">
        <DSGrid minItem={132} gap={12}>
          {HAIR_STYLES.map(hs => (
            <div key={hs} style={{
              background: '#fff', border: `2px solid ${t.ink}`, boxShadow: `3px 3px 0 0 ${t.ink}`,
              padding: '12px 4px 4px', display: 'flex', flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'center', gap: 4,
            }}>
              <F.SmoothSprite width={44} hair="#5C3A1A" hairStyle={hs} skin="#F8D7B2" shirt="#A7F3D0" shirtDk="#4FC79D" leg="#475569" expression="neutral" dir="down"/>
              <F.SmoothSprite width={44} hair="#5C3A1A" hairStyle={hs} skin="#F8D7B2" shirt="#A7F3D0" shirtDk="#4FC79D" leg="#475569" expression="neutral" dir="right"/>
              <F.SmoothSprite width={44} hair="#5C3A1A" hairStyle={hs} skin="#F8D7B2" shirt="#A7F3D0" shirtDk="#4FC79D" leg="#475569" expression="neutral" dir="up"/>
            </div>
          ))}
        </DSGrid>
        <div style={{ marginTop: 8, fontFamily: '"Galmuri11",monospace', fontSize: 11, color: t.ink, opacity: 0.7 }}>
          각 카드: 왼쪽부터 정면 / 측면 / 후면. 양갈래는 측면에서 뒤로 묶음 하나, 긴 머리는 뒤로 늘어뜨림, 쪽머리는 뒤통수에 쪽, 포니테일은 뒤로 낮게 묶어 길게 늘어뜨림.
        </div>
      </DSSection>

      {/* ── Hair styles ── */}
      <DSSection title="◆ Hair styles · 10 종 + 모자 2" hint="SmoothSprite hairStyle prop">
        <DSGrid minItem={110} gap={12}>
          {HAIR_STYLES.map(hs => (
            <div key={hs} style={{
              background: '#fff', border: `2px solid ${t.ink}`, boxShadow: `3px 3px 0 0 ${t.ink}`,
