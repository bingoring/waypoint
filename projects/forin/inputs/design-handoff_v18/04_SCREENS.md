...
| `reference/forin Flow.html` | ④ Briefing, ⑤ Dialogue, ⑰ Growth | `app-flow.jsx` |
...
`ScreenCampusCurriculum` / `ScreenCampusBuildings` / `ScreenCampusToday`.
Shared chrome: a fixed 116px header (title + Lv chip + streak, then a
3-way segmented tab strip; header and tabs use `box-sizing: border-box` so the
tab underline meets the buttons), an absolutely-bounded scroll body, the
`ExploreDock` card, and `ForinBottomNav active="campus"`.
...
### C. 오늘의 상황 — free choice
- Independent of the curriculum: pick any situation and solve it now. Cards show
  a tag (`긴급` red card / `진행중` / `신궜`), dept·floor, title, `Lv` + est.
  minutes, and a `시작` button. A `⇅ 정렬` control sits in the header row.
- Footer note: refreshed daily at 06:00. Data: `TODAY`.
