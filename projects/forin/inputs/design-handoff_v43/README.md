...
> **디자인 방향 확정 (2026-08):** forin은 기존 픽셀아트 디자인으로 계속 개발한다. `forin Modern - *` 페이지들은 모던 리디자인 탐색 시안으로 보관용. 전역 아이콘은 FIcon 시스템(`forin-pixel-icons.jsx`, 02_COMPONENTS.md 참조)을 사용하며 이모지는 자동 치환된다.

> **2026-09 · Notebook 리디자인 라인:** 근무 수첩 그림체의 개편 시안이 `07_NOTEBOOK_REDESIGN.md`에 문서화되어 있다 (forin Notebook - *.html / forin-notebook-*.jsx). 기존 픽셀 라인과 병행 비교용. 탭 워딩: 커리어→일터, 상황판→라운지.

> **2026-09 · 여정 지도 리소스:** 일터 탭 여정 지도(Journey)의 컴포넌트·SVG 노드·데이터 모델·설계 원칙은 `08_JOURNEY_RESOURCES.md` 참조.
