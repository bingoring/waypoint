...
.nb-hinge-in{position:absolute;top:0;height:100%;transform-origin:left center;transform-style:preserve-3d;animation:nb-hinge-in var(--dur,1.1s) cubic-bezier(.3,.6,.3,1) both;animation-delay:var(--d)}
...
`;
  ...
  function CurlIn({ children, bg = '#2E4636', dur }) {
    const W = 402, N = 24, w = W / N;
    const angles = Array.from({ length: N }, (_, i) => -(4 + 8 * (i / (N - 1))));
    const delays = Array.from({ length: N }, (_, i) => i * 0.0045); // 책등부터 덮임
    const build = (i) => (
      <div className="nb-hinge-in" style={{ left: i === 0 ? 0 : w, width: W - i * w, '--a': angles[i].toFixed(2) + 'deg', '--d': delays[i].toFixed(3) + 's', ...(dur ? { '--dur': dur } : {}) }}>
      ...
      <div className="nb-page" key="p1">
        ...
        <div style={{ padding: '2px 24px 0 34px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 17 }}>
            <span style={{ fontFamily: HW, fontSize: 16, color: c.soft, whiteSpace: 'nowrap' }}>여권에 쓸 이름</span>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="예) Minji" maxLength={14} style={{ flex: 1, minWidth: 0, background: 'transparent', border: 'none', outline: 'none', borderBottom: '2px solid rgba(62,54,43,.5)', fontFamily: HW, fontSize: 21, color: c.ink, padding: '2px 4px', textAlign: 'center' }}/>
          </div>
        </div>
        <div style={{ position: 'absolute', left: 34, right: 24, bottom: 34 }}>
          {dots(1)}
      ...
      <div className="nb-page" key="p6">
        ...
        <div style={{ padding: '18px 24px 0 34px' }}>
          <NbPaper rot={-0.8} tape tapeLeft={130} style={{ padding: '12px 14px', display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontFamily: MONO, fontSize: 13, fontWeight: 700, color: c.ink }}>RN · {name.trim() || 'Learner'}</div>
  ...
  Object.assign(window, { OnbFlow, NbOnbFX: { CurlOut, CurlIn, Gutter, GLogo } });
})();
