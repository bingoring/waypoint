...
function DCArtboardFrame({ sectionId, artboard, label, order, onRename, onReorder, onFocus, onDelete }) {
  const { id: rawId, label: rawLabel, width = 260, height = 480, children, eager = false, style = {} } = artboard.props;
  ...
  return (
    <div ref={ref} data-dc-slot={id} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      <div ref={cardRef} className="dc-card"
        ...
        {eager ? (children || <DCEmptyLabel id={id}/>) : <DCLazyMount cardRef={cardRef} placeholderId={id}>{children}</DCLazyMount>}
      </div>
    </div>
  );
}

// DCLazyMount — only mounts heavy artboard children once the card scrolls
// near the viewport (and keeps them mounted after). Without this, a canvas
// with many heavy screens force-mounts them ALL on first paint, which can
// hang the page. Uses IntersectionObserver against the scroll viewport.
// Module-level mount scheduler — drains queued artboard mounts a couple per
// tick so the page never mounts many heavy screens in one synchronous paint.
let _dcQ = [];
...
function _dcDrain() {
  ...
  _dcRunning = true;
  const tick = () => {
    const item = _dcQ.shift();           // one heavy mount per tick (safest)
    if (item) { try { item(); } catch (e) {} }
    if (_dcQ.length) setTimeout(tick, 120);
    else _dcRunning = false;
  };
  setTimeout(tick, 0);
}
function dcRequestMount(fn) { _dcQ.push(fn); _dcDrain(); }

// DCLazyMount — mounts an artboard's heavy children ONLY when the card is
// actually near the viewport. The canvas applies its pan/zoom as an
// imperative CSS transform (no scroll events, and IntersectionObserver does
// not fire reliably through that transform), so we poll the card's own
// bounding rect on a cheap timer until it's visible, then enqueue the mount
// and stop polling. Off-screen artboards never mount — that's what keeps a
// dense canvas from hanging when you pan.
function DCLazyMount({ cardRef, placeholderId, children }) {
  ...
  React.useEffect(() => {
    ...
    const near = () => {
      ...
      const mx = 400, my = 400; // pre-mount margin around viewport
      return r.bottom > -my && r.top < window.innerHeight + my
          && r.right > -mx && r.left < window.innerWidth + mx;
    };
    const check = () => {
      if (done) return;
      if (near()) {
        ...
        dcRequestMount(() => setShown(true));
      }
    };
    // 백그라운드 탭에서 setInterval이 ~1Hz로 스로틀되는 환경 보완:
    // 팬/줌 입력 이벤트에서도 즉시 가시성을 재확인한다.
    ...
    check(); // immediate first check (first screens mount right away)
    return () => { done = true; clearInterval(id); window.removeEventListener('pointermove', check); window.removeEventListener('wheel', check); };
  }, [shown]);

  if (shown) return children || <DCEmptyLabel id={placeholderId}/>;
  return (
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#cbd2da', fontSize: 12, fontFamily: DC.font, background: '#fafbfc' }}>
      <span style={{ opacity: .7 }}>로딩 대기 · {placeholderId}</span>
    </div>
  );
}
function DCEmptyLabel({ id }) {
  return <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#bbb', fontSize: 13, fontFamily: DC.font }}>{id}</div>;
}
