...
(function () {
  ...
  function MatchFmt({ q }) {
    ...
    return (<>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 30px 1fr', gap: 8, alignItems: 'stretch' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {q.pairs.map((p, i) => <div key={i}>{chip(p.en, p.sub, 'L', p.state, p.picked)}</div>)}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {q.pairs.map((p, i) => {
            const done = p.state === 'correct' || p.state === 'wrong';
            const color = p.state === 'correct' ? t.mintShadow : p.state === 'wrong' ? '#EF4444' : (p.picked ? t.yellowShadow : t.ink + '22');
            return <div key={i} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ width: '100%', height: 3, background: color, backgroundImage: (p.state === 'wrong' || p.picked) ? `repeating-linear-gradient(90deg, ${color} 0 4px, transparent 4px 7px)` : 'none' }}/>
            </div>;
          })}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
  ...
  function MonitorFmt({ q }) {
    ...
    return (<>
      ...
      <div style={{ background: '#0F1A24', border: `4px solid ${t.ink}`, boxShadow: `4px 4px 0 0 ${t.ink}`, padding: 10, position: 'relative' }}>
        ...
        {q.wave && <div style={{ marginTop: 8, height: 24, background: '#000', border: `1.5px solid ${t.ink}`, overflow: 'hidden' }}><svg viewBox="0 0 200 24" width="200" height="24" preserveAspectRatio="none" style={{ width: '100%', display: 'block' }}><polyline points={q.wave} fill="none" stroke="#22D3EE" strokeWidth="1.5"/></svg></div>}
      </div>
      <div style={{ marginTop: 14 }}>
