    ...
    <div data-screen-label={hintOn ? '07b Dialogue · Hint ON' : '07a Dialogue · Free'} style={{ height: '100%', background: '#1F2937', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', left: 16, top: 130, zIndex: 3 }}>
        <PortraitFrame name="Mrs. Hopkins" side="L" status="Pain 7/10">
          ...
          <span style={{ position: 'absolute', top: 6, right: -10, fontSize: 18, zIndex: 3 }}>💧</span>
        </PortraitFrame>
      </div>
      ...
      <div style={{ position: 'absolute', left: 14, right: 14, bottom: 24, zIndex: 6 }}>
        ...
        <div style={{ background: T.cream, border: `3px solid ${T.ink}`, padding: '14px 16px 12px', boxShadow: `4px 4px 0 0 ${T.ink}`, position: 'relative' }}>
          ...
          {/* blinking next-arrow */}
          <div style={{ position: 'absolute', right: 10, bottom: -8, width: 20, height: 20, background: T.yellow, border: `2px solid ${T.ink}`, fontFamily: '"DungGeunMo",monospace', fontSize: 12, lineHeight: '16px', textAlign: 'center', boxShadow: `2px 2px 0 0 ${T.ink}` }}>▼</div>
        </div>
          ...
          <div style={{ marginTop: 14 }}>
            <div style={{ background: '#fff', border: `3px solid ${T.ink}`, boxShadow: `3px 3px 0 0 ${T.ink}`, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 2, height: 20 }}>
                {[3, 7, 12, 8, 4].map((h, i) => (
                  <div key={i} style={{ width: 3, height: h, background: T.ink + '55' }}/>
                ))}
              </div>
            </div>
          </div>
...
function ScreenDialogueResult() {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, top: 56, bottom: 0, overflowY: 'auto', padding: '14px 22px 28px', textAlign: 'center', zIndex: 2 }}>
        ...
        <div ref={stickerRef} style={{ margin: '18px auto', width: 130, height: 130, background: T.yellow, border: `4px solid ${T.ink}`, boxShadow: `5px 5px 0 0 ${T.ink}`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', transform: 'rotate(-4deg)' }}>
          <div style={{ width: 100, height: 100, borderRadius: '50%', border: `3px dashed ${T.ink}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
            <window.FIcon name="thumb" size={32}/>
        ...
        {/* 직접 말하기 기록 — 이번 시나리오에서 소리 내어 답한 문장 모아보기 */}
        <div style={{ marginTop: 14, background: '#fff', border: `3px solid ${T.ink}`, boxShadow: `4px 4px 0 0 ${T.ink}`, padding: 14, textAlign: 'left' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
            <span style={{ fontSize: 14 }}>🎙</span>
            <div style={{ flex: 1, fontFamily: '"DungGeunMo",monospace', fontSize: 12, color: T.textSoft }}>직접 말한 문장 3</div>
            <div style={{ background: T.mint, border: `2px solid ${T.ink}`, padding: '2px 7px', fontFamily: '"DungGeunMo",monospace', fontSize: 10, color: T.ink }}>평균 81</div>
          </div>
          {[
            { s: 'Where does it hurt the most?', ko: '어디가 가장 아프세요?', sc: 92, tone: T.mint },
            { s: "I'm giving you acetaminophen 650 milligrams.", ko: '아세트아미노펜 650mg 투여합니다', sc: 74, tone: T.yellow },
            { s: 'Please bear with me for a moment.', ko: '잠시만 기다려 주세요', sc: 78, tone: T.yellow },
          ].map((r, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 0', borderBottom: i < 2 ? `1.5px dotted ${T.ink}22` : 'none' }}>
              <div style={{ width: 30, flexShrink: 0, background: r.tone, border: `2px solid ${T.ink}`, padding: '4px 0', textAlign: 'center', fontFamily: '"DungGeunMo",monospace', fontSize: 11, color: T.ink }}>{r.sc}</div>
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ fontFamily: '"Galmuri11","Pretendard",sans-serif', fontSize: 12, color: T.ink, lineHeight: 1.3 }}>{r.s}</div>
                <div style={{ fontFamily: '"Galmuri11",monospace', fontSize: 9.5, color: T.textSoft, marginTop: 2 }}>{r.ko}</div>
              </div>
              <div style={{ background: T.blue, border: `2px solid ${T.ink}`, padding: '5px 7px', fontFamily: '"DungGeunMo",monospace', fontSize: 10, color: T.ink, flexShrink: 0 }}>🔊</div>
            </div>
          ))}
          <div style={{ marginTop: 11, background: T.lilac, border: `2.5px solid ${T.ink}`, boxShadow: `2.5px 2.5px 0 0 ${T.ink}`, padding: '9px 0', textAlign: 'center', fontFamily: '"DungGeunMo",monospace', fontSize: 11.5, color: T.ink }}>
            🎯 낮은 점수 2문장 다시 연습하기
          </div>
        </div>
