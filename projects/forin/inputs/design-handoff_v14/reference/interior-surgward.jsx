...
function ScreenInteriorSurgWard() {
  ...
  const rooms = [
    ...
    { id: 'major',    name: '대수술 중증실', sub: 'NG·Hemovac·SCD', icon: '🚨', color: '#FCA5A5', x: 13, y: 44, questCount: 1 },
  ];
    ...
    <InteriorScreen
      ...
      render={() => (
        <>
          ...
          {/* ═══ ROOM4 / MAJOR DIVIDER (y35) ═══ */}
          <IWall x={1}  y={35} w={9} h={1}/>
          <Th    x={10} y={35} w={3} h={1} label="→ 중증실"/>
          ...
          <BayLabel x={1} y={36} text="MAJOR POST-OP · 대장암/위암 절제" highlight/>
          {/* major-resection patient with NG tube + multiple drains */}
          <IBed x={4} y={38} variant="ward" occupied label="POST-OP MAJOR"/>
          <window.NGSuction x={1} y={38}/>
          <IMonitor x={8} y={38} beep/>
          <IIV x={9} y={38}/>
          <window.PCAPump x={11} y={38}/>
          {/* Hemovac drains pinned to the sheet, close beside the bed */}
          <window.Hemovac x={4} y={40.5}/>
          <window.Hemovac x={6} y={40.5}/>
          {/* SCD/DVT prophylaxis on the legs */}
          <window.SCDDevice x={13} y={42}/>
          {/* nurse checking drain patency + IV rate */}
          <INpc x={8} y={41} kind="nurse" hair="#3C2A18" shirt="#A5D8E8"/>
          <IHotspot x={5} y={38} kind="quest" label="배액관 개통성 확인"/>
          <window.SuctionUnit x={16} y={39}/>
          <IChair x={20} y={40} color="#FED7AA" facing="left"/>
          <window.Sofa x={20} y={45} w={3} color="#9CB4C8"/>
          <INpc x={21} y={41} kind="parent" hair="#3C2A18"/>
          <IPlant x={25} y={48}/>
        </>
      )}
    />
