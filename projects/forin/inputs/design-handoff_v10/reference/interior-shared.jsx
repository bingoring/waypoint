  ...
  return (
    <div style={{
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width={w * ITILE} height={H} shapeRendering="crispEdges" preserveAspectRatio="none">
        {/* DESK TOP (foreshortened trapezoid) — white clinical laminate */}
        <path d={`M 3 1 L ${W-3} 1 L ${W-1} ${topB} L 1 ${topB} Z`}
              fill="#ECEAE1" stroke={IP.ink} strokeWidth=".5"/>
        {/* top back-edge highlight */}
        <path d={`M 4 1.5 L ${W-4} 1.5 L ${W-5} 3.4 L 5 3.4 Z`} fill="#FAF8F2"/>
        ...
        <rect x="1" y={topB} width={W-2} height={apronH} fill="#C6C2B6" stroke={IP.ink} strokeWidth=".4"/>
        <rect x="2" y={topB+0.5} width={W-4} height=".7" fill="#DAD6CA"/>
...
function NurseDeskI({ x, y, w = 8, h = 2, label }) {
  ...
  return (
    <div style={{ position: 'absolute', left: x * 16, top: y * 16 - R, width: W, height: TH, filter: 'drop-shadow(3px 4px 0 rgba(0,0,0,.3))' }}>
...
function IChair({ x, y, color = '#FED7AA', facing = 'down' }) {
  ...
  return (
    <div style={{ position: 'absolute', left: x * ITILE, top: y * ITILE - ITILE * 0.5, width: ITILE, height: ITILE * 1.6, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.26))' }}>
