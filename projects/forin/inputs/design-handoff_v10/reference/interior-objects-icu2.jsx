...
(function () {
  ...
  function CRRTMachine({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 14, width: T * 2, height: T * 3.4, filter: 'drop-shadow(3px 4px 0 rgba(0,0,0,.3))' }}>
        <svg viewBox="0 0 32 54" width={T * 2} height={T * 3.4} shapeRendering="crispEdges">
          {/* top bar with 4 dialysate bags */}
          <rect x="2" y="0" width="28" height="1.5" fill="#9CA3AF"/>
          {[4,11,18,25].map((bx,i) => (
            <g key={i}>
              <rect x={bx} y="1.5" width="5" height="8" fill={i%2? '#D7F0E0':'#FCE7C8'} stroke={C} strokeWidth=".4"/>
              <rect x={bx+0.6} y="2.5" width="1.4" height="6" fill={i%2? '#A7E0BE':'#F4D29A'}/>
            </g>
          ))}
          {/* screen */}
          <path d="M3 11 L29 11 L30 13 L2 13 Z" fill="#475569" stroke={C} strokeWidth=".4"/>
          <rect x="2" y="13" width="28" height="11" fill="#5B6776" stroke={C} strokeWidth=".5"/>
          ...
          {/* blood pump (rotating circle) */}
          <circle cx="24" cy="18.5" r="4" fill="#1F2937" stroke={C} strokeWidth=".5"/>
          <circle cx="24" cy="18.5" r="1.4" fill="#DC2626"/>
          <rect x="23.4" y="14.8" width="1.2" height="3.7" fill="#7F1D1D"/>
          {/* filter column with blood lines */}
          <rect x="3" y="25" width="3" height="14" fill="#FCA5A5" stroke={C} strokeWidth=".4"/>
          ...
          {/* body + wheels */}
          <rect x="2" y="25" width="28" height="20" fill="#9CA3AF" stroke={C} strokeWidth=".5" opacity=".0"/>
          <rect x="8" y="25" width="22" height="14" fill="#9CA3AF" stroke={C} strokeWidth=".5"/>
          <rect x="9" y="26.5" width="20" height="3" fill="#fff" stroke={C} strokeWidth=".3"/>
          <rect x="9" y="31" width="20" height="3" fill="#fff" stroke={C} strokeWidth=".3"/>
  ...
  function IVPumpTower({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 22, width: T, height: T * 3.4, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.3))' }}>
  ...
  function EVDStand({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 20, width: T, height: T * 3, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.28))' }}>
  ...
  function ICPMonitor({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 1, top: y * T - 6, width: T - 2, height: T * 1.6, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.26))' }}>
  ...
  function TTMUnit({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 4, width: T * 1.6, height: T * 2.4, filter: 'drop-shadow(3px 4px 0 rgba(0,0,0,.28))' }}>
        <svg viewBox="0 0 26 38" width={T * 1.6} height={T * 2.4} shapeRendering="crispEdges">
          {/* coiling hoses to the cooling pad (left) */}
          <path d="M2 6 Q-2 10 4 13 Q9 16 3 19" fill="none" stroke="#7FB8D8" strokeWidth="2.5"/>
          {/* top face */}
          <path d="M3 4 L20 4 L21 6 L2 6 Z" fill="#2C5E7C" stroke={C} strokeWidth=".4"/>
          {/* body (cool teal) */}
          <rect x="2" y="6" width="19" height="15" fill="#3E7CA0" stroke={C} strokeWidth=".5"/>
          {/* screen with target temp */}
          <rect x="4" y="8" width="11" height="7" fill="#0F1A24" stroke={C} strokeWidth=".4"/>
          <text x="9.5" y="12.4" fontSize="3.4" fill="#7DD3FC" textAnchor="middle" fontFamily="monospace">34°</text>
          <rect x="4" y="15.5" width="11" height="1" fill="#22D3EE"/>
          {/* water reservoir window (cool blue) */}
          <rect x="16" y="8" width="4" height="10" fill="#BFE3EE" stroke={C} strokeWidth=".4"/>
          <rect x="16" y="13" width="4" height="5" fill="#9FD0E4"/>
          {/* wheeled base */}
          <rect x="2" y="21" width="19" height="9" fill="#6B7280" stroke={C} strokeWidth=".5"/>
          <rect x="3.5" y="22.5" width="16" height="3" fill="#fff" stroke={C} strokeWidth=".3"/>
  ...
  function FoleyBag({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 4, top: y * T + 2, width: T - 8, height: T * 1.2, filter: 'drop-shadow(1px 1.5px 0 rgba(0,0,0,.22))' }}>
  ...
  function Intercom({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 3, top: y * T + 1, width: T - 6, height: T * 1.3, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.24))' }}>
  ...
  function GownBox({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 2, top: y * T, width: T - 4, height: T * 1.4, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.24))' }}>
