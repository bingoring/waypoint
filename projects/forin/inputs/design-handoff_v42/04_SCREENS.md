...
| Screen | State |
|---|---|
| `ScreenHome` | Default — LiveWard(DAY 무드) + 오늘의 호출(PagingCall) 대기 + 오늘의 한 가지 |
| `ScreenHomeNight` | NIGHT 무드 — 밤 하늘 별 + 어두운 병동 + 호출 응답 완료 배지 |
...
### 홈 전용 라이브 모듈 (2026-08)
- **LiveWard(라이브 병원 무드)** — 홈 최상단의 실시간 픽셀 미니 병동. 시간대별 3무드: DAY(하늘색 하늘+해, 분주, 회진 대화 출현↑) / EVENING(노을, 인계 준비, SBAR 출현↑) / NIGHT(별 깜빡임+어둠 오버레이, 야간 호출 출현↑). 침대 3개 + 창문 + 깜빡이는 바이탈 모니터 + 순회하는 간호사 스프라이트(CSS 애니메이션 fw-walk/fw-blink/fw-pulse). 하단 무드 바가 학습 확률 변화를 설명.
- **PagingCall(오늘의 호출)** — 하루 1회 페이저 스타일 긴급 호출 카드. 네이비 페이저 프레임 + 빨간 헤더(깜빡이는 라이트 + CODE PAGE) + 흔들림 애니메이션 + 제한시간 바(⏱ 남은 시간, 오늘 놓치면 소멸) + [지금 응답 → 단기 시나리오 입장 / 무시]. 응답 보너스 +40 XP. 응답 후에는 한 줄 "✓ 응답함" 요약으로 접힘.
