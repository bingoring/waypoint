...
`;
  ...
  function TopicBinder() {
    ...
    return (
      <Frame label="여정 · 부서 → 주제 (차트 바인더)">
        {/* 부서 헤더 — 바인더 표지 라벨 */}
        <div style={{ padding: '6px 22px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontFamily: HW, fontSize: 14, color: c.soft }}>‹ 부서</span>
          <div style={{ flex: 1 }}/>
        ...
        <div style={{ margin: '10px 22px 0', display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: c.soft, marginTop: 3 }}>주제 5 · 상황 {total} · 우표 <b style={{ color: c.green }}>{done}</b> 모음</div>
          </div>
        </div>
        {/* 부서 진행 — 얇은 잉크 게이지 */}
        <div style={{ margin: '10px 22px 0', height: 8, border: `1.4px solid ${c.ink}`, borderRadius: 3, overflow: 'hidden', display: 'flex' }}>
          {topics.map((t, i) => <div key={i} style={{ width: (t.n / total * 100) + '%', borderRight: i < 4 ? `1.4px solid ${c.ink}` : 'none', background: `linear-gradient(90deg, ${t.state === 'done' ? c.green : c.amber} ${t.d / t.n * 100}%, transparent ${t.d / t.n * 100}%)` }}/>)}
        </div>
        {/* 인덱스 간지 스택 — 바인더 링 + 오른쪽 색 탭 */}
        <div style={{ position: 'absolute', top: 214, left: 0, right: 0, bottom: 78, overflowY: 'auto', padding: '10px 22px 20px 30px' }}>
          {/* 바인더 링 */}
  ...
  function BinderShelf() {
    ...
    const Binder = ({ d, i }) => {
      ...
      return (
        <div style={{ position: 'relative', width: 78, height: 118, flexShrink: 0, transform: `rotate(${[-1.5, 1, -0.5, 1.5][i % 4]}deg)`, cursor: 'pointer' }}>
          {/* 바인더 등 */}
    ...
    return (
      <Frame label="여정 · 부서 선택 (바인더 서가)">
        ...
        <div style={{ position: 'absolute', top: 94, left: 0, right: 0, bottom: 78, overflowY: 'auto', padding: '4px 22px 20px' }}>
          {/* 내 부서 — 펼쳐진 바인더 */}
          <NbPaper rot={-0.4} tape tapeLeft={130} style={{ padding: '12px 14px', boxShadow: `0 3px 8px rgba(62,54,43,.18), 0 0 0 2px ${c.amber}` }}>
  ...
  Object.assign(window, { JourneyStampsAlt, TopicBinder, StampAlbum, StampTrail, BinderShelf, NbStampNode: Stamp, NbYarn: Yarn });
})();
