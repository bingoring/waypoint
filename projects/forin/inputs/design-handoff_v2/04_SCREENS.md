...
See `05_MAP_AND_INTERIORS.md`. Interiors come in two groups:

**Acute departments** (bespoke layouts): **ER, OR, ICU, Pediatrics, Pharmacy**.
Each is a larger tile map with regions/rooms, walls/doors/glass, furniture +
equipment, NPCs, and `!` hotspots that launch scenarios.

**Outpatient clinics** (data-driven, shared layout): **내과 Internal Medicine,
외과 General Surgery, 정형외과 Orthopedics, 피부과 Dermatology**. These are
generated by one reusable `ClinicInterior` engine from a per-department config
(standard plan: reception+waiting → 3 exam rooms → a department-specific
procedure/treatment room). Adding a future clinic = one more config object.
See `05_MAP_AND_INTERIORS.md` → "Outpatient clinic engine".

Every interior includes a **mission banner**, a **HUD** (zone label, D-pad, A
button, 빠른이동/fast-travel), and a Fast-Travel modal (room grid → teleport).
