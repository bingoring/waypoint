...
| `left` / `right` | **3/4 side view** — one visible eye + a small nose bump at the head's front edge; minimal mouth. **Hat** uses a side-profile variant (`hatSide()`): visor/brim points toward the front, and the nurse cross / police badge sits at the front of the cap (never center). **Body** is a narrower side torso with back-edge shading; the chest mark/cross is hidden, and **a single arm** is tucked along the torso (not two front-facing arms). **Two legs swing in opposite phase at body center, crossing front↔back** (far leg darker for depth) for a natural side stride. `left` is the `right` drawing mirrored (`scale(-1,1)`). |
