...
## 홈 개선안 — "출근 브리핑 데스크" (forin-notebook-home2.jsx · HomeV2)
홈 = 매일 아침 받는 근무 브리핑 쪽지. 모듈 순서(위→아래):
1. **라이브 병동**(LiveWardNb) — 기기 시간 연동 3무드(DAY 해/EVENING 노을/NIGHT 별+달+어둠 워시). 수첩 낙서 병동: 침대 3, 바이탈 모니터(파형 펄스), **NbCharacter** 간호사 순회(nbh-walk + 아장아장 waddle) — 하단 무드 바가 시간대별 시나리오 출현 확률 변화를 설명(회진/SBAR/야간 호출).
2. **오늘의 호출**(PagingNb) — 하루 1회 페이저 쪽지: 빨간 헤더 + 흔들림 + 빗금 제한시간 바(오늘 놓치면 소멸) + [지금 응답 → 3분 단기 시나리오 / 무시] + +40 XP. 응답 후 한 줄 '✓ 응답함'으로 접힘.
3. **오늘의 근무 브리핑**(TodayBrief) — 할 일 3칸 체크(복습 5장·커리큘럼 이어서·오늘의 문장), n/3 카운터, 완료 시 취소선, 3/3이면 근무 도장+연속일.
4. **이어서 하기**(ContinueCard) — 커리큘럼 좌표(ER 2-3) + 빗금 진행 바 + 계속.
5. **오늘의 문장**(PhraseCard) — 형광펜 문장 + 듣기/따라하기.
6. **동료 소식 한 줄**(PeerLine) — 라운지 연동 1건 + 응원 링크.
아트보드 A(DAY·호출 대기·1/3) / B(EVENING·응답 완료·2/3) / C(NIGHT·3/3 완료, 인사말 "오늘도 무사히, 밤 근무"). 기존 홈은 참고 섹션으로 보존.

### NbCharacter — 전신 2등신 캐릭터 (forin-notebook-avatar.jsx)
- NbAvatar 에셋 완전 호환: 머리는 NbAvatar의 BACK/FRONT 머리·눈·입 레이어를 0.8 스케일로 그대로 재사용 (skin·hair·hairColor·eyes·mouth·outfitColor props 공유).
- 몸: 2등신 — 옷색 몸통(V넥 선, y38 시작 — 머리와 살짝 겹치게 밀착) + 짧은 팔 2(끝에 살색 손) + 짧은 다리 2(발 꺾임). viewBox 64×96, waddle 기준점 32,83.
- walk=true: 아장아장 — 몸 전체 waddle(±3.5° 발끝 기준) + 팔·다리 교차 스윙(nbc-limb ±22°, .42s alternate). flip=true로 좌우 반전.
- 용도: 홈 라이브 병동 순회 캐릭터, 향후 탐험 모드·라운지 연출 공용.