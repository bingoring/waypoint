      ...
      <Frame label="수첩 리뷰랩 · 말하기">
        <div style={{ position: 'absolute', top: 44, left: 0, right: 0, bottom: 78, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', borderTop: `1.5px solid ${c.ink}`, padding: '13px 20px 20px' }}>
            {/* 검색 + 정렬 */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 4px', borderBottom: `2px solid rgba(62,54,43,.45)` }}>
              <NbIcon name="magnify" size={16}/>
              <span style={{ fontFamily: HW, fontSize: 15, color: '#B4A88F' }}>문장 검색…</span>
              <div style={{ flex: 1 }}/>
              <span style={{ fontFamily: HW, fontSize: 13.5, color: c.ink, whiteSpace: 'nowrap' }}>점수 낮은순 ∨</span>
            </div>
            {/* 점수 분포 */}
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              {[['60↓', 12, 'rgba(199,81,70,.22)'],['60–79', 41, 'rgba(233,196,90,.35)'],['80+', 75, 'rgba(95,141,90,.25)']].map((b, i) => (
                <div key={i} style={{ ...paper(i % 2 ? 0.5 : -0.5), flex: 1, padding: '7px 0', textAlign: 'center', background: b[2] }}>
                  <div style={{ fontFamily: HW, fontSize: 18, color: c.ink, lineHeight: 1 }}>{b[1]}</div>
                  <div style={{ fontSize: 9.5, color: c.soft, marginTop: 2 }}>{b[0]}</div>
                </div>
              ))}
            </div>
            {rows.map((r, i) => (
