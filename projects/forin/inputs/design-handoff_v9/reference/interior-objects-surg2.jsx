...
(function () {
  ...
  function PCAPump({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 18, width: T, height: T * 3, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.28))' }}>
        <svg viewBox="0 0 16 48" width={T} height={T * 3} shapeRendering="crispEdges">
          ...
          {/* pump unit (locked box) */}
          <rect x="2" y="9" width="12" height="11" fill="#475569" stroke={C} strokeWidth=".5"/>
          <rect x="3" y="10" width="10" height="5" fill="#0F1A24"/>
          <rect x="3.6" y="11" width="6" height="1.2" fill="#22D3EE"/>
          <rect x="3.6" y="13" width="8" height="1" fill="#10B981"/>
          {/* the patient demand button on a coiled cord */}
          <path d="M14 16 Q18 19 15 23" fill="none" stroke={C} strokeWidth=".5"/>
          <rect x="13.5" y="23" width="3.5" height="4.5" rx="1.4" fill="#DC2626" stroke={C} strokeWidth=".4"/>
          <circle cx="15.2" cy="25.2" r="1" fill="#FCA5A5"/>
          {/* lock icon */}
          <rect x="10.5" y="16" width="2.5" height="2.5" fill="#FACC15"/>
          {/* pole + base */}
          <rect x="7" y="20" width="2" height="22" fill="#CBD5E1" stroke={C} strokeWidth=".3"/>
  ...
  function JPDrain({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 3, top: y * T + 2, width: T - 6, height: T * 1.3, filter: 'drop-shadow(1px 1.5px 0 rgba(0,0,0,.22))' }}>
  ...
  function Hemovac({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 2, top: y * T + 2, width: T - 4, height: T - 2, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.22))' }}>
  ...
  function NGSuction({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T, width: T * 1.2, height: T * 1.8, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.24))' }}>
  ...
  function SCDDevice({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 2, width: T * 1.5, height: T * 1.6, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.24))' }}>
        <svg viewBox="0 0 24 26" width={T * 1.5} height={T * 1.6} shapeRendering="crispEdges">
          {/* pump unit */}
          <rect x="1" y="2" width="13" height="11" fill="#5B6776" stroke={C} strokeWidth=".5"/>
          <rect x="2.5" y="3.5" width="7" height="4" fill="#0F1A24"/>
          <rect x="3" y="4.2" width="5" height="1" fill="#22D3EE"/>
          <circle cx="11.5" cy="5.5" r="1.6" fill="#10B981" stroke={C} strokeWidth=".3"/>
          {/* twin air hoses to the leg sleeve */}
          <path d="M14 8 Q19 9 19 13" fill="none" stroke="#94A3B8" strokeWidth="1.2"/>
          <path d="M14 10 Q21 11 21 15" fill="none" stroke="#94A3B8" strokeWidth="1.2"/>
          {/* inflatable leg sleeve (velcro segments) */}
          <rect x="16" y="13" width="7" height="11" rx="2" fill="#A8C7DC" stroke={C} strokeWidth=".5"/>
          <line x1="16" y1="17" x2="23" y2="17" stroke={C} strokeWidth=".4"/>
          <line x1="16" y1="20.5" x2="23" y2="20.5" stroke={C} strokeWidth=".4"/>
          <rect x="17" y="14" width="5" height="1.4" fill="#C3DAEA"/>
          {/* base */}
          <ellipse cx="7" cy="14" rx="6" ry="1.6" fill="#6B7280" stroke={C} strokeWidth=".3"/>
        </svg>
      </div>
    );
  }
  ...
  function Walker({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T, width: T * 1.4, height: T * 1.5, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.22))' }}>
  ...
  function WalkerRack({ x, y, w = 3 }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T, width: T * w, height: T * 1.4, filter: 'drop-shadow(2px 2px 0 rgba(0,0,0,.2))' }}>
  ...
  function OPScheduleBoard({ x, y, w = 4 }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T, width: T * w, height: T * 1.5, filter: 'drop-shadow(2px 2px 0 rgba(0,0,0,.24))' }}>
  ...
  function StapleRemover({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 3, top: y * T + 4, width: T - 6, height: T - 8, filter: 'drop-shadow(1px 1.5px 0 rgba(0,0,0,.2))' }}>
  ...
  function AbdoBinder({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 3, top: y * T + 3, width: T - 6, height: T - 6, filter: 'drop-shadow(1px 1.5px 0 rgba(0,0,0,.2))' }}>
