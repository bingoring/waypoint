...
function ScreenInteriorWard() {
  ...
  const regions = [
    ...
    { id: 'room4',   name: '4인용 일반 병실',        icon: '🛏', bounds: { x: 0,  y: 20, w: 28, h: 16 } },
    { id: 'private', name: '1인실',                 icon: '🚪', bounds: { x: 0,  y: 35, w: 14, h: 17 } },
    { id: 'iso',     name: 'VRE 접촉 격리실',        icon: '⚠️', bounds: { x: 13, y: 35, w: 15, h: 17 } },
  ];
  ...
  const rooms = [
    ...
    { id: 'iso',     name: 'VRE 격리실',   sub: '접촉 격리',  icon: '⚠️', color: '#FCA5A5', x: 21, y: 44, questCount: 1 },
  ];
    ...
    <InteriorScreen
      ...
      render={() => (
        <>
          ...
          {/* ═══ ROOM4 / LOWER DIVIDER (y35) ═══ */}
          <IWall x={1}  y={35} w={5} h={1}/>
          <Th    x={6}  y={35} w={2} h={1} label="→ 1인실"/>
          <IWall x={8}  y={35} w={11} h={1}/>
          <Th    x={19} y={35} w={2} h={1} label="→ 격리"/>
          <IWall x={21} y={35} w={6} h={1}/>
          {/* private | isolation divider */}
          <IWall x={13} y={36} w={1} h={15}/>
          ...
          {/* ════════════════ 1인실 (private, y36-50) ════════════════ */}
          <BayLabel x={1} y={36} text="1인실 · PRIVATE"/>
          <IBed x={3} y={38} variant="ward" occupied label="PRIVATE"/>
          <IMonitor x={2} y={38} beep/>
          <IIV x={6} y={38}/>
          <W.WallTV x={2} y={43} w={2}/>
          <IChair x={8} y={40} color="#FED7AA" facing="left"/>
          <window.Sofa x={8} y={45} w={3} color="#9CB4C8"/>
          <INpc x={9} y={41} kind="parent" hair="#3C2A18"/>
          <INpc x={5} y={42} kind="nurse"  hair="#7C3F00" shirt="#A5D8E8"/>
          <IHotspot x={4} y={38} kind="info" label="면역저하 케어"/>
          <IPlant x={11} y={49}/>

          {/* ════════════════ VRE 접촉 격리실 (iso, y36-50) ════════════════ */}
          <BayLabel x={14} y={36} text="VRE 접촉 격리실" highlight/>
          {/* yellow isolation sign on the door + cart OUTSIDE the room */}
          <IsoSign x={19} y={35}/>
          <W.IsolationCart x={15} y={37}/>
          {/* student nurse gowning at the cart, just outside */}
          <INpc x={17} y={39} kind="nurse" hair="#3C2A18" shirt="#FEF9C3"/>
          <IHotspot x={15} y={37} kind="quest" label="가운·장갑 착용"/>
          {/* inside: isolated patient watching TV, dedicated equipment */}
          <IBed x={22} y={38} variant="ward" occupied label="VRE"/>
          <IMonitor x={26} y={38}/>
          <W.WallTV x={24} y={36} w={2}/>
          <DedicatedBP x={22} y={42}/>
          <WasteBin x={20} y={43} tone="infectious"/>
          <IHotspot x={22} y={38} kind="info" label="전용 의료기기"/>
        </>
      )}
    />
  );
...
function MealCart({ x, y }) {
  return (
    <div style={{ position: 'absolute', left: x * ITILE, top: y * ITILE - 2, width: ITILE * 1.6, height: ITILE * 1.8, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.24))' }}>
...
function SharpsBin({ x, y }) {
  return (
    <div style={{ position: 'absolute', left: x * ITILE + 3, top: y * ITILE + 2, width: ITILE - 6, height: ITILE - 2, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.22))' }}>
...
function IsoSign({ x, y }) {
  return (
    <div style={{ position: 'absolute', left: x * ITILE - 4, top: y * ITILE - 14, zIndex: 4, filter: 'drop-shadow(2px 2px 0 rgba(0,0,0,.26))' }}>
...
function DedicatedBP({ x, y }) {
  return (
    <div style={{ position: 'absolute', left: x * ITILE, top: y * ITILE - 12, width: ITILE, height: ITILE * 2.2, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.22))' }}>
