...
(function () {
  ...
  // ── NbCharacter — 전신 2등신 캐릭터 (아바타 에셋 호환: 마리는 NbAvatar 레이어 재사용) ──
  // 큰 머리 + 짧은 팔·다리 몸통. walk=true면 아장아장(몸 기울임 + 팔다리 스윙).
  function NbCharacter({ size = 44, skin = 'beige', hair = 'short', hairColor = 'darkbrown', eyes = 'dot', mouth = 'line', outfitColor = 'sage', walk = false, flip = false, style }) {
    const sk = SKINS[skin] || skin, hc = HAIRC[hairColor] || hairColor, oc = OUTC[outfitColor] || outfitColor;
    const isBald = hair === 'bald';
    if (walk && !document.getElementById('nb-char-css')) {
      const s = document.createElement('style');
      s.id = 'nb-char-css';
      s.textContent = '@keyframes nbc-waddle{0%{transform:rotate(-3.5deg)}100%{transform:rotate(3.5deg)}}@keyframes nbc-limb{0%{transform:rotate(-22deg)}100%{transform:rotate(22deg)}}';
      document.head.appendChild(s);
    }
    const limb = (ox, oy, rev) => walk ? { transformOrigin: `${ox}px ${oy}px`, animation: `nbc-limb .42s ease-in-out infinite ${rev ? 'alternate-reverse' : 'alternate'}` } : {};
    return (
      <svg viewBox="0 0 64 96" width={size} height={size * 96 / 64} style={{ display: 'block', overflow: 'visible', ...(flip ? { transform: 'scaleX(-1)' } : {}), ...style }}>
        <g style={walk ? { transformOrigin: '32px 83px', animation: 'nbc-waddle .42s ease-in-out infinite alternate' } : undefined}>
          {/* 다리 (짧음) */}
          <g style={limb(27, 67)}><path d="M27 67 L25.5 79 M25.5 79 L22.5 79" {...SN} strokeWidth="2.4"/></g>
          <g style={limb(37, 67, true)}><path d="M37 67 L38.5 79 M38.5 79 L41.5 79" {...SN} strokeWidth="2.4"/></g>
          {/* 몸통 (옷 색) */}
          <path d="M23 43 Q23 38 32 38 Q41 38 41 43 L42.5 65 Q32 69 21.5 65 Z" fill={oc} {...S}/>
          <path d="M27 41 L32 46 L37 41" {...SN} strokeWidth="1.4"/>
          {/* 팔 (짧음) */}
          <g style={limb(23.5, 45)}><path d="M23.5 45 L18.5 54" {...SN} strokeWidth="2.4"/><circle cx="18" cy="55.5" r="2" fill={sk} stroke={K} strokeWidth="1.3"/></g>
          <g style={limb(40.5, 45, true)}><path d="M40.5 45 L45.5 54" {...SN} strokeWidth="2.4"/><circle cx="46" cy="55.5" r="2" fill={sk} stroke={K} strokeWidth="1.3"/></g>
          {/* 머리 — NbAvatar 레이어 그대로 (섯리기 18→48 영역) */}
          <g transform="translate(6.5 -1) scale(.8)">
            {!isBald && BACK[hair] && BACK[hair](hc)}
            <circle cx="32" cy="32" r="14" fill={sk} stroke={K} strokeWidth={W}/>
            {(MOUTHS[mouth] || MOUTHS.line)()}
            {(EYES[eyes] || EYES.dot)()}
  ...
  Object.assign(window, { NbAvatar, NbCharacter, NbAvatarAssets });
})();
