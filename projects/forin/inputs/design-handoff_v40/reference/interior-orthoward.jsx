...
function ScreenInteriorOrthoWard() {
  ...
  const rooms = [
    ...
    { id: 'hip',     name: '고관절 골절실', sub: '탈구 방지',   icon: '🦯', color: '#FCA5A5', x: 13, y: 40, questCount: 1 },
  ];
    ...
    <InteriorScreen
      ...
      render={() => (
        <>
          ...
          {/* ═══ ROOM4 / HIP DIVIDER (y31) ═══ */}
          <IWall x={1}  y={31} w={8} h={1}/>
          <Th    x={9}  y={31} w={4} h={1} label="→ 고관절실"/>
          ...
          <INpc x={3} y={30} kind="doctor" hair="#1F2937"/>

          {/* ════════════════ 1인용 고령 고관절 골절 병실 (hip, y32-46) ════════════════ */}
          <BayLabel x={1} y={32} text="GERIATRIC HIP FRACTURE · THR" highlight/>
          {/* elderly patient flat in bed with an abduction pillow between the legs */}
          <IBed x={4} y={34} variant="ward" occupied label="THR · 절대안정"/>
          <window.AbductionPillow x={6} y={36}/>
          <IMonitor x={3} y={34} beep/>
          <IIV x={9} y={34}/>
          {/* bed alarm mat at the bedside */}
          <window.BedAlarm x={4} y={38}/>
          {/* elevated toilet + safety guard (bathroom corner) */}
          <window.ElevatedToiletGuard x={20} y={33}/>
          {/* nurse educating dislocation precautions + worried daughter */}
          <INpc x={8} y={37} kind="nurse"  hair="#3C2A18" shirt="#A5D8E8"/>
          <IHotspot x={5} y={34} kind="quest" label="탈구 방지 교육"/>
          <INpc x={11} y={37} kind="parent" hair="#9A6B3F"/>
          <IChair x={11} y={39} color="#FED7AA" facing="left"/>
          <window.Sofa x={13} y={40} w={3} color="#9CB4C8"/>
          <IPlant x={17} y={40}/>
        </>
      )}
    />
...
function CMSChart({ x, y }) {
  return (
    <div style={{ position: 'absolute', left: x * ITILE, top: y * ITILE, width: ITILE * 2, height: ITILE * 1.2 }}>
      <svg viewBox="0 0 32 18" width={ITILE * 2} height={ITILE * 1.2} shapeRendering="crispEdges" preserveAspectRatio="none">
          <ellipse cx="16.0" cy="15.3" rx="10.9" ry="3.7" fill="rgba(0,0,0,.16)"/>
