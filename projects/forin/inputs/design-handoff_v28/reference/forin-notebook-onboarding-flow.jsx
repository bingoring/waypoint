    ...
    const pageCover = (
      <div className="nb-page" key="p0" style={{ background: '#2E4636', backgroundImage: 'none' }}>
        <div style={{ position: 'absolute', inset: '54px 26px 40px', border: '1.6px solid rgba(212,180,106,.85)', padding: '30px 18px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1 }}/>
          <div onClick={() => go(1)}>
            <NbButton variant="yellow" size="lg" full>여권 펼치기 →</NbButton>
          </div>
          <div style={{ fontFamily: HW, fontSize: 13.5, color: 'rgba(243,230,200,.6)', marginTop: 12 }}>이미 계정이 있어요 · <span style={{ textDecoration: 'underline', textUnderlineOffset: 3, color: '#F3E6C8' }}>로그인</span></div>
        </div>
      </div>
    );
