...
(function () {
  ...
  function TractionFrame({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 20, width: T * 2.4, height: T * 3.4, zIndex: 2, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.28))' }}>
  ...
  function CPMMachine({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 6, width: T * 2.2, height: T * 1.7, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.26))' }}>
        <svg viewBox="0 0 36 28" width={T * 2.2} height={T * 1.7} shapeRendering="crispEdges">
          ...
          {/* base unit + control */}
          <rect x="1" y="19" width="20" height="6" fill="#5B6776" stroke={C} strokeWidth=".5"/>
          <rect x="2.5" y="20.5" width="6" height="3" fill="#0F1A24"/>
          <rect x="3" y="21.2" width="4" height="1" fill="#22D3EE"/>
          <text x="15" y="23.5" fontSize="2.4" fill="#A7F3D0" textAnchor="middle" fontFamily="monospace">90°</text>
          <ellipse cx="4" cy="25.5" rx="2" ry="1.2" fill={C}/>
          <ellipse cx="18" cy="25.5" rx="2" ry="1.2" fill={C}/>
        </svg>
      </div>
    );
  }
  ...
  function PlasterTrapSink({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 2, width: T * 1.6, height: T * 1.9, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.24))' }}>
        <svg viewBox="0 0 26 32" width={T * 1.6} height={T * 1.9} shapeRendering="crispEdges">
          {/* faucet */}
          <rect x="12" y="0" width="2" height="5" fill="#9CA3AF" stroke={C} strokeWidth=".3"/>
          <rect x="12" y="4" width="6" height="2" fill="#9CA3AF" stroke={C} strokeWidth=".3"/>
          <rect x="16.5" y="6" width="1" height="3" fill="#7DD3FC"/>
          {/* deep basin (plaster-cloudy water) */}
          <ellipse cx="13" cy="11" rx="11" ry="3.5" fill="#E5E7EB" stroke={C} strokeWidth=".5"/>
          <ellipse cx="13" cy="11.5" rx="9.5" ry="2.6" fill="#D7DCD6"/>
          <path d="M2 11 L24 11 L22 19 L4 19 Z" fill="#9CA3AF" stroke={C} strokeWidth=".4"/>
          {/* the big plaster trap filter canister underneath */}
          <rect x="9" y="19" width="8" height="9" fill="#B7BEC6" stroke={C} strokeWidth=".5"/>
          <rect x="9.5" y="20" width="7" height="2" fill="#CBD5E1"/>
          <ellipse cx="13" cy="28.5" rx="4.5" ry="2" fill="#94A3B8" stroke={C} strokeWidth=".4"/>
          <text x="13" y="25" fontSize="2.6" fill={C} textAnchor="middle" fontFamily="monospace">TRAP</text>
        </svg>
      </div>
    );
  }
  ...
  function CastCutter({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 2, top: y * T + 3, width: T - 4, height: T - 6, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.22))' }}>
  ...
  function CastRollShelf({ x, y, w = 3 }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 4, width: T * w, height: T * 1.5, filter: 'drop-shadow(2px 2px 0 rgba(0,0,0,.22))' }}>
  ...
  function BraceRack({ x, y, w = 3 }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 8, width: T * w, height: T * 2, filter: 'drop-shadow(2px 2px 0 rgba(0,0,0,.22))' }}>
  ...
  function AbductionPillow({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 2, top: y * T + 1, width: T - 4, height: T * 1.2, zIndex: 2, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.22))' }}>
  ...
  function ElevatedToiletGuard({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T, width: T * 1.4, height: T * 1.5, filter: 'drop-shadow(2px 2px 0 rgba(0,0,0,.22))' }}>
  ...
  function BedAlarm({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T + 2, top: y * T + 2, width: T - 4, height: T - 4, filter: 'drop-shadow(1.5px 2px 0 rgba(0,0,0,.22))' }}>
  ...
  function PACSViewer({ x, y }) {
    return (
      <div style={{ position: 'absolute', left: x * T, top: y * T - 4, width: T * 2, height: T * 1.4, filter: 'drop-shadow(2px 3px 0 rgba(0,0,0,.26))' }}>
        <svg viewBox="0 0 32 22" width={T * 2} height={T * 1.4} shapeRendering="crispEdges">
          {[0,16].map((mx,i) => (
            <g key={i}>
              <rect x={mx+1} y="0" width="14" height="13" fill="#111827" stroke={C} strokeWidth=".5"/>
              <rect x={mx+2} y="1" width="12" height="11" fill="#0B1220"/>
              {/* bone X-ray hint */}
              <rect x={mx+7} y="2.5" width="2" height="8.5" fill="#9FB6C8"/>
              <ellipse cx={mx+8} cy="6.5" rx="2.6" ry="1.4" fill="#C3D2DC"/>
              <rect x={mx+5} y="6" width="6" height="1.4" fill="#7E96A8"/>{/* fracture/plate line */}
            </g>
          ))}
          {/* stand */}
          <rect x="14" y="13" width="4" height="6" fill="#9CA3AF" stroke={C} strokeWidth=".3"/>
