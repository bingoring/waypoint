...
> 11 도메인 · 35주제 · 각 21 상황(기초 6/응용 9/위기 6) = 735. **완료(5/29)**: ER(736) · ICU(735) · OR(735) · WARD(735) ·
> PEDS(735). 누적 태깅 3676 · 175주제. 부서별 코어는 `core-*-<code>`(dept 스코프)로 그 부서 트랙을 이끈다(D-P2-D). 화자
> 배분은 부서 특성대로: ICU 동료·가족, OR 동료(집도의·마취과·소독/순회), WARD 환자 위주, PEDS 부모(가족 486) 위주.
> tagline 선행 무대지시 괄호는 QA 정규화로 제거(WARD 49건, PEDS 0건 — 저작 지시로 예방).
> **다음**: 나머지 24부서 → DB 시드 + 라이브 `/me/curriculum` 계약 전환 → P3 여정 지도 UI.
>
