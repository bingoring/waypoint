...
> 11 도메인 · 35주제 · 각 21 상황(기초 6/응용 9/위기 6) = 735. **완료(3/29)**: ER(736) · ICU(735) · OR(735), WARD·PEDS
> 진행 중. 누적 태깅 2206 · 105주제. 부서별 코어는 `core-*-<code>`(dept 스코프)로 그 부서 트랙을 이끈다(D-P2-D). ICU는 진정·
> 삽관 환자가 많아 화자를 동료·가족 위주로, OR은 intraop 특성상 동료(집도의·마취과·소독/순회) 위주로 배분했다.
> **다음**: WARD → PEDS → 나머지 24부서 → DB 시드 + 라이브 `/me/curriculum` 계약 전환 → P3 여정 지도 UI.
>
