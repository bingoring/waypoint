...
> ✅ **커리큘럼 v3 P2 — ICU 정본 콘텐츠 생성 완료 (2026-09-09, 2/29 부서).** ER과 동일 방식으로 두 번째 부서를 냈다.
> **① 지도 설계**: 중환자 간호 택소노미를 서브에이전트로 설계해 아티팩트로 검수(11 도메인 · 35주제 · 735 구별 상황,
> 각 21개 = 기초 6/응용 9/위기 6). **② 저작**: 지도를 8개 클러스터로 나눠 병렬 저작(seed 1건당 상황 1건, 페르소나 뻥튀기
> 없음). ICU 특성상 진정·삽관 환자가 많아 화자 역할을 환자(73)뿐 아니라 동료(490)·가족(172)으로 분배했다. **③ 부서별
> 코어(D-P2-D)**: 인계·언어·가족·안전을 `core-*-icu`(dept:ICU)로 두어 ICU 트랙을 이끈다(OR→ICU 수술후 인계 등 ICU
> 맥락). **④ 검증**: ICU 735 전부 태깅 · 35주제 전부 21개(THIN 0) · dup-title 0 · **ICU 고아 0**. 누적 ER+ICU 1471
> 태깅 · 70주제. 빌드·테스트 그린. **다음**: 나머지 27부서 fan-out(같은 파이프라인) → DB 시드 + 라이브 계약 전환 → P3.
>
