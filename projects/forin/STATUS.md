...
> 🏗 **현재 초점: TestFlight 피드백 19건 처리(2026-08-18~).** 내부 배포에서 나온 실사용 피드백을 그룹으로
> 나눠 진행 중. 완료: **A**(#1 이모지 제거 → 아이콘 42종) · **B**(#11 키보드·#6 DM 정렬·#18 바텀시트
> 제스처·#19 글자 크기 +15%) · **E**(#16 효과음 6종·#17 페르소나별 TTS) · **G**(#12 최근 10일 리듬·
> #15 이탈 연출+재개) · **C**([커리큘럼 v2](02-construction/curriculum-v2/build-spec-index.md) —
> 층별 테마 커리큘럼 89개·커리어 탭 병합·이어하기) · **D 진행 중**
> ([다국어](02-construction/i18n/build-spec-index.md) — P1~P3 완료, P4~P6 남음).
> 남은 그룹: **F**(#9 명함형 프로필·#10 배지→칭호) · #8 캐릭터 애니메이션 · #13 첫 행동 명확화.
> 결정 근거는 [DECISIONS](DECISIONS.md) 2026-08-18~19 참조.
>
> ⚠️ **미검증(headless 한계):** 언어 변경 시 화면 재렌더 · ja/de 의료 용어의 임상 정확성(원어민 검수 필요) ·
> 마이그레이션 000023과 `/me/ui-lang` 실 DB 왕복(CI에 `TEST_DATABASE_URL` 없음).
>
> ⚠️ **배포 주의:** `/me/curriculum`이 `{chapters}` → `{buildings}`, `/me/situations`에 `tagCode` 추가 —
> **파괴 변경**이므로 서버 배포와 모바일 OTA가 같이 나가야 한다.
>
