...
## 2026-08-13 — 9-A 실배포 완료: staging 스모크 57/0 + 실경로가 잡은 결함 3종
서울 Cloud Run + Cloud SQL에 **콘솔 클릭 0회로 66리소스**를 세우고 파이프라인 전 단계를 실경로로 통과했다.
상세는 [3-1 §11.1·§11.2](03-operations/01-deployment.md).

**이 라운드의 핵심 교훈: 정적 검증의 통과와 실경로의 동작은 다른 사건이다.** 세 번의 컨텍스트 분리 리뷰, `terraform
validate`, YAML 파싱, **로컬 스모크 57/0**을 모두 통과한 구성에서 실제 apply·배포가 결함 세 개를 즉시 드러냈다.
셋 다 "명령을 실제 리소스에 쏴봐야만" 보이는 부류였고, 그래서 §11에 남긴 "첫 실행 관측" 체크리스트가 값을 했다.
- **Cloud SQL 에디션**: 새 Postgres 인스턴스가 `ENTERPRISE_PLUS`로 기본 생성돼 공유 코어 티어를 아예 거부.
  놓쳤으면 월 예산이 자릿수째 달라졌다. **비용에 직결되는 필드는 제공자 기본값에 맡기지 않는다.**
- **gcloud 프로젝션**: `:` 가 부분 문자열 매칭이라 `filter("type:Ready")`가 `ContainerReady`까지 잡았고,
  `extract()`가 값을 `['name']`으로 감쌌다. 그 결과 `promote.yml`의 준비 확인은 **항상 빈 값을 받아 모든 승격이
  실패할 상태**였다. `=` + `.flatten()`으로 교정, 네 형태 모두 실제 리소스에 대고 실측.
- **시크릿 후행 개행**: `openssl rand -hex 32`의 개행이 그대로 저장돼 65바이트가 됐는데 호출측 `$(...)`는 64바이트를
  보냈다 → dev-auth 게이트가 항상 거부 → 스모크가 15/56. **게이트는 정확히 작동했고 시크릿이 잘못 저장돼 있었다.**
  진단을 가른 것은 응답이 Go 기본 평문 404가 아니라 핸들러의 JSON 404였다는 점 — R-2가 "staging에서 오시크릿 응답이
  JSON이라 구별된다"를 사소한 정보 노출로 Minor 이연했는데, **그 구별이 여기서 진단 도구가 됐다.**

**설계가 의도대로 작동한 것들**
- **verified 태그 게이트가 양방향으로 확인됐다**: 스모크 실패 실행에서는 태그 스텝이 `skipped`돼 그 SHA에 승격할
  이미지가 없었고, 통과 실행에서 붙었다. 실패한 빌드가 prod로 갈 경로가 실제로 닫혀 있다.
- **`traffic`을 `ignore_changes`에 넣은 것**: 첫 배포 후 `plan`에 traffic diff가 **없다** — 무관한 apply가 롤백을
  되돌리는 위험이 실측으로 닫혔다.
- **시드를 배포에서 분리한 것**: 배포만으로는 DB가 비어 다이얼로그·부서 상황·홈·access가 실패했다(49/57).
  `seed.yml`을 돌린 뒤 57/0. **첫 환경 구축 순서는 배포 → 시드 → 스모크**임을 기록해 둔다.

**새로 문서화한 알려진 한계**: 서비스 `scaling` 블록의 영구 diff. API가 `manual_instance_count`를 항상 0으로 보고하는데
서비스 레벨 스키마가 그 인자를 받지 않고 `ignore_changes`도 그 중첩 깊이에 닿지 못한다. 블록을 통째로 무시하면
문서화한 "prod를 1로 올리기"가 조용히 무력화되므로, Terraform 소유권을 지키고 **이 diff는 대기 중 변경이 아니라는 것**을
`runtime.tf`에 못박았다 — plan이 시끄러우면 진짜 변경을 놓친다.
...
## 2026-08-18 · 앱 전역 글자 크기 상향 — 핸드오프 타이포와 의도적으로 어긋난다
- **결정:** 모든 `fontSize`를 `theme/tokens.ts`의 `fs()` 한 함수로 통과시키고 **≤16px 구간에 1.15배**를 적용한다
  (정수 반올림, 하한 9, 상한 17). 17px 이상은 손대지 않는다. TestFlight 피드백 "앱 전체적으로 글자 크기를 키워야
  할것같아. 지금 너무 오밀조밀해".
- **핸드오프 대비 편차임을 명시한다.** `design-handoff_v22/01_DESIGN_TOKENS.md`는 Label/HUD 9–10, Micro 6–8,
  Caption 11, Body 12–14를 정본으로 정한다. 이 결정은 그 값을 렌더 시점에 키우므로 **SoT와 1:1이 아니다.**
  이 프로젝트는 핸드오프 픽셀 1:1을 규율로 지켜왔으므로 편차를 감사 기록으로 남긴다.
- **근거:** 실측 결과 하드코딩된 `fontSize`가 **696건**, 스케일(`type.*`) 사용은 **12건**이었다. 즉 토큰 스케일이
  사실상 쓰이지 않아 "스케일만 올린다"는 방법이 작동하지 않는다. 674건을 값으로 직접 고치면 되돌리기 어렵고
  튜닝도 불가능하므로, **한 곳의 노브**를 만들어 전부 통과시켰다(`fs()`).
- **왜 ≤16px만:** 조밀함은 작은 글자에서 온다. 스플래시 로고(64)·결과 제목(34) 같은 표시용 크기를 함께 키우면
  레이아웃만 밀리고 불만은 해결되지 않는다.
- **왜 정수 반올림:** DungGeunMo·Galmuri11은 픽셀 폰트다. 비정수 크기는 커지는 대신 뭉개진다.
- **왜 17로 상한:** 없으면 16px이 18로 렌더되는데 손대지 않은 17px은 17로 남아 **작은 설계값이 더 크게 보인다.**
  경계에서 단조증가를 유지한다(기계적으로 확인).
- **대안(탈락):** ①스케일만 올린다 — 하드코딩 696건이 그대로 작다 ②674건을 값으로 직접 고친다 — 되돌리기·튜닝 불가
  ③v23 핸드오프에서 타이포를 다시 받는다 — 사용자가 지금 고치기를 택했다.
- **미검증:** 고정 폭 컨테이너(예: 점수 라벨 `width: 26`·`42`)에서 **글자가 잘릴 수 있다.** 이 환경이 headless라
  시각 확인을 못 했다. TestFlight에서 확인이 필요하다.
- **결정자:** 사용자(방향·편차 허용) / AI(구현 방식).

## 2026-08-18 · 커리큘럼 v2 — 층 1개 = 챕터 1개를 버리고 층별 테마 커리큘럼으로
- **결정 3개(맞물려 있어 한 항목으로 둔다):** ①층과 커리큘럼은 **전부 열린다**, 순차는 커리큘럼 안의
  스텝에만 남는다 ②테마는 근무 흐름 3단이지만 **이름과 배정을 층별로 손저작**한다(기계 배정은 폴백으로만)
  ③`CH.1~25` 전체 로드맵 목록을 삭제하고 **건물 → 층 → 커리큘럼 계층이 로드맵을 대체**한다.
- **왜 한 항목인가:** 층 해금이 없어지면 "다음에 뭘 하지"를 알려줄 주체가 커리어 탭에서 사라진다. 그 역할은
  홈의 이어하기(#7)가 전담해야 하므로 세 항목은 **분리 배포할 수 없다.**
- **근거(기계 배정을 버린 이유):** authored 필드(`Diff`/`Acuity`/`Role`)로 3단을 나누면 **29개 부서 중
  14개에서 깨진다.** SIM·LOUNGE·SPD는 환자가 없어 10개 주제가 전부 `colleague`고(→ ①② 공백), ER은 긴급
  주제가 7개다(응급실의 실제 성격이지 데이터 오류가 아니다). 전수 계산으로 확인했다.
- **근거(층별 손저작으로 간 이유 — 사용자 판단이 내 추천을 뒤집었다):** 나는 처음 "규칙 + 예외표"를
  추천했다. 사용자가 *"층층들은 다 달라야하는거 아닌가"*라고 물어 확인해 보니, **이름을 층별로 쓰는 순간
  그 층의 10개 주제를 어차피 다 읽어야 하므로 배정까지의 추가 비용이 거의 0**이고 예외표의 장점(작은 diff)은
  사라진다. 또 주제 제목이 부서 간 2건 중복(`다학제 회진 인계`·`검사실 인계`)이라 **제목을 키로 쓰는 표는
  두 부서에 동시 적용된다** — 기존 `urgentTopics`의 잠재 결함이고 손저작에서는 발생하지 않는다.
- **선결 결함(이 결정의 진짜 계기):** 손저작 챕터 1·2의 **스텝 이름 11개가 실제 시나리오와 무관**했다.
  "출근 · 인사와 자기소개"를 누르면 `SCN-ER-00001` = 흉통 환자 트리아지가 열렸다. id는 전부 존재해 시드
  가드는 통과했고, **두 문자열을 비교하는 테스트가 하나도 없었다.** 상세는 `curriculum-v2/build-spec-index.md` D1.
- **대안(탈락):** ①난이도 3티어로 정직하게(①기본 ②응용 ③고난도) — 저작 0이지만 "근무 흐름" 이야기가
  사라진다 ②규칙 + 예외표 — 위 근거로 이점이 사라진다 ③층 전부 완료해야 다음 층 — 커리큘럼 89개를
  한 줄로 꿰면 가혹하다.
- **미검증:** 89개 커리큘럼 목록의 스크롤 감각·층 아코디언 동작(headless 불가). `/me/curriculum`이
  `{chapters}` → `{buildings}` **파괴 변경**이라 서버 배포와 모바일 OTA가 같이 나가야 한다.
- **결정자:** 사용자(3개 결정 + 손저작 방향) / AI(구현·저작 내용·불변식 테스트).

## 2026-08-18 · 다국어 — 전면 구현하되 기계번역으로 카탈로그를 채우지 않는다
- **결정:** ①프로필 설정에서 **앱 UI 언어**를 바꿀 수 있다(재시작 없이 즉시 반영) ②학습은 온보딩에서 고른
  **목적지 나라의 언어**로 이루어진다 ③UI 언어와 학습 언어는 **별개 축**이다 ④번역이 없는 키는 한국어로
  폴백하고, 그 언어를 "지원됨"으로 광고하지 않는다.
- **왜 별개 축인가:** 한국인이 미국으로 가면 UI=ko, 학습=en이다. 하나로 합치면 독일어 UI로 영어를 배우는
  조합이 표현 불가능해진다. 서버에서도 `ui_lang`을 `native_lang`과 분리했다 — `native_lang`은 AI가 교정을
  어느 언어로 설명할지 정하므로, 영어 UI를 고른 한국인이 영어 설명을 받게 되면 안 된다.
- **왜 기계번역을 금지하는가:** 사용자 지시 *"야매로 하지 말고"*. 그리고 `구획증후군 의심 사정`·
  `헤파린 더블 체크` 같은 의료 지시문은 **오역이 안전 문제**가 된다. 번역 완성도는 카탈로그에서 **계산**하며
  (손으로 적은 "지원됨" 라벨은 키가 늘면 즉시 거짓이 된다) 설정 화면이 그 값을 그대로 보여준다.
- **조사에서 드러난 것:** 학습 언어(대화·발음·TTS)는 **이미 `TargetLang`을 따르고 있었다**
  (`conversation/engine.go`의 `langContext`, `speech`의 `localeFor`/`voicesByLocale`). 빈 곳은 UI 언어(0%)와
  서버가 보내는 콘텐츠 문자열(0%)이었다. 사용자에게 보이는 한국어는 전수 **4,788개**(클라이언트 1,728 +
  서버 3,060)이므로 순서를 인프라 → 추출 → 번역으로 잡았다.
- **대안(탈락):** ①인프라만 만들고 번역은 나중에 — 사용자가 *"나중으로 미루지 말고 지금 다 구현해"*로 기각
  ②전용 번역 서버 + DB + 관리 UI — 번역이 DB로 가면 앱이 런타임 의존을 얻고(캐시 무효화·오프라인)
  카탈로그가 git을 떠나 diff·리뷰·롤백을 잃는다. 대신 **카탈로그에서 생성하는 편집 가능한 번역표**를 만들었다
  (`mobile/scripts/i18n-matrix.py`) ③TMS(Crowdin 등) — 실제 번역가가 붙는 단계에서 맞다.
- **미검증:** 언어 변경 시 각 화면이 실제로 다시 그려지는지(시뮬레이터 필요). **ja/de 의료 용어의 임상
  정확성** — `Hauptbeschwerde`·`Übergabereihenfolge`가 사전적으로 맞아도 독일 병동에서 쓰는 표현인지
  보증할 수 없다. 원어민 검수 전에는 그 언어를 완전 지원으로 제시하지 않는다.
- **결정자:** 사용자(범위·품질 기준·관리 방식) / AI(레이어 설계·키 체계·검증 장치).

## 2026-08-19 · i18n 서버 카탈로그에 한국어를 넣지 않는다
- **결정:** 서버 문자열 카탈로그는 **비한국어 오버라이드만** 담고, 조회는 저작된 한국어를 폴백으로 넘긴다:
  `i18n.Tr(locale, c.Key, c.Name)` — `Name`이 곧 한국어다. 키는 콘텐츠가 이미 가진 식별자
  (커리큘럼 `Key`, 스텝 `ScenarioID`)를 재사용하고 **두 번째 키 공간을 만들지 않는다.**
- **근거:** 700개를 카탈로그로 옮기는 편은 **커리큘럼 v2의 불변식 테스트가 지키는 바로 그 파일들
  (`authored_*.go`)을 전부 건드리는 조용한 대규모 편집**이었다. 폴백 방식은 ①번역 없는 로케일이 오늘과
  똑같이 렌더되고 ②언어 추가가 콘텐츠 파일을 한 줄도 건드리지 않고 ③저작된 이름을 읽는
  `catalog_content_test.go`가 계속 유효하다.
- **부수 결정:** 미들웨어는 프로필을 읽지 않고 `Accept-Language`만 본다. 언어를 알기 위해 DB 왕복을
  추가하면 앱 첫 화면까지 느려지고 클라이언트는 이미 자기 설정을 안다. `profiles.ui_lang`(마이그레이션
  000023)은 **재설치 복구용**이다.
- **부수 결정:** 상황 태그를 `tagCode`(cleared/urgent/new) + `tag`(요청 언어 라벨)로 쪼갰다. 한국어 하나로
  상태와 표시를 겸해서 클라이언트가 `'완료'`와 비교할 수밖에 없었고, **그 문자열은 비교를 깨지 않고는
  번역이 불가능했다.**
- **대안(탈락):** 한국어까지 카탈로그로 이관 — 위 회귀 위험. 로케일별 콘텐츠 파일 분기 — 한 언어에서만
  고쳐지는 버그를 만든다.
- **결정자:** AI(설계) — 사용자 승인 범위 안의 구현 판단.

## 2026-08-21 · React Compiler가 켜져 있다는 사실을 설계 제약으로 기록한다
- **발견 경로:** 문서가 아니라 **컴파일된 번들**이다. 즐겨찾기 별이 "접었다 펴야 반영된다"를 좁히다가
  번들에서 `function PixelIcon(t0)`를 봤고(인자 하나로 재작성된 형태), `app.json`의
  `experiments.reactCompiler: true`를 확인했고, 같은 기전이 번역에도 있는지 번들을 grep해
  **메모 슬롯에 캐시된 `t()` 호출 66개**를 셌다. 이 저장소의 문서에는 컴파일러 언급이 **한 건도**
  없었다 — 실험이 켜져 있다는 사실도, 그 결과도.
- **제약:** 컴파일러는 표현식을 reactive input으로 캐시한다. 따라서 **인자가 변하지 않는 모듈 상태
  읽기는 컴포넌트 인스턴스당 한 번만 계산된다.** 리렌더는 값을 갱신하지 않는다.
  - `isFloorFavorite({building, floor})` — 별이 낡았다. `useFavorites()`를 반환값 없이 호출해
    구독만 하는 것은 **메모 그래프에 아무것도 넣지 않는다.**
  - `t("some.key")` — 언어를 바꿔도 처음 마운트될 때의 문자열이 남는다.
- **결정:** ①스토어 값은 훅이 돌려주는 스냅샷에서 파생시킨다(`useIsFloorFavorite`). ②루트
  내비게이터를 로케일로 키잉해 언어 변경이 트리를 리마운트하게 한다.
- **대안(탈락):** 로케일에 묶인 번역 함수(`const tr = useT()`)로 모든 호출 지점에 reactive input을
  주는 것. **더 옳고 스크롤 위치도 지킨다.** 호출 지점이 수백 곳이어서 기기 확인 없이 한 번에 하기엔
  회귀 위험이 커 보류했다 — 사용자 판단 대기.
- **검증 장치:** 세 가드 모두 **소스 규칙**이다. jest의 트랜스폼은 컴파일러를 돌리지 않아서,
  **기기가 틀린 내내 렌더 테스트가 통과했다.** 이 부류는 렌더 테스트로 잡을 수 없다.
- **결정자:** AI(진단·즉시 조치) / 사용자(useT 전면 전환 여부).

## 2026-09-13 · CI 복구 — 계약 드리프트 + v2 카탈로그를 손저작 우선으로 재지정
- **발견:** 9/8 `ad5ecda`(리뷰랩 v40)부터 `contract`·`deploy`가 빨갛다 — **5일·푸시 12건 동안 staging 배포 0**
  (마지막 성공 9/7 `9fbe106`). 원인이 둘이다. ①**계약 드리프트** — `ModelAnswerGroup.steps`(`ad5ecda`)와
  `GET /me/curriculum/tracks`(`8fea6e9`, v3 P1)가 `make contract` 없이 들어갔다. ②9/10 `badbf54`(WARD)부터
  **server 테스트도 레드** — P2 정본 재발급(R-P2-8)이 gen 시나리오 id(`001xx`)를 새로 매겨, 아직 라이브인 v2
  카탈로그의 60스텝(WARD·PEDS·LD·NICU·PICU·PSYCH 각 10)이 엉뚱한 시나리오를 가리켰다
  (`TestEveryStepNamesItsContent`: "자살 위험 사정" → "지남력 확인"). ER·ICU·OR은 v2가 손저작 id만 가리켜 무사했다.
- **왜 놓쳤나:** 부서 커밋마다 `themed`·`gencontent` 패키지만 돌려 STATUS에 "테스트 그린"을 적었고, 전체
  `go test ./...`·계약 검사·CI 결과를 보지 않았다. 2026-08-19에 적은 "계약을 바꾸는 커밋은 파이프라인 확인을 같은
  체크포인트에 넣는다"의 **재발**이다.
- **결정:** v2 스텝을 새 id로 **재지정**하고(테스트를 좁히지 않는다), 스텝 이름은 가리키는 시나리오 제목으로 바꾼다.
  **손저작 `scn-*`(`0000x`)을 우선** — gencontent가 재발급하지 않으므로 다음 재생성에도 흔들리지 않는다(44/60).
  맞는 손저작이 없을 때만 gen id(16 — WARD는 손저작이 0개라 10 전량). boss 스텝에는 난이도 2~3을 둔다.
- **대안(탈락):** ①재발급 부서를 v3 전환 전까지 테스트에서 제외 — 라이브 경로가 실제로 틀린 시나리오를 여는 동안
  가드를 끄는 것. ②v3 전환을 앞당겨 v2 삭제 — P2가 9/29라 불가.
- **적용:** 남은 20부서 fan-out에서 부서 커밋 전 게이트 3개 — ①v2 스텝 재지정 ②`go test ./...` 전체 ③`make contract`
  무드리프트. v2(`authored_*.go`)는 P3 라이브 전환 때 사라지므로 이 비용도 그때 끝난다.
- **부수(툴체인):** 이 Mac에서 Go 1.26.2로 빌드한 swag는 macOS가 SIGKILL(exit 137) — `codesign --force -s -`로
  ad-hoc 서명하면 돈다. `golang:1.26` 도커 이미지는 go.mod(`go 1.26.2`)보다 낮아 `GOTOOLCHAIN=local`로 실패한다.
- **결정자:** 사용자(재지정 방식 선택) / AI(진단·시나리오 선정).

## 2026-09-13 · P2 PHARMA — 간호사 시점 프레이밍 · 손저작 인라인 태깅 · 교차 클러스터 검증
- **결정:** ①PHARMA를 비임상 4곳(LOUNGE·SPD·MORGUE·SIM)과 달리 **35×21 임상 모델로** 낸다. 학습자는 간호사이므로
  "약제부에서 일하는 간호사"가 아니라 **약물 안전·약사/처방의 협업·복약 교육 접점**으로 구성한다(colleague 49%).
  ②부서 fan-out에서 **그 부서의 손저작 `scn-*`와 인사 `00900`에 theme을 인라인 태깅**한다(R-P2-2 — 앞선 9부서에서는
  빠져 113건이 고아로 남았다). ③병렬 클러스터 저작 뒤 **교차 클러스터 검증**을 한다: 같은 상황은 제목만 바꾸지 않고
  한쪽을 다른 상황으로 재저작, 수치 스팟체크, 페르소나 이름 중복은 결정적 성씨 교체.
- **근거:** 병렬 클러스터는 서로를 보지 못해 같은 상황을 각자 쓴다 — 이번에 제목 중복 4쌍 + **제목이 다른 같은 상황 1쌍**
  (U-500을 일반 인슐린 주사기로 과량 투여)이 나왔고 후자는 제목 검사로 걸리지 않았다. 수치 오류 3건(aPTT 48을
  "증량" 구간으로 씀 · 부메타니드 1 mg ↔ 정주 푸로세마이드 40 mg · 근거 불확실한 처치 문장)도 저작자 자기검증을 통과했다.
- **대안(탈락):** PHARMA를 비임상과 함께 보류 — 약물 안전은 간호 핵심 역량이라 커리큘럼 공백이 크다.
- **결정자:** 사용자(fan-out 진행) / AI(프레이밍·검증 규칙).

## 2026-09-14 · 계약 생성기 버전 고정 — `@latest`가 API 변경 없이 드리프트를 만들었다
- **발견:** SURGWARD 커밋(`b4bb32a`, 콘텐츠만 변경)에서 `contract`·`deploy`가 드리프트로 실패했다. 차이는 `types.ts`에서
  `$ref` 속성의 JSDoc 설명 위치뿐. 원인은 CI가 `go install swag@latest`로 **swag v2.0.0-rc6**(2026-09-13 09:12 UTC 배포)을
  받은 것 — 커밋된 계약은 rc5로 생성됐다. PHARMA CI(05:04 UTC)는 rc6 배포 전이라 통과. 로컬 swag는 9/12 빌드(rc5)라 로컬
  검사도 통과했다. **어떤 커밋이든 같은 방식으로 실패**했을 것이다.
- **결정:** 생성기 버전을 **고정**한다 — swag `v2.0.0-rc5`(Makefile 설치 안내 · `contract.yml` · `deploy.yml`), openapi-typescript
  `7.13.0`(Makefile). 계약 산출물은 그대로(재생성 시 무드리프트 확인).
- **근거:** 드리프트 게이트는 "코드와 계약의 불일치"를 잡으려는 것인데, `@latest`는 도구 릴리스를 코드 변경처럼 보이게 한다.
  CI 결과가 외부 릴리스 시각에 좌우되면 게이트가 신뢰를 잃는다.
- **대안(탈락):** rc6로 재생성해 커밋 — 다음 릴리스에 똑같이 깨진다. rc6 채택은 **의도적 업그레이드**(버전 3곳 동시 변경 + 재생성
  커밋)로 따로 한다.
- **결정자:** AI(진단·수정) — CI 복구 범위 안의 판단.

## 2026-09-15 · D-P2-E 비임상 4부서 콘텐츠 모델 — 주제 수는 부서마다, 상황 21은 불변
- **발견:** 비임상 4곳(LOUNGE·SPD·MORGUE·SIM)을 한 묶음으로 보고 "35×21 임상 모델이 안 맞으니 보류"로
  미뤄 왔으나, 실제로는 네 곳의 성격이 서로 다르다. 손저작 12건이 그 차이를 이미 보여준다 — LOUNGE는
  교대 조율·갈등·멘토링·번아웃·문화 차이(화자 전부 동료), SPD는 멸균·재고·리콜(동료), MORGUE는 유족
  응대·검시·기증(가족 위주), SIM은 디브리핑·술기 평가(메타 성격)다.
- **실측(설계의 전제):** **주제 수는 엔진에 무해하다.** 티어 해금은 `Unlocked(prevTierDone bool)`로 직전
  티어 완료만 보고(`internal/domain/learning/policy.go:48`), audit 게이트는 주제당 상황 수(<20이면 THIN)만
  본다. 부서당 35라는 숫자는 관행이지 구조적 제약이 아니다. 반대로 **주제당 21상황은 THIN 게이트에
  직결되므로 불변**이다. 조정 가능한 축은 주제 수 하나뿐이다.
- **결정:**
  - **LOUNGE = 35주제(735상황), 축을 임상에서 직장 생활로 전환.** 코어 4는 안전(직원 안전)·언어·
    인계(교대)·**동료 관계·팀워크**(가족 소통 대체). 심화 31은 스케줄 협상·프리셉터·번아웃·동료 갈등·
    문화 차이·성과 평가 면담·상사와의 의견 차이·면접·보수교육·위원회 발언·인시던트 리포트 문화·동료의
    실수 지적·피드백·차별과 미세공격 대응·직장 내 괴롭힘·급여 문의·노조·휴가 요청·이직 통보·부서 이동·
    육아 휴직·직원 부상 보고·이메일 소통·전화 응대·사교 행사·경계 설정 등.
  - **SPD = 15주제(315상황).** 코어 4는 안전(멸균·환자 안전)·언어·인계·**타 부서 요청 응대**(가족 대체).
    심화 11은 멸균 요청·재고 조달·물품 추적·재처리·리콜·유효기간·케이스 카트·내시경 소독·대여 장비·
    오염 노출·신규 물품 교육.
  - **MORGUE = 15주제(315상황).** 코어 4 유지(안전=감염 시신·언어·인계·가족=유족). 심화 11은 사후 처치·
    유족 첫 대면·신원 확인·검시 절차·기증 안내·문화 관습·소지품 반환·장례식장 인계·무연고 사망·소아
    사망·직원 디브리핑.
  - **SIM = 15주제(315상황), 자체 저작 · 디브리핑 축.** 코어 4 유지(인계=SBAR 시뮬·가족=표준화 가족 역할).
    심화 11은 사전 브리핑·구조화된 디브리핑·술기 루브릭·팀 리더십·위기 자원 관리·근접사례 리뷰·피드백·
    자기 성찰·동료 평가·재응시·대량재해 시뮬.
  - **화자 배분의 정직한 편차:** LOUNGE colleague ~90%·patient 0 / SPD colleague 거의 전부·patient 0 /
    MORGUE family ~60%·colleague ~40%·patient 0(고인 비발화) / SIM colleague ~80%·표준화 환자 ~15%·
    가족 역할 ~5%. patient 0은 NICU(신생아 비발화) 선례가 있다.
  - **구조 변경 0:** themes.yaml 엔트리 추가뿐. 신규 nameKey 2개(`theme.core.peer`·`theme.core.request`)를
    i18n 4개 언어에 추가한다. 클러스터 수만 주제 수에 비례해 조정(35주제=8, 15주제=4).
- **근거:** LOUNGE는 축소 대상이 아니라 재정의 대상이다 — 임상 심화 31이 안 맞을 뿐, 직장 생활 축으로
  바꾸면 35주제가 넉넉하고 "한국 간호사의 미국 직장 영어"라는 제품 목표에 가장 직결된다(어느 임상
  부서에도 담기지 않는 영역이다). SPD·MORGUE는 주제 축 자체가 좁아 15가 정직한 규모이고, 억지로 35를
  채우면 얕거나 중복된 주제가 생긴다.
- **대안(탈락):** ①네 곳 모두 동일 축소(15×21) — 파이프라인은 단순하나 LOUNGE의 주제 폭을 인위적으로
  버린다. ②네 곳 모두 35×21 유지 — SPD·MORGUE에 얕은 주제가 생긴다. ③**SIM을 재사용 트랙으로 설계**
  (다른 부서 시나리오가 SIM 주제에도 속함) — 시나리오의 `theme`은 단수 문자열이라(`themed/theme.go:16`)
  태그 복수화라는 엔진 변경이 필요하고, 조립·고아 판정·마이그레이션 000038에 파급되며 P3-A 라이브
  전환과 시기가 겹친다. 게다가 회고·자기평가는 원 시나리오와 발화 형태가 완전히 달라 재사용 가치도
  낮다. ④SIM을 커리큘럼에서 제외 — 맵에 있는 장소가 여정에서 비게 된다.
- **결정자:** 사용자(곳마다 다른 모델 · SIM 자체 저작 · 코어를 부서 맥락으로 교체) / AI(주제 목록·화자 배분).

## 2026-09-17 · D-P2-F 초기 8부서 재생산 — 주제 키는 유지하고 21상황만 전면 재설계
- **발견:** 29부서를 완주한 뒤 부서 간 품질을 측정했더니, 초기에 저작한 부서들이 D-P2-B의 전제를
  지키지 못하고 있었다. D-P2-B는 주제를 "서로 구별되는 상황들의 경로"로 규정했는데, 이 부서들은
  **21상황을 한 사건의 시간 순서로 쪼갠 절차 단계**로 만들어져 있다.
  - WARD `ward-transfusion`: 동의 확인 → 교차 채혈 → 기준 활력징후 → 이중 확인 → 목적 설명 → 정맥로 확보
  - RAD `rad-contrast-anaphylaxis`: 초기 증상 인지 → 즉시 중단 → 도움 요청 → 활력징후 → 기도 관찰 → 산소 준비
  - PSYCH `psych-suicide-risk`: 개방형 질문 → C-SSRS 시작 → 안심 → 과거 시도력 → 보호 요인 → 절망감
  이 구조에서는 학습자가 21회 플레이해도 서로 다른 임상 상황을 만나지 못한다. 게다가 단계가 이어지는데
  페르소나는 상황마다 바뀌어서(같은 수혈인데 63세→58세→71세) 내적 모순까지 생긴다.
- **지표:** 제목 평균 길이가 RAD 7.9·PSYCH 8.0·LD 8.5·NICU 8.6·ONCO 8.6·PEDS 9.2·PICU 9.2·WARD 9.7자로,
  최근 부서(SPECIALTY 28.6·LOUNGE 35.9·GEN 37.4·MORGUE 40.1)의 4분의 1이다. 짧은 제목은 원인이 아니라
  증상이다 — 절차 단계에 이름을 붙이면 명사구가 되고, 장면을 서술하면 길어진다. `sub`이 정형 인구통계
  (`"63y / Male"`)인 비율도 LD 85%·DIAL 84%·INFUSION 82%·ER 74%·WARD 68%·RAD 65%인데 최근 부서는 0%다.
- **결정:** **하위 8부서(RAD·PSYCH·LD·NICU·ONCO·PEDS·PICU·WARD = 5,880상황)를 전량 재생산한다.**
  - **주제 35개의 키와 이름은 유지한다.** 축 자체는 임상적으로 잘 갈라져 있고(RAD의 조영제 5분할,
    WARD의 내과 질환군), 무엇보다 손저작 시나리오와 v2 스텝이 이 키를 참조한다. 키를 바꾸면 고아가 생긴다.
  - **21상황만 전면 재설계한다.** 설계 지시서에 「절차 단계로 쪼개지 마십시오」를 최상위 규칙으로 두고,
    상황을 가르는 축을 명시한다(환자가 가져오는 조건·심각도와 양상·막히는 지점·대화 상대·국면).
  - **735건 규모를 유지한다.** v2 `authored_*.go`가 참조하는 `SCN-<CODE>-00101`~`00835` 범위의 id가
    그대로 남아야 하고, 이름은 `namesync.py`가 다시 맞춘다.
  - **방 목록을 부서마다 정본화한다.** 재생산 전 RAD의 `room`은 64종이었고 `CT 1`·`CT 검사실`·
    `영상의학과 검사실`·`RAD Waiting`·`IR Suite`가 뒤섞여 있었다.
  - **미태깅 손저작을 함께 처리한다.** 이 8부서에 남은 94건(PEDS 15·WARD 1·나머지 13씩)은 233건 백로그의
    일부이므로, 부서를 재생산할 때 같이 태깅한다.
- **중위 10부서는 손대지 않는다:** OR·GERI·ER·ENDO·DIAL·ICU·INFUSION·DERM·SURGWARD·PHARMA는 제목이
  10~15자로 짧지만 상황이 절차 단계로 쪼개져 있지 않다. 특히 DERM은 14.0자인데 「품질 기준」 절과
  성씨 3축 검증을 갖춘 파이프라인을 온전히 통과했다. **제목 길이는 판정 기준이 아니다.**
- **대안(탈락):** ①제목·`sub`·`keyPhrases`만 보정 — 저렴하지만 "한 주제의 21상황이 서로 다른 장면"이라는
  근본 문제가 남는다. ②15자 미만 18부서 전량 재생산(13,231상황) — 문제없는 10부서까지 다시 만든다.
  ③보류하고 라이브 전환 먼저 — 전환 뒤에 콘텐츠를 바꾸면 학습자 진척이 이미 붙은 상태에서 갈아야 한다.
- **결정자:** 사용자(재생산 범위 = 하위 8부서) / AI(주제 키 유지·방 정본화·손저작 동시 태깅).
