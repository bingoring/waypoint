...
- **staging 스모크의 인증 경로**(구현 계획 수립 중 발견한 공백). 스모크 57 assert는 `POST /auth/dev`로 인증하는데 그 경로는
  ...
  `X-Dev-Auth` 헤더를 요구**한다. prod에는 시크릿을 주지 않으므로 **경로 자체가 존재하지 않는다**(위 404 불변식 유지).
  응답은 401이 아니라 **404** — 밖에서는 없는 것처럼 보여야 한다. 빈 시크릿은 빈 헤더와도 일치하지 않으며(오설정된 staging이
  문을 열지 않게) 비교는 constant-time이다.
