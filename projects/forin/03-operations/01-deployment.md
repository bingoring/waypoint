...
- **발행 전 대조를 `ota.yml`의 차단 게이트로 구현했다(§12.6).** §12.1이 "발행 후 요약의 `runtimeVersion`이 배포된
  빌드의 것과 같은지 확인하는 것이 유일한 방법"이라고 했던 그 확인이, 이제 **되돌릴 수 없는 발행 전에** 일어난다
- 로컬에서 손으로 확인할 때는 `fingerprint:generate` 단독보다 `fingerprint:compare --build-id`가 안전하다. 후자는
  무엇과 비교하는지가 명시되므로 "어떤 트리의 값인가"를 착각할 여지가 적다. **단 CI 게이트로는 쓸 수 없다 —
  §12.6 참조**
...
### 12.6 발행 전 도달 가능성 게이트 (2026-08-15)

§12.4가 실제로 겪은 함정 — 커밋되지 않은 `app.json` 한 줄 때문에 `master`가 출시 IPA와 다른 지문을 계산하는 상태 —
을 **사후 경고가 아니라 사전 차단**으로 막는다. `eas update`에는 un-publish가 없으므로 발행 후 경고는 이미 늦다.

`ota.yml`의 `Publish update` **앞에** 스텝을 넣었다. 플랫폼별로: 채널의 최신 완료 빌드의 `fingerprint.hash`
(`build:list --json`)와 이 커밋의 `fingerprint:generate --build-profile "$CHANNEL" --json`의 `.hash`를 비교한다.
채널명 == 빌드 프로필명은 `deployConfig.test.ts`가 매 push에 강제하므로 `$CHANNEL`을 프로필로 써도 안전하다.

**`eas fingerprint:compare`를 게이트로 쓰지 않은 이유가 핵심이다 — 그 명령은 일치·불일치 모두 exit 0이다.**
지문 입력을 일부러 흔들어 확인했다: 종료 코드는 그대로 0이고 산문만 `matches` → `differs from`으로 바뀐다.
즉 `compare || exit 1`로 썼다면 **게이트가 한 번도 발동하지 않으면서 있는 것처럼 보였을 것이다.** 그래서 해시를
직접 비교하고 양쪽 값을 요약에 찍는다. 이 세션이 반복해서 만난 "잘못된 검증을 통과시키는 도구"의 또 한 사례다.

동작(모킹으로 5경로 전부 `bash -eo pipefail`에서 실행 검증):

| 상황 | 결과 |
|---|---|
| 양 플랫폼 일치 | 통과, 요약에 ✅ + 해시 |
| 한쪽 불일치 | **exit 1**, `::error`, 원인 후보와 해법을 요약에 안내 |
| 한쪽 불일치 + `allow_fingerprint_mismatch` | 통과하되 `::warning` (승인 전 `summarize` 요약에도 ⚠️로 노출) |
| 채널에 완료 빌드 없음 | 통과 — 도달할 대상이 없는 건 정상이다(첫 preview OTA가 그 경우였다) |
| 로컬 지문 계산 실패 | **exit 1** — 무엇에 도달할지 모르는 채로 발행하지 않는다 |

우회 입력을 둔 이유: 한 플랫폼의 빌드만 오래된 경우(예: iOS는 오늘 빌드, Android는 지난주) 그 한쪽이 다른 쪽의
정당한 OTA를 영구히 막게 된다. 9-A의 `SEED_ALLOW_REMOVAL=1`과 같은 패턴 — **기본은 차단, 우회는 명시적이고 기록에
남는다.** 기존 사후 확인 스텝은 남겼다: 게이트는 "이 커밋이 빌드했을 해시"를, 사후 확인은 "`eas update`가 실제로
찍은 `runtimeVersion`"을 본다. §12.4가 도구가 지문 입력을 고쳐 쓴다는 걸 보여준 만큼 **둘은 다른 증거다.**
...
assigned")로 막힌다** — 초대와 그룹 배정은 원자적이지 않다. **수락 직후 같은 호출이 204로 통과했고**
테스터 상태가 `NOT_INVITED` → `INVITED`로 바뀌었다(2026-08-17).

**이 경로는 테스터에게 메일이 두 통 간다.** 1통차는 App Store Connect **팀 초대**(링크가
`developer.apple.com` 로그인으로 간다 — 테스터는 이걸 TestFlight로 기대하므로 혼란 지점이다),
2통차가 실제 **TestFlight 초대**다. 외부 테스터 경로는 1통뿐이지만 Beta App Review가 붙는다. 즉
**§12.3의 "내부는 즉시"는 심사 대기가 없다는 뜻이지 단계가 없다는 뜻이 아니다.** 테스터를 안내할 때
이 두 통을 미리 알려야 한다.

**TestFlight는 기기의 App Store(미디어 및 구입) Apple ID를 기준으로 앱을 노출한다** — 초대한 주소와
기기 로그인 계정이 다르면 목록에 뜨지 않는다. 어긋나면 기기 계정을 바꾸는 것보다 **실제 Apple ID로
재초대하는 편이 싸다.**
...
**남은 미실행**: 실기기 검증(소셜 로그인 3종·마이크 권한·prod 커리큘럼) — 테스터 2명 모두 그룹에 들어갔고
한 명은 `INSTALLED`, 한 명은 `INVITED`다 · **Android 전량**(Play 신원확인·서비스 계정 키 — 이번 확인에서도 로컬·GCP 어디에도 키가 없음을 재확인했다.
android 빌드는 여전히 0개다).
