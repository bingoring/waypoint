...
- 이월 확장(사용자 설명 "이월되는 것에 대해 많은 것을 고려"): 모든 달·연도 경계에 같은 규칙(12월→1월, 28일 2월, 30일 달 테스트), 다음 달이 이미 있을 때 앞 달 수정의 경계 검사, 주말 미배정 연속 개월 수, D·E·N 3개월 누적, 교육 연간 횟수, 잔여 초과. 검증: 단위 230 · 통합 35 · E2E 9.
- 요구사항 원문 추적 점검: 절마다 검증 위치를 대조해 Build Spec §5에 표로 남겼다. 빠진 원문 예시·패턴·휴일 종류 테스트와 경조휴가 일수·트레이닝 기본 종료일을 추가했다(단위 255).
- 추가 반영(승인 전 사용자 답변): 달을 걸친 주말은 토요일이 속한 달로 센다. 한 사람의 D·E·N 차이가 2를 넘으면 소프트 경고(규칙 설정에서 조정). 검증: 단위 216 · 통합 35 · E2E 9.
