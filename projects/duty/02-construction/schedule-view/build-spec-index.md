...
- [ ] 화면 컴포넌트 + 인쇄 CSS
- [ ] 공통 셸 v2: 메뉴 재구성(`/adjust` 자리표시, `/leave`·`/admin/adjust` 제거) + 「내 휴가 잔여」 카드 + nav 단위 테스트·E2E 갱신(관리자 메뉴 3개)
...
- [ ] E2E: 모든 화면 사이드바에 「내 휴가 잔여」, 간호사도 `근무 조정` 메뉴, 관리자 메뉴 3개. 로그인 → `/?ym=2026-10` 격자 11행·내 줄 2번째 행·카드 5개, `›`/`‹` 이동, 근무표 없는 달 빈 상태, `emulateMedia('print')`에서 사이드바·카드 숨김
